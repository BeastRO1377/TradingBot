...
class MLXInferencer:
    ...
    def persist(self, model_path: str = "golden_model_mlx.safetensors", scaler_path: str = "scaler.pkl"):
        """Сохранение весов и скейлера (если поддерживается)."""
        try:
            if self.model is not None and hasattr(self.model, "save_weights"):
                self.model.save_weights(model_path)
            if self.scaler is not None:
                import pickle
                Path(scaler_path).write_bytes(pickle.dumps(self.scaler))
            logger.info("[MLX] weights/scaler saved")
        except Exception as e:
            logger.warning(f"[MLX] persist failed: {e}", exc_info=True)
