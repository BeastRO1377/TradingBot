...
# ----------------------------------------------------------------------------------------
    ...
    async def evaluate_entry_candidate(self, candidate: dict, features: dict):
        ...
        try:
            ...
            side = candidate.get("side") or candidate.get("action") or ""
            if symbol:
                pos = self.open_positions.get(symbol)
                if pos and safe_to_float(pos.get("volume", 0)) > 0:
                    logger.info(f"[ENTRY_GUARD] already open position for {symbol}, skip")
                    return False
                signal_key = f"{symbol}:{side}:{int(time.time())//30}"
                if self._is_pending(signal_key):
                    logger.info(f"[ENTRY_GUARD] pending exists for {signal_key}, skip")
                    return False
                #self._set_pending(signal_key, ttl=6.0)
        except Exception:
            pass
    ...
    async def place_unified_order(
            ...
            *,
            comment: str = "",
            orderLinkId: str | None = None,     # ← новый аргумент
            order_link_id: str | None = None,   # ← альтернативное имя
            **kwargs,                           # ← на будущее (price, tif, reduceOnly и т.п.)
        ):
        """
        Единая точка выставления ордеров (Unified V5) с поддержкой idempotency через orderLinkId.
        """
        # 1) Нормализуем client id
        _cli_id = (orderLinkId or order_link_id or
                kwargs.pop("orderLinkId", None) or kwargs.pop("order_link_id", None))

        # 2) Базовые параметры (для USDT-перпов — category=linear)
        params = {
            "category": "linear",
            "symbol": symbol,
            "side": side,              # "Buy" / "Sell"
            "orderType": order_type,   # "Market" / "Limit"
            "qty": str(qty),           # Bybit любит строки
        }
        if _cli_id:
            params["orderLinkId"] = _cli_id

        # 3) Прокидываем дополнительные поля, если заданы (price, timeInForce, reduceOnly и т.д.)
        for k, v in kwargs.items():
            if v is not None and k not in params:
                params[k] = v

        logger.debug(f"[BYBIT_REQ] place_order params={{ {k: params[k] for k in params if k != 'price'} }}")

        # 4) Вызов клиента Bybit (пытаемся найти подходящий объект)
        _resp = None
        try:
            if hasattr(self, "rest") and hasattr(self.rest, "place_order"):
                _resp = await self.rest.place_order(**params)
            elif hasattr(self, "unified") and hasattr(self.unified, "place_order"):
                _resp = await self.unified.place_order(**params)
            elif hasattr(self, "client") and hasattr(self.client, "place_order"):
                _resp = await self.client.place_order(**params)
            else:
                raise RuntimeError("Нет подходящего клиента для place_order (rest/unified/client)")
        except TypeError:
            # если клиент синхронный — выполним в пуле
            import asyncio, functools
            target = None
            if hasattr(self, "rest") and hasattr(self.rest, "place_order"):
                target = self.rest.place_order
            elif hasattr(self, "unified") and hasattr(self.unified, "place_order"):
                target = self.unified.place_order
            elif hasattr(self, "client") and hasattr(self.client, "place_order"):
                target = self.client.place_order
            if target is None:
                raise
            _resp = await asyncio.to_thread(functools.partial(target, **params))

        # 5) Обработка ответа + идемпотентность
        try:
            rc = int(_resp.get("retCode", 0))
        except Exception:
            rc = 0
        ret_msg = str(_resp.get("retMsg", ""))

        if rc == 0:
            return _resp

        # Дубликат по orderLinkId — считаем успехом (идемпотентность)
        if _cli_id and (rc == 10006 or "duplicate" in ret_msg.lower()):
            logger.info(f"[BYBIT_IDEMPOTENT] duplicate orderLinkId={_cli_id} -> treat as success")
            return _resp

        raise RuntimeError(f"Bybit place_order failed: retCode={rc}, retMsg={ret_msg}, resp={str(_resp)[:300]}")
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if side == "Buy": # Ищем самую низкую цену
                ...
                if last_price > best_price_seen * 1.001: # Отскок на 0.1%
                    ...
                    await self.place_unified_order_guarded(symbol, side, qty, "Market")
                    entry_made = True
                    break
