...
class TradingBot:
    ...
    async def subscribe_to_ticker(self, symbol: str):
        """Подписка на обновления по тикеру для указанного символа"""
        try:
            if not self.ws_ticker_stream:
                logger.warning("[subscribe_to_ticker] ws_ticker_stream не инициализирован")
                return

            if symbol not in self.active_position_subscriptions:
                await self.ws_ticker_stream.subscribe(f"tickers.{symbol}", callback=self.handle_ticker_for_symbol)
                self.active_position_subscriptions.add(symbol)
                logger.info(f"[Ticker Subscribe] Подписка на tickers.{symbol} успешно выполнена")
            else:
                logger.debug(f"[Ticker Subscribe] Уже подписан на tickers.{symbol}, пропускаем")
