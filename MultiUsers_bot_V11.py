...
TRAILING_GAP_PERCENT_CUSTOM = Decimal("2.5")
...
MAX_ALLOWED_PRICE_CHANGE = Decimal("2.2")   # 150 % за 5 мин – верхний предел
# ----------------------------------------------------------------------
...
#        self._init_http_session()
        ...
        try:
            ...
            if resp.get("retCode") == 0:
                ...
                await self.log_trade(
                    ...
                    side=side,
                    open_interest=None,
                    action=f"Установлен каст.стоп, новый уровень={stop_price}",
                    result="TrailingStop"
                )
                # Обновляем локальный stop_price только если позиция ещё открыта
                if symbol in self.open_positions:
                    self.open_positions[symbol]["stop_price"] = stop_price
                else:
                    logger.warning(f"[set_fixed_stop_loss] Symbol {symbol} не найден в open_positions, пропускаем обновление")
    ...
    async def process_real_time_close_price(self, data):
        ...
        try:
            ...
            if not symbol:
                return
            close_price = self.safe_decimal(data.get("markPrice", 0))
            self.recent_closes[symbol] = close_price
            self.latest_mark_prices[symbol] = close_price

            if symbol in self.open_positions:
                pos = self.open_positions[symbol]
                # avg_price — это Decimal, хранимый в open_positions при parse_position
                entry_price = pos.get("avg_price")  
                leverage    = pos.get("leverage", self.leverage)  # тоже Decimal
                if entry_price and entry_price != 0:
                    ...
                    # 3) Дальше всё в Decimal
                    pnl_pct = (diff / entry_price * leverage * Decimal("100")) \
                            .quantize(Decimal("0.000001"))
                    if pnl_pct > 0:
                        await self.manage_trailing_stop(symbol, pos, pnl_pct, pos.get("side"))
    ...
    async def _process_ticker_data(self, data):
        ...
        try:
            ...
            if not symbol:
                return
            self.latest_mark_prices[symbol] = self.safe_decimal(data.get("markPrice", 0))
        except Exception as e:
    ...
    def _parse_position(self, position_data):
        ...
        return {
            ...
            "stop_price":   self.safe_decimal(position_data['stopLoss']),
            "stop_set":     False,
        }
    ...
    async def _process_ticker_data(self, data):
        ...
        self.latest_mark_prices[symbol] = self.safe_decimal(raw_markPrice)
    ...
    async def execute_golden_setup_websocket(self, symbol: str):
        ...
        # # ─── Impulse filter ────────────────────────────────────────────
        # if IMPULSE_FILTER_ENABLED and abs(Decimal(str(price_change))) > MAX_ALLOWED_PRICE_CHANGE:
        #     logger.info(
    ...
    async def apply_custom_trailing_stop(self, symbol, pos, leveraged_pnl_percent, side):
        ...
        entry_price = pos.get("entryPrice") or pos.get("avg_price")
        if not isinstance(entry_price, Decimal):
            entry_price = Decimal(str(entry_price))
        if entry_price <= 0:
            return
    ...
    async def open_position(self, symbol: str, side: str, volume_usdt: Decimal, reason: str):
        ...
        async with self.state_lock, self.open_positions_lock:
            ...
            last_price = await self.get_last_close_price(symbol)
            # Приводим к Decimal для согласованной арифметики
            last_price_dec = self.safe_decimal(last_price)
        if not last_price or last_price <= 0:
            ...
            async with self.open_positions_lock:
                if symbol in self.open_positions:
                    del self.open_positions[symbol]
            return
        qty_dec = volume_usdt / last_price_dec
        qty_float = float(qty_dec)
        pos_idx = 1 if side.lower() == "buy" else 2
...
#                    await trading_logic.execute_trading_mode()
    ...
    def get_exchange_positions(self):
        try:
            ...
            exchange_positions = {}
            for pos in positions:
                size = self.safe_decimal(pos.get("size", 0))
                if size == 0:
                    continue
                sym = pos.get("symbol")
                side = pos.get("side", "").capitalize()
                entry_price = self.safe_decimal(pos.get("avgPrice", 0))
        ...
        try:
            ...
            for pos in positions_data:
                ...
                if size > 0 and symbol and side:
                    entry_price = self.safe_decimal(pos.get("entryPrice", 0))
                    mark_price = self.safe_decimal(pos.get("markPrice", 0))
