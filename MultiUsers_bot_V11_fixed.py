...
LIQUIDATION_SIGNAL_THRESHOLD = Decimal("1000")  # порог (USDT) для открытия позиции по ликвидации
LIQUIDATION_MAX_TOTAL_VOLUME = Decimal("1000")     # USDT: максимальный общий объём по стратегиям ликвидаций
LIQUIDATION_POSITION_VOLUME    = Decimal("500")    # USDT: объём одной сделки по ликвидациям
# ----------------------------------------------------------


# --- IMPULSE FILTER ---------------------------------------------------
IMPULSE_FILTER_ENABLED = True          # включаем фильтр
MAX_ALLOWED_PRICE_CHANGE = Decimal("3.0")   # 150 % за 5 мин – верхний предел
# ----------------------------------------------------------------------

# --- MONITORING INTERVALS --------------------------------------------
MONITOR_ACTIVE_INTERVAL = 2   # сек, когда есть открытые позиции
MONITOR_IDLE_INTERVAL   = 10  # сек, когда позиций нет
# ----------------------------------------------------------------------
...
golden_params = {
    "Buy": {
        ...
        "price_change": Decimal("0.7"),
        "volume_change": Decimal("3500"),
        "oi_change": Decimal("0.9"),
    },
    "Sell": {
        "period_iters": Decimal("4"),
        "price_change": Decimal("0.5"),
        "volume_change": Decimal("1300"),
        "oi_change": Decimal("0.8"),
    },
}
...
async def init_user_bots():
    ...
    for uid, user_data in users.items():
        ...
        # Запуск основного торгового цикла для каждого пользователя (включая мониторинг стоп-лоссов)
        bot_instance.loop.create_task(bot_instance.run_bot())
...
#        self._init_http_session()
        ...
        self.MAX_TOTAL_VOLUME = self.safe_decimal(str(max_total_volume))
        self.POSITION_VOLUME = self.safe_decimal(str(position_volume))
        if self.mode == "demo":
            self.session = HTTP(
                demo=True,
        ...
        self.pending_open: set[str] = set()   # ордера, ждём first size>0
        ...
        # ----------------------------------------------------
        # после строки self.recent_liquidations = defaultdict(list)
        self.LIQUIDATION_MAX_TOTAL_VOLUME   = LIQUIDATION_MAX_TOTAL_VOLUME
        self.LIQUIDATION_POSITION_VOLUME    = LIQUIDATION_POSITION_VOLUME
        self.liquidation_open_volume        = Decimal("0")
        self.liquidation_open_positions     = set()  # символы, по которым уже открыты liquidation-сделки

        self.ticker_event_queue = asyncio.Queue()
        self.ticker_worker_task = None
        self.disconnect_times = []
        self.reconnect_throttle_seconds = 30  # сколько секунд окно наблюдения
        self.disconnect_threshold = 3          # сколько разрывов за окно допустимо
        self._last_snapshot_hash: int | None = None   # ↲ новое поле
        self.stop_checks_silenced_until = 0
        ...
        async with self.open_positions_lock:
            for sym, pos in self.open_positions.items():
                try:
                    ...
                    vol = self.safe_decimal(str(vol_str))
                    total_vol += vol
                except Exception as e:
    ...
    async def prepare_features_for_model(self, df: pd.DataFrame):
        try:
            ...
            if 'volume' not in df.columns:
                raise ValueError("Missing 'volume' column in input data.")
    ...
    def get_selected_symbols(self):
        ...
        if not isinstance(tickers_resp, dict):
            logger.error("[get_selected_symbols] get_tickers вернул некорректный тип (ожидался dict).")
            self.selected_symbols = []
        ...
        if not isinstance(inst_resp, dict):
            logger.error("[get_selected_symbols] get_instruments_info вернул некорректный тип (ожидался dict).")
            self.selected_symbols = []
            return self.selected_symbols
            ...
            for tk in tickers_data:
                ...
                if "USDT" in sym and "BTC" not in sym and "ETH" not in sym:
                    if not trading_status.get(sym, False):
                        continue
                    turnover24 = self.safe_decimal(str(tk.get("turnover24h", "0")))
                    volume24 = self.safe_decimal(str(tk.get("volume24h", "0")))
    ...
    async def run_bot(self):
        ...
        try:
            ...
            #self.stop_check_task = asyncio.create_task(self.stop_checker_loop())
            self.stop_check_task = asyncio.create_task(self.check_stop_conditions())
            ...
            self.watchdog_task = asyncio.create_task(self.websocket_watchdog())
    ...
    async def update_initial_positions(self):
        ...
        try:
            ...
            for pos in positions:
                try:
                    size = self.safe_decimal(pos['size'])
                except Exception:
    ...
    async def websocket_watchdog(self):
        """
        Мониторит количество разрывов WebSocket.
        Если их становится слишком много за короткое время — ставит бота на паузу.
        """
        logger.info("[watchdog] Запущен контроль стабильности WebSocket")
        while self.active:
            try:
                now = time.time()
                # Очищаем записи старше заданного окна
                self.disconnect_times = [t for t in self.disconnect_times if now - t < self.reconnect_throttle_seconds]

                if len(self.disconnect_times) >= self.disconnect_threshold:
                    logger.warning(f"[watchdog] Обнаружено {len(self.disconnect_times)} разрывов соединения за {self.reconnect_throttle_seconds}с – ставлю бота на паузу")
                    self.trading_paused = True
                    await telegram_bot.send_message(self.user_id, "⚠️ Слишком много разрывов WebSocket – торговля временно приостановлена")

                    await asyncio.sleep(60)  # Ждём минуту перед повторной попыткой

                    self.disconnect_times.clear()
                    self.trading_paused = False
                    await telegram_bot.send_message(self.user_id, "✅ Попытка восстановления WebSocket и возобновления торговли")

                await asyncio.sleep(10)

            except Exception as e:
                logger.error(f"[watchdog] Ошибка в watchdog: {e}")
                await asyncio.sleep(5)

    async def init_websocket(self):
        def _on_ws_error(error):
            logger.warning(f"[WebSocket Error] Обнаружен разрыв: {error}")
        ...
        self.ws_public.on_error = _on_ws_error
        self.ws_private.on_error = _on_ws_error
    ...
    async def handle_liquidation(self, message: dict):
        ...
        try:
            ...
            for itm in entries:
                sym   = itm.get("symbol")
                price = self.safe_decimal(str(itm.get("price", "0")))
                size  = self.safe_decimal(str(itm.get("size",  "0")))
                if not sym or price == 0 or size == 0:
                    continue
                usd_val = price * size
                self.recent_liquidations[sym].append((now_ts, usd_val))
                logger.info(f"Найдена ликвидация {sym} на общую сумму {usd_val}")

            if usd_val >= self.LIQUIDATION_SIGNAL_THRESHOLD:
                # ограничиваем только по «ликвидационному» капу
                if self.liquidation_open_volume + self.LIQUIDATION_POSITION_VOLUME > self.LIQUIDATION_MAX_TOTAL_VOLUME:
                    logger.info(f"[liquidation_signal] {sym}: reached liquidation cap {self.liquidation_open_volume}/{self.LIQUIDATION_MAX_TOTAL_VOLUME}")

                # направление противоположное ликвидированной позиции
                position_side = itm.get("side")
                new_side = "Sell" if position_side == "Buy" else "Buy"
                logger.info(f"[liquidation_signal] {sym}: large liquidation {usd_val}USDT, opening {new_side}")

                # резервируем объём
                self.liquidation_open_positions.add(sym)
                self.liquidation_open_volume += self.LIQUIDATION_POSITION_VOLUME

                # открываем сделку
                asyncio.create_task(
                    self.open_position(sym, new_side, self.LIQUIDATION_POSITION_VOLUME, reason="LiquidationSignal")
                )
    ...
    async def recalculate_supertrend(self, symbol: str):
        """
        Re‑calculate fast / slow / confirm SuperTrend lines and store them
        alongside candle data so that prepare_features_for_model can always
        build st_*_delta and above_* features.
        """
        try:
            df = self.candles_data.get(symbol)
            if df is None or df.empty:
                logger.debug(f"[recalculate_supertrend] no candles for {symbol}")
                return

            # Fast, slow, confirm ST
            st_fast = await self.calculate_supertrend_universal(df.copy(),
                                                                length=FAST_ST_LENGTH,
                                                                multiplier=FAST_ST_MULTIPLIER,
                                                                use_wilder_atr=False)
            st_slow = await self.calculate_supertrend_universal(df.copy(),
                                                                length=SLOW_ST_LENGTH,
                                                                multiplier=SLOW_ST_MULTIPLIER,
                                                                use_wilder_atr=False)
            st_conf = await self.calculate_supertrend_universal(df.copy(),
                                                                length=CONFIRM_ST_LENGTH,
                                                                multiplier=CONFIRM_ST_MULTIPLIER,
                                                                use_wilder_atr=False)

            # Merge back into main dataframe
            full = df.copy()
            full["st_fast"] = st_fast["supertrend"]
            full["st_slow"] = st_slow["supertrend"]
            full["st_conf"] = st_conf["supertrend"]

            # Keep max 500 rows
            self.candles_data[symbol] = full.tail(500).reset_index(drop=True)

        except Exception as e:
            logger.error(f"[recalculate_supertrend] {symbol}: {e}")
    ...
    async def process_real_time_close_price(self, data):
        """Обработка тикера в реальном времени с постановкой в очередь"""
        try:
            ...
            if symbol in self.open_positions:
                pos = self.open_positions[symbol]
                try:
                    entry_price = self.safe_decimal(str(pos.get('avg_price')))
                    close_decimal = self.safe_decimal(close_price)
                    leverage = self.safe_decimal(str(pos.get('leverage', self.leverage)))

                    if entry_price and entry_price != 0:
                        diff = close_decimal - entry_price if pos.get('side') == 'Buy' else entry_price - self.safe_decimal(str(close_price))
                        leveraged_pnl_percent = (diff / entry_price * leverage * Decimal('100')).quantize(Decimal('0.000001'))

                        if leveraged_pnl_percent > Decimal('0'):
                            # Если есть профит — сразу вызываем управление стопами
                            await self.manage_trailing_stop(symbol, pos, leveraged_pnl_percent, pos.get('side'))
                except Exception as e:
                    logger.error(f"[process_real_time_close_price] Ошибка расчёта PNL для {symbol}: {e}")
            try:
                oi = data.get("openInterest", 0)
                self.latest_open_interest[symbol] = self.safe_decimal(str(oi))
            except Exception as e:
                logger.error(f"[process_real_time_close_price] Invalid openInterest for {symbol}: {e}")

            # Ставим событие в очередь для быстрого мониторинга стопа
            await self.ticker_event_queue.put((symbol, close_price))
    ...
    def _parse_position(self, position_data):
        """Нормализует данные позиции из REST или WebSocket в единый формат."""
        # def self.safe_decimal(value, default=Decimal("0")):
            # try:
            #     return self.safe_decimal(str(value))
            # except (TypeError, ValueError, ArithmeticError):
            #     return default

        # --- средняя цена входа (учитываем все варианты названий) -------------
        avg_price_raw = (
            position_data.get("entryPrice")       # WebSocket
            or position_data.get("avgPrice")      # REST
            or position_data.get("avgEntryPrice") # иногда встречается
            or position_data.get("avg_price")     # на всякий случай
            or 0
        )

        # --- markPrice может называться lastPrice в некоторых WS-сообщениях ----
        mark_price_raw = (
            position_data.get("markPrice")
            or position_data.get("lastPrice")
        ...
        return {
            ...
            "size":         self.safe_decimal(position_data['size']),
            "avg_price":    self.safe_decimal(position_data['entryPrice']),
            "entry_price":  self.safe_decimal(position_data['entryPrice']),   # <------------
            "leverage":     self.safe_decimal(position_data['leverage'], Decimal("10")),
            "liq_price":    self.safe_decimal(position_data['liqPrice']),
            "mark_price":   self.safe_decimal(position_data['markPrice']),
    ...
    async def _process_position_update(self, data):
        ...
        # Pybit private `position_stream` emits a message shaped like
        # {"topic": "position", "ts": ..., "data": [ {...}, {...} ]}.
        # Earlier code expected *only* the inner list which led to crashes
        # when the wrapper dict was forwarded here.  We unwrap it so that
        # `data` is always an iterable of position dictionaries.
        ...
        async with self.open_positions_lock:
            for pos in data:
                # ensure we really have a position dictionary
                if not isinstance(pos, dict) or "symbol" not in pos:
                    logger.warning(f"[position_update] skipped non‑position entry: {pos}")
                    continue
                symbol = pos["symbol"]

                # ---- парсим новый размер -------------------------------------------------
                try:
                    new_size = self.safe_decimal(str(pos["size"]))
                except Exception as e:
                ...
                old_size = self.safe_decimal(str(old_pos["size"])) if old_pos else Decimal("0")
    ...
    async def _process_kline(self, symbol, kline: dict):
        ...
        try:
            ...
            close_price = self.safe_decimal(str(kline.get("close")))
            high_price = self.safe_decimal(str(kline.get("high")))
            low_price = self.safe_decimal(str(kline.get("low")))
    ...
    #     try:
    #         if self.ws_public is None:
    #             logger.error(f"[subscribe_symbol] WebSocket public не инициализирован")
    #             return

    #         if symbol in self.active_position_subscriptions:
    #             logger.debug(f"[subscribe_symbol] Уже подписан на {symbol}, пропускаю")
    #             return

    #         # Реальная подписка на тикер
    #         self.ws_public.ticker_stream(
    #             symbol=symbol,
    #             callback=lambda msg: run_coroutine_threadsafe(self.handle_ws_message(msg), self.loop)
    #         )
    ...
    #         logger.info(f"[subscribe_symbol] Подписка на {symbol} успешно выполнена")

    #     except Exception as e:
    #         logger.error(f"[subscribe_symbol] Ошибка подписки на {symbol}: {e}")

    # async def unsubscribe_symbol(self, symbol):
    #     """Отписка от тикера-стрима по символу."""

    #     if symbol not in self.active_subscriptions:
    #         logger.info(f"[unsubscribe_symbol] Символ {symbol} не подписан, пропускаем.")
    #         return

    #     try:
    #         await self.ws_public.stop_ticker_stream(symbol)
    #         self.active_subscriptions.remove(symbol)
    #         logger.info(f"[unsubscribe_symbol] Отписка от тикера {symbol} успешна.")
    #     except Exception as e:
    #         logger.error(f"[unsubscribe_symbol] Ошибка отписки от тикера {symbol}: {e}")
    ...
    async def manage_symbol_subscriptions(self):
        ...
        try:
            if not isinstance(self.selected_symbols, (list, set, tuple)):
                logger.warning(
                    f"[manage_symbol_subscriptions] selected_symbols had invalid type "
                    f"{type(self.selected_symbols)}; resetting to empty list"
                )
                self.selected_symbols = []
        # ----------------------------------
    ...
    async def execute_golden_setup_websocket(self, symbol: str):
        ...
        if current_oi is None:
            ...
            return
        current_vol   = self.safe_decimal(str(df["volume"].iloc[-1]))
        current_price = self.safe_decimal(str(df["closePrice"].iloc[-1]))
        ...
        price_prev = self.safe_decimal(str(df["closePrice"].iloc[-period]))
        if price_prev == 0:
            return
        ...
        sell_p  = self.safe_decimal(str(self.golden_params["Sell"]["price_change"]))
        sell_v  = self.safe_decimal(str(self.golden_params["Sell"]["volume_change"]))
        sell_oi = self.safe_decimal(str(self.golden_params["Sell"]["oi_change"]))
        buy_p   = self.safe_decimal(str(self.golden_params["Buy"]["price_change"]))
        buy_v   = self.safe_decimal(str(self.golden_params["Buy"]["volume_change"]))
        buy_oi  = self.safe_decimal(str(self.golden_params["Buy"]["oi_change"]))

        # ─── Last-two-bars volatility filter ────────────────────────────
        # если период == 4, и оба из двух последних баров изменились более чем на 1% — пропускаем
        # if period == 4:
        #     # рассчитываем %-изменение закрытий
        #     series = df["closePrice"].astype(float)
        #     chg = series.pct_change() * 100
        #     if abs(chg.iloc[-2]) > 1 or abs(chg.iloc[-1]) > 1:
        #         logger.info(
        #             f"[golden_setup] {symbol}: волатильность последних 2 баров "
        #             f"{chg.iloc[-2]:+.2f}%,{chg.iloc[-1]:+.2f}% >1%, сетап пропущен"
        #         )
        #         return None, None
        # # ────────────────────────────────────────────────────────────────
        # price_change = ((current_price - price_prev) / price_prev) * 100
        # # ─── Impulse filter ────────────────────────────────────────────
        # if IMPULSE_FILTER_ENABLED and abs(safe_decimal(str(price_change))) > MAX_ALLOWED_PRICE_CHANGE:
        #     logger.info(
        #         f"[golden_setup] {symbol}: price_change={price_change:+} exceeds cap"
        #         f"{MAX_ALLOWED_PRICE_CHANGE}, setup skipped"
        #     )
        #     return
        # # ───────────────────────────────────────────────────────────────
        ...
        # # ─── Absolute volume growth filter ────────────────────────────────
        # if volume_change > Decimal("25000"):
        #     logger.info(
        #         f"[golden_setup] {symbol}: абсолютный рост объёма {volume_change:.2f} > 25000, сетап пропущен"
        #     )
        #     return None, None
        # # ────────────────────────────────────────────────────────────────────
        ...
        for symbol, pos in positions_snapshot:
            try:
                ...
                if raw_price is None:
                    continue
                try:
                    current_price = self.safe_decimal(str(raw_price))
                except Exception as e:
                ...
                if entry_raw is None:
                    continue
                try:
                    entry_price = self.safe_decimal(str(entry_raw))
                except Exception as e:
                ...
                try:
                    leverage = self.safe_decimal(str(raw_leverage))
                except Exception as e:
                ...
                if not isinstance(current_price, Decimal):
                    current_price = self.safe_decimal(str(current_price))
                if not isinstance(entry_price, Decimal):
                    entry_price = self.safe_decimal(str(entry_price))
                ...
                if not isinstance(diff, Decimal):
                    diff = self.safe_decimal(str(diff))
    ...
    async def set_dynamic_stop(self, symbol, leveraged_pnl_percent, side):
        ...
        async with self.open_positions_lock:
            ...
            if not isinstance(leverage, Decimal):
                leverage = self.safe_decimal(str(leverage))
    ...
    async def manage_trailing_stop(self, symbol, pos, leveraged_pnl_percent, side):
        ...
        try:
            ...
            #elif pos.get('stop_set') and pos.get('stop_price') and self.should_move_stop(pos, self.safe_decimal(str(self.latest_closes.get(symbol, 0)))):
            #    await self.move_stop(symbol, self.safe_decimal(str(self.latest_closes.get(symbol, 0))), side)
        ...
        async with self.open_positions_lock:
            for symbol, pos in self.open_positions.items():
                try:
                    ...
                    try:
                        leverage = self.safe_decimal(str(raw_lev))
                    except Exception:
    ...
    async def update_supertrend_from_ws(self, symbol: str, new_tick: dict, length: int = 10, multiplier: float = 3.0, use_wilder_atr: bool = False) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            price = self.safe_decimal(str(new_tick.get("price", "0")))
            high = self.safe_decimal(str(new_tick.get("high", "0")))
            low = self.safe_decimal(str(new_tick.get("low", "0")))
    ...
    async def adjust_quantity(self, symbol: str, raw_qty: float) -> float:
        ...
        min_qty = self.safe_decimal(str(lot.get("minOrderQty", "0")))
        qty_step = self.safe_decimal(str(lot.get("qtyStep", "1")))
        max_qty = self.safe_decimal(str(lot.get("maxOrderQty", "9999999")))
        min_order_value = self.safe_decimal(str(info.get("minOrderValue", 5)))
        ...
        if not last_price or last_price <= 0:
            ...
            return 0.0
        price_dec = self.safe_decimal(str(last_price))
        dec_qty = self.safe_decimal(str(raw_qty))
        ...
        try:
            ...
            for sym, pos in positions_copy.items():
                if pos.get("trailing_stop_set"):
                    continue
                side = pos["side"]
                entry_price = self.safe_decimal(str(pos["avg_price"]))
                current_price = await self.get_last_close_price(sym)
                if current_price is None:
                    continue
                cp = self.safe_decimal(str(current_price))
                if side.lower() == "buy":
        ...
        async with self.open_positions_lock:
            for symbol, pos in self.open_positions.items():
                if pos.get("insurance_stop_set"):
                    continue
                entry_price = self.safe_decimal(str(pos.get("avg_price", "0")))
    ...
    async def set_stop_loss_to_fast_st(self, symbol: str, side: str):
        ...
        new_fast_st = self.safe_decimal(str(st_df["supertrend"].iloc[-1]))
    ...
    async def set_trailing_stop(self, symbol, size, trailing_gap_percent, side):
        try:
            ...
            if not pos_idx:
                return
            avg_price = self.safe_decimal(str(pos_info.get("avgPrice", "0")))
            if avg_price <= 0:
                return
            trailing_distance_abs = (avg_price * self.safe_decimal(str(trailing_gap_percent))).quantize(Decimal("0.0000001"))
    ...
    async def open_averaging_order(self, symbol: str, side: str, volume_usdt: Decimal, reason: str):
        ...
        qty_dec = volume_usdt / self.safe_decimal(str(last_price))
    ...
    async def open_position(self, symbol: str, side: str, volume_usdt: Decimal, reason: str):
        ...
        async with self.state_lock, self.open_positions_lock:
            current_total = sum(safe_decimal(str(pos.get("position_volume", 0))) for pos in self.open_positions.values())
...
#                    await trading_logic.execute_trading_mode()
    ...
    async def update_open_positions_from_exch_positions(self, exch_positions: dict):
        ...
        async with self.open_positions_lock, self.state_lock:
            ...
            for sym in list(self.open_positions.keys()):
                if sym not in exch_positions:
                    ...
                    if close_price:
                        cp = self.safe_decimal(str(close_price))
                        avg_price = pos.get("entryPrice")
                        if avg_price is None:
                            logger.warning(f"avg_price для {sym} не задано, пропускаем расчёт PnL.")
                            continue
                        try:
                            ep = self.safe_decimal(str(avg_price))
                        except Exception as e:
                            logger.error(f"Ошибка конвертации avg_price для {sym}: {avg_price} - {e}")
                            continue                        
                        pnl = (cp - ep) / ep * self.safe_decimal(str(pos.get("position_volume", 0))) if pos["side"].lower() == "buy" else (ep - cp) / ep * self.safe_decimal(str(pos.get("position_volume", 0)))
                        if trade_id:
        ...
        async with self.open_positions_lock:
            ...
            total = sum(safe_decimal(str(p["position_volume"])) for p in self.open_positions.values())
...
#            await self.bot.force_refresh_candles()
    ...
    async def handle_golden_setup(self, symbol, df):
        try:
            current_oi = self.safe_decimal(str(df.iloc[-1]["open_interest"]))
            current_vol = self.safe_decimal(str(df.iloc[-1]["volume"]))
            current_price = self.safe_decimal(str(df.iloc[-1]["closePrice"]))
            async with history_lock:
                ...
                oi_prev = open_interest_history[symbol][-period]
                vol_prev = volume_history[symbol][-period]
                price_prev = self.safe_decimal(str(df.iloc[-period]["closePrice"]))
                if price_prev == 0:
                    return None, None
                # ─── Last-two-bars volatility filter ────────────────────────────
                # если период == 4, и оба из двух последних баров изменились более чем на 1% — пропускаем
                ...
                # ─── Absolute volume growth filter ────────────────────────────────
                if volume_change > Decimal("9000"):
                    logger.info(
                        f"[golden_setup] {symbol}: абсолютный рост объёма {volume_change:.2f} > 9000, сетап пропущен"
                    )
                    return None, None
                # ────────────────────────────────────────────────────────────────────
    ...
    async def execute_st_cross1(self):
        ...
        for sym in symbols:
            ...
            curr_diff = curr_fast - curr_slow
            last_close = df_fast.iloc[-1]["closePrice"]
            curr_diff_pct = (safe_decimal(curr_diff) / self.safe_decimal(last_close)) * 100
            margin = 0.01
            first_cross_up = prev_diff <= 0 and curr_diff > 0
            first_cross_down = prev_diff >= 0 and curr_diff < 0
            if first_cross_up:
...
async def set_max_volume_step2(message: Message, state: FSMContext):
    ...
    try:
        val_dec = self.safe_decimal(new_value_str)
        if val_dec <= 0:
...
async def status_cmd(message: Message):
    ...
    async with bot_instance.open_positions_lock:
        ...
        for sym, pos in bot_instance.open_positions.items():
            side_str = pos["side"]
            entry_price = self.safe_decimal(str(pos["avg_price"]))
            volume_usdt = self.safe_decimal(str(pos["position_volume"]))
            ...
            for user_id, bot in user_bots.items():
                ...
                for symbol, pos in positions.items():
                    ...
                    if profit_perc <= -bot.TARGET_LOSS_FOR_AVERAGING:
                        ...
                        if profit_perc <= -bot.TARGET_LOSS_FOR_AVERAGING:
                            current_volume = self.safe_decimal(str(bot.open_positions[symbol]["position_volume"]))
