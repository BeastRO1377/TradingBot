...
class TradingBot:
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        prov = (self.user_profile.get("ai_provider") or "ollama").strip().lower()
        # починка потенциальной кириллицы: 'оllama' -> 'ollama'
        prov = prov.replace("о", "o")  # кириллическая 'о' -> латинская 'o'
        self.ai_provider = prov
        self.ai_model = (self.user_profile.get("ai_model") or "llama3.1:8b").strip()
        # используем OpenAI-совместимый эндпоинт, как в ваших логах
        self.ai_endpoint = (self.user_profile.get("ai_endpoint") or "http://localhost:11434/v1/chat/completions").strip()
        self.ai_temperature = float(self.user_profile.get("ai_temperature", 0.15))
...
# ----------------------------------------------------------------------------------------
    ...
    def _core_decision(self, features: dict, user_cfg: dict) -> tuple[bool, str, str]:
        """Возвращает (passed, side, reason) только по ядру: golden/squeeze/(liq)."""
        gp = getattr(self, "golden_params", None)
        def _gp(name, default):
            try: return float(getattr(gp, name))
            except Exception: return float(default)
        trend = abs(features.get("trend_5m", 0.0))
        vol_surge = features.get("volume_surge", 0.0)
        oi_delta = features.get("oi_delta_5m", None)
        min_trend = _gp("min_trend_abs", 0.004)
        min_vol   = _gp("min_volume_surge", 2.0)
        min_oi    = _gp("min_oi_delta", 0.003)
        golden_ok = (trend >= min_trend and vol_surge >= min_vol and (True if oi_delta is None else (oi_delta >= min_oi)))
        if golden_ok:
            return True, "", "core:golden", {"trend": trend, "vol_surge": vol_surge, "oi_delta": oi_delta}
        try:
            sq_th_pct = float(user_cfg.get("squeeze_threshold_pct", 6.0)) / 100.0
        except Exception:
            sq_th_pct = 0.06
        squeeze_state = features.get("squeeze_state")
        squeeze_power = features.get("squeeze_power", 0.0)
        squeeze_atten = bool(features.get("squeeze_attenuation", False))
        if squeeze_state in ("jump", "dive") and squeeze_atten and squeeze_power >= sq_th_pct:
            side = "Sell" if squeeze_state == "jump" else "Buy"
            return True, side, "core:squeeze_reversal", {"state": squeeze_state, "power": squeeze_power, "threshold": sq_th_pct}
        liq_dist = features.get("liq_cluster_dist")
        liq_side = features.get("liq_cluster_side")
        if liq_dist is not None and liq_side in ("Buy", "Sell"):
            liq_th = _gp("liq_proximity_px", 0.002)
            if liq_dist <= liq_th:
                side = "Buy" if liq_side == "Buy" else "Sell"
                return True, side, "core:liq_bounce", {"liq_dist": liq_dist, "liq_th": liq_th, "liq_side": liq_side}
        return False, "", "core:fail", {"trend": trend, "vol_surge": vol_surge, "oi_delta": oi_delta, "squeeze_state": squeeze_state, "squeeze_power": squeeze_power}
                    ...
                    if safe_to_float(exec_data.get("leavesQty", 0)) == 0:
                        ...
                        try:
                            trade = {
                                ...
                                "side": pos["side"],
                                "qty": float(pos_volume),
                                "entry_price": float(safe_to_float(pos["avg_price"])),
                                "exit_price": float(exit_price),
                                "pnl": float(pnl_usdt),
                                "pnl_pct": float(pnl_pct),
                                "reason": "closed_by_execution"
    ...
    #         # --- ML-фильтр ---
    #         try:
    #             # [ИЗМЕНЕНИЕ] Теперь PyTorchInferencer сам обработает скейлер
    ...
    # [ИСПРАВЛЕНО] ВНУТРИ КЛАССА TRADINGBOT
    async def evaluate_entry_candidate(self, candidate: dict, features: dict):
        symbol = (candidate.get("symbol") if isinstance(candidate, dict) else None) or features.get("symbol")
        if not symbol:
            logger.info("[EVAL_DROP] no-symbol candidate")
            return
        ...
        side_hint = (candidate.get("side") or features.get("side") or "").title() or None

        logger.info("[EVAL_IN] %s %s src=%s", symbol, side_hint or "?", source or "?")
        dbg_trace(symbol, "candidate", feat_keys=list(features.keys())[:20], side=side_hint, src=source)

        user_cfg = getattr(self, "user_profile", {}) or {}

        # --- ВАЖНО: не блокируем 'golden_setup' через _core_decision ---
        reason_core = ""
        side_by_core = None
        if source == "golden_setup":
            passed = True
            side_by_core = side_hint  # доверяем направлению из кандидата
            reason_core = "golden_source_bypass"
            core_details = {"bypass": True}
        else:
            passed, side_by_core, reason_core, core_details = self._core_decision(features, user_cfg)

        dbg_trace(symbol, "core_result", passed=passed, side=side_by_core, reason=reason_core, details=core_details)

        if not passed:
            logger.info("[EVAL_DROP] %s by CORE (%s)", symbol, reason_core)
            return

        try:
            self.stats["core_pass"] += 1
        except Exception:
            self.stats["core_pass"] = self.stats.get("core_pass", 0) + 1

        side = side_by_core or side_hint or "Buy"

        # вспомогательные признаки (не блок)
        aux = {
            "rsi": features.get("rsi"),
            "ma_bias": features.get("ma_bias"),
            "bb_width": features.get("bb_width"),
            "atr": features.get("atr"),
        }

        # --- AI подтверждение (локальная Ollama) ---
        ai_ok, ai_conf, ai_msg = True, 0.7, ""
        try:
            r = await self.ai_confirm(symbol, side, features, aux)
            if isinstance(r, tuple):
                if len(r) == 3:
                    ai_ok, ai_conf, ai_msg = bool(r[0]), float(r[1] or 0.0), str(r[2] or "")
                elif len(r) == 2:
                    ai_ok, ai_conf = bool(r[0]), float(r[1] or 0.0)
                else:
                    ai_ok = bool(r[0])
            elif isinstance(r, dict):
                ai_ok  = bool(r.get("approve", False))
                ai_conf = float(r.get("confidence", 0.5) or 0.5)
                ai_msg  = str(r.get("reason", ""))
            else:
                ai_ok = bool(r)
        except Exception as e:
            logger.exception(f"[AI_ERROR] {symbol}/{side} confirm failed: {e}")
            ai_ok = False

        dbg_trace(symbol, "ai_result", ok=ai_ok, conf=ai_conf, msg=ai_msg)
        if not ai_ok:
            logger.info("[AI_DECLINE] %s/%s conf=%.2f msg=%s", symbol, side, ai_conf, ai_msg)
            return

        try:
            ok, reason_risk = await self._risk_checks_before_entry(symbol, side, features)
        except TypeError:
            ok, reason_risk = await self._risk_checks_before_entry(symbol, side)

        dbg_trace(symbol, "risk_result", ok=ok, reason=reason_risk)
        if not ok:
            try:
                self.stats["risk_block"] += 1
            except Exception:
                self.stats["risk_block"] = self.stats.get("risk_block", 0) + 1
            logger.info("[RISK_BLOCK] %s/%s -> %s", symbol, side, reason_risk)
            return

        comment = ai_msg or reason_core
        logger.info("[EXECUTE] %s/%s: Все проверки пройдены. Отправка ордера...", symbol, side)

        try:
            await self.execute_trade_entry(symbol, side, features, comment_for_log=comment)
            try:
                self.stats["executed"] += 1
            except Exception:
                self.stats["executed"] = self.stats.get("executed", 0) + 1
        except TypeError:
            await self.execute_trade_entry(symbol, side, features)
            try:
                self.stats["executed"] += 1
            except Exception:
                self.stats["executed"] = self.stats.get("executed", 0) + 1
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        try:
            ...
            comment_for_log = f"JARVIS' OPINION: {candidate['justification']}"
            ...
            if source == 'squeeze':
                ...
                await self.place_unified_order_guarded(symbol, side, qty, "Market", comment=comment_for_log)

        except Exception as e:
            logger.error(f"[execute_trade] Критическая ошибка для {symbol}: {e}", exc_info=True)
            # Если что-то пошло не так, очищаем pending
            self.pending_orders.pop(symbol, None)
            self.pending_timestamps.pop(symbol, None)
    ...
    async def evaluate_candidate_with_ollama(self, candidate: dict, features: dict) -> dict:
        ...
        try:
            ...
            def _format(v, spec):
        def _safe_candles(sym: str):
            try:
                return list(self.shared_ws.candles_data.get(sym, []))
            except Exception:
                return []
        def _safe_pct(sym: str, minutes: int):
            cds = _safe_candles(sym)
            try:
                return compute_pct(cds, minutes)
            except Exception:
                return 0.0
        
        import json
        import math return f"{v:{spec}}" if isinstance(v, (int, float)) else "N/A"
            m, source = candidate.get('base_metrics', {}), candidate.get('source', 'unknown').replace('_', ' ').title()

            # --- Сбор данных для продвинутого промпта ---
            vol_base = float(m.get('vol_change_pct', 0.0) or 0.0)
            vol_anomaly = vol_base / 100.0 + 1.0
            trend = "Uptrend" if (features.get('supertrend') or 0) > 0 else "Downtrend"
            btc_change_1h = _safe_pct("BTCUSDT", 60)
            eth_change_1h = _safe_pct("ETHUSDT", 60)
    ...
    async def _golden_logic(self, symbol: str):
        ...
        try:
            ...
            if self._squeeze_allowed(symbol) and self.shared_ws.has_5_percent_growth(symbol, minutes=20):
                # избегаем «гонки за свечой», правило из вашей логики
                return
            await self.ensure_symbol_meta(symbol)
            if symbol in self.failed_orders and time.time() - self.failed_orders[symbol] < 600:
                return
            ...
            ok_sell_len = (len(recent) > sell_period and len(vol_hist_5m) > sell_period and len(oi_hist_5m) > sell_period)
            price_change_pct_sell = volume_change_pct_sell = oi_change_pct_sell = 0.0
            sell_ok = False
            if ok_sell_len:
                p0 = safe_to_float(recent[-1 - sell_period]["closePrice"]) or 0.0
                p1 = safe_to_float(recent[-1]["closePrice"]) or 0.0
                v0 = safe_to_float(vol_hist_5m[-1 - sell_period]) or 0.0
                v1 = safe_to_float(vol_hist_5m[-1]) or 0.0
                o0 = safe_to_float(oi_hist_5m[-1 - sell_period]) or 0.0
                o1 = safe_to_float(oi_hist_5m[-1]) or 0.0

                price_change_pct_sell  = 100.0 * ((p1 / p0) - 1.0) if p0 > 0 else 0.0
                volume_change_pct_sell = 100.0 * ((v1 / v0) - 1.0) if v0 > 0 else 0.0
                oi_change_pct_sell     = 100.0 * ((o1 / o0) - 1.0) if o0 > 0 else 0.0

                sell_ok = (
                    price_change_pct_sell <= -sell_params["price_change"] and
            ...
            ok_buy_len = (len(recent) > buy_period and len(vol_hist_5m) > buy_period and len(oi_hist_5m) > buy_period)
            price_change_pct_buy = volume_change_pct_buy = oi_change_pct_buy = 0.0
            buy_ok = False
            if ok_buy_len:
                p0 = safe_to_float(recent[-1 - buy_period]["closePrice"]) or 0.0
                p1 = safe_to_float(recent[-1]["closePrice"]) or 0.0
                v0 = safe_to_float(vol_hist_5m[-1 - buy_period]) or 0.0
                v1 = safe_to_float(vol_hist_5m[-1]) or 0.0
                o0 = safe_to_float(oi_hist_5m[-1 - buy_period]) or 0.0
                o1 = safe_to_float(oi_hist_5m[-1]) or 0.0

                price_change_pct_buy  = 100.0 * ((p1 / p0) - 1.0) if p0 > 0 else 0.0
                volume_change_pct_buy = 100.0 * ((v1 / v0) - 1.0) if v0 > 0 else 0.0
                oi_change_pct_buy     = 100.0 * ((o1 / o0) - 1.0) if o0 > 0 else 0.0

                buy_ok = (
                    price_change_pct_buy >= buy_params["price_change"] and
            ...
            # --- Диагностика решения ---
            logger.debug(
                "[GOLDEN_DECISION] %s buy_ok=%s sell_ok=%s "
                "buy(p=%.3f%% v=%.1f%% oi=%.3f%% thr=%.3f/%.1f/%.3f) "
                "sell(p=%.3f%% v=%.1f%% oi=%.3f%% thr=%.3f/%.1f/%.3f) "
                "liq: side=%s val=%.1f thr=%.1f",
                symbol, buy_ok, sell_ok,
                price_change_pct_buy,  volume_change_pct_buy,  oi_change_pct_buy,
                buy_params["price_change"],  buy_params["volume_change"],  buy_params["oi_change"],
                price_change_pct_sell, volume_change_pct_sell, oi_change_pct_sell,
                sell_params["price_change"], sell_params["volume_change"], sell_params["oi_change"],
                liq_side, liq_val, threshold
            )

            # --- Выбор стороны: сначала Buy, затем Sell ---
            action = None
            if buy_ok:
                action = "Buy"
                price_change_pct, volume_change_pct, oi_change_pct = (
                    price_change_pct_buy, volume_change_pct_buy, oi_change_pct_buy
                )
                logger.info(f"{symbol} имеет следующие характеристики: {volume_change_pct_buy}, {price_change_pct_buy}, {oi_change_pct_buy}")
            elif sell_ok:
                action = "Sell"
                price_change_pct, volume_change_pct, oi_change_pct = (
                    price_change_pct_sell, volume_change_pct_sell, oi_change_pct_sell
                )
                logger.info(f"{symbol} имеет следующие характеристики: {volume_change_pct_sell}, {price_change_pct_sell}, {oi_change_pct_sell}")
            else:
                # Явно логируем, что именно не прошло
                logger.debug(
                    "[GOLDEN_SKIP] %s buy_ok=%s sell_ok=%s (см. GOLDEN_DECISION выше)",
                    symbol, buy_ok, sell_ok
                )
                return

            # --- Если есть сигнал, готовим кандидат ---
            features = await self.extract_realtime_features(symbol)
            if not features:
            ...
            logger.info("[Signal Candidate] Golden Setup: %s on %s", action, symbol)
            await self.evaluate_entry_candidate(candidate, features)
            return
