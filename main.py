...
def load_users_from_json(json_path: str | Path | None = None) -> List[Dict[str, Any]]:
    path = Path(json_path) if json_path else Path(config.USER_STATE_FILE)
    if not path.exists():
        return []
...
def load_golden_params(csv_path: str | Path | None = None) -> dict:
    default_params = {
    ...
    path = Path(csv_path) if csv_path else Path(config.GOLDEN_PARAMS_CSV)
    if path.exists():
        try:
            df = pd.read_csv(path)
            overrides = {}
...
def _resolve_config_source_path() -> Path:
    cfg_path = Path(config.__file__).resolve()
    if cfg_path.suffix == ".pyc":
        candidate = cfg_path.with_suffix(".py")
        if candidate.exists():
            cfg_path = candidate
    return cfg_path
...
async def watch_config_changes(
    shared_ws: PublicWebSocketManager,
    golden_param_store: Dict[Any, Any],
    interval: float = 5.0,
) -> None:
    cfg_path = _resolve_config_source_path()
    try:
        last_mtime = cfg_path.stat().st_mtime
    except FileNotFoundError:
        logger.warning("Config watcher: файл %s пока не найден.", cfg_path)
        last_mtime = None
    else:
        logger.info("Config watcher отслеживает %s", cfg_path)
    ...
    while True:
        ...
        try:
            ...
            if last_mtime is not None:
                logger.warning("Config watcher: файл %s недоступен.", cfg_path)
            last_mtime = None
            continue
        ...
        logger.info("Обнаружено обновление config.py (mtime=%s). Перезагружаю модуль...", current_mtime)
        try:
            importlib.reload(config)
        except Exception as e:
            logger.error("Не удалось перезагрузить config.py: %s", e, exc_info=True)
            continue

        try:
            config.setup_logging()
        except Exception as e:
            logger.error("Ошибка при повторной настройке логирования после обновления config: %s", e, exc_info=True)

        try:
            shared_ws.on_config_reload()
        except Exception as e:
            logger.error("Ошибка обновления PublicWebSocketManager после reload config: %s", e, exc_info=True)

        if golden_param_store is not None:
            try:
                updated_store = load_golden_params()
                golden_param_store.clear()
                golden_param_store.update(updated_store)
                logger.info("Обновлен golden_param_store из config.")
            except Exception as e:
                logger.error("Не удалось обновить golden_param_store после reload config: %s", e, exc_info=True)

        for bot in list(GLOBAL_BOTS):
            try:
                bot.on_config_reload()
            except Exception as e:
                logger.error(
                    "Не удалось обновить настройки бота %s после reload config: %s",
                    getattr(bot, "user_id", "unknown"),
                    e,
                    exc_info=True,
                )
...
async def run_all() -> None:
    ...
    config_watch_task = asyncio.create_task(
        watch_config_changes(shared_ws=shared_ws, golden_param_store=golden_param_store)
    )
    ...
    try:
        ...
        if config_watch_task:
            all_tasks.append(config_watch_task)
