...
# === MLX-only build: disable PyTorch symbols to satisfy static analysis & avoid runtime ===
try:
    from types import SimpleNamespace
except Exception:
    class SimpleNamespace: pass  # fallback

def __torch_disabled__(*args, **kwargs):
    raise RuntimeError("PyTorch is disabled in this build. Use MLX-only paths.")

# Minimal dummies so Pylance doesn't flag undefined names if legacy code remains
torch = SimpleNamespace(
    no_grad=lambda: (lambda f: f),
    cuda=SimpleNamespace(is_available=lambda: False),
    tensor=__torch_disabled__,
    from_numpy=__torch_disabled__,
    save=__torch_disabled__,
    load=__torch_disabled__,
    nn=SimpleNamespace()  # legacy compatibility
)
nn = SimpleNamespace(Module=object)          # legacy compatibility
optim = SimpleNamespace(Adam=__torch_disabled__, AdamW=__torch_disabled__, SGD=__torch_disabled__)
Dataset = object
...
class TradingBot:
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        #self.ml_inferencer = MLXInferencer("golden_model_v19.pt")
        try:
        except Exception:
    ...
    async def predict_action(self, symbol: str) -> str:
        try:
            ...
            if self.MLX_model:
                ...
                prediction = self.MLX_model.predict({"input": input_arr})
