...
def _append_trades_unified(row: dict) -> None:
    """
    Пишет строку в trades_unified.csv; при первом запуске — пишет заголовок.
    """
    import os, csv
...
class PublicWebSocketManager:
        ...
        while True:
            try:
                ...
                self.ws.all_liquidation_stream(
                    symbol=list(self.active_symbols),
                    callback=_on_message
                )
    ...
    async def route_message(self, msg):
        ...
        if topic.startswith("kline."):
            ...
            for evt in data:
                try:
                    # Передаем каждое событие каждому боту-обработчику
                    for handler in self.position_handlers:
                        if hasattr(handler, "on_liquidation_event"):
                            # Запускаем обработку в фоне, чтобы не задерживать WS
                            asyncio.create_task(handler.on_liquidation_event(evt))
                except Exception:
                    logger.exception("[route_message] Ошибка при передаче события ликвидации обработчику")
    ...
    async def handle_kline(self, msg):
        ...
        for entry in entries:
            ...
            # <-- NEW: триггерим проверку входа по закрытию свечи
            for bot in self.position_handlers:
                # Запуск в фоне — эквивалентно старому сканеру, но только по этому символу
                asyncio.create_task(bot.execute_golden_setup(symbol))
    ...
    async def place_order_ws(self, symbol, side, qty,
        ...
        # Ждём именно наш ACK с совпадающим req_id, с таймаутом
        ack = None
        while True:
            resp_raw = await asyncio.wait_for(self.ws_trade.recv(), timeout=3.0)
        ...
        logger.info("[ORDER][%s] recv %dms %s", cid or "-", dt_ms, j(ack))

        if ack.get("retCode", 1) != 0:
            logger.error("[ORDER][%s] failed ret=%s msg=%s", cid or "-", ack.get("retCode"), ack.get("retMsg"))
            raise RuntimeError(f"Order failed: {ack}")

        return ack.get("data", ack)
...
class TradingBot:
    ...
    __slots__ = (
        ...
        "squeeze_ai_timeout_sec", "ENABLE_POSITION_FALLBACK_CLOSE", "signal_task_queue", "order_command_queue", "signal_worker_process",
        )
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        self.ENABLE_POSITION_FALLBACK_CLOSE = False
        ...
        # self.last_private_evt_ts = time.time()  # обновляй в handle_position_update / handle_execution
        # self.private_ws_down_until = 0
        ...
        # ─ ликвидации ─
        self.liq_buffers = defaultdict(lambda: deque(maxlen=200))               # symbol -> deque[(ts, side_evt, usdt)]
        ...
        import multiprocessing as mp
        from signal_worker import start_worker_process
        self.signal_task_queue = None
        self.order_command_queue = None
        
        # Запускаем процесс Аналитика, передавая ему только user_id
        self.signal_worker_process = None
        #     target=start_worker_process,
        #     args=(self.signal_task_queue, self.order_command_queue, user_data['user_id']),
        #     daemon=True
        # )
        #self.signal_worker_process.start()
        logger.info("[SignalWorker] Процесс-аналитик запущен с PID: %d", self.signal_worker_process.pid)
    ...
    async def _order_queue_listener(self):
        logger.info("[OrderListener] Запущен и ожидает приказы.")
        loop = asyncio.get_running_loop()
        while True:
            try:
                if not self.order_command_queue.empty():
                    order_data = await loop.run_in_executor(None, self.order_command_queue.get)
                    if order_data and order_data.get("action") == "EXECUTE_ORDER":
                        ...
                        candidate = {
                            ...
                            "source": order_data["source"], "volume_usdt": order_data["volume_usdt"],
                            "justification": order_data.get("justification", "")
                        }
                        asyncio.create_task(self.execute_trade_entry(candidate, {}))
            except Exception as e:
                logger.error(f"[OrderListener] Критическая ошибка: {e}", exc_info=True)
    ...
    async def _calc_qty_from_usd(self, symbol: str, usd_amount: float, price: float | None = None) -> float:
        ...
        try:
            ...
            logger.error(f"[_calc_qty] meta fail for {symbol}: {e}")
            return 0.0

        step    = float(self.qty_step_map.get(symbol, 0.001) or 0.001)
        min_qty = float(self.min_qty_map.get(symbol, step) or step)

        # Цена — сначала WS, потом аргумент/фичи
        p = price or self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0)
        try:
            p = float(p)
        except Exception:
            p = 0.0
        ...
        if not (p and p > 0):
            logger.warning(f"[EXECUTE_ABORT_PRICE] {symbol}: invalid price={p}")
            return 0.0

        raw_qty = float(usd_amount) / p
        qty = (int(raw_qty / step) * step) if step > 0 else raw_qty
        qty = float(f"{qty:.12f}")
        if qty < min_qty:
            logger.debug(f"[_calc_qty] {symbol}: qty({qty})<min({min_qty}) -> use min")
            qty = min_qty
        if qty <= 0:
            logger.warning(f"[EXECUTE_ABORT_QTY_ZERO] {symbol}: usd={usd_amount} price={p} step={step} min={min_qty} raw={raw_qty}")
            return 0.0
        return qty
    ...
    def _dataset_metrics(self, symbol: str) -> dict:
        """
        Возвращает: price (last), open_interest, volume (последняя 1m свеча),
        rsi14, adx14, vol_anom20 (vol/SMA20).
        """
        ...
        # --- Рыночные значения
        price = sfloat(self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0.0))
        oi = sfloat(
            self.shared_ws.latest_open_interest.get(symbol)
            or (self.shared_ws.oi_history.get(symbol, [])[-1] if self.shared_ws.oi_history.get(symbol) else 0.0)
        )

        # --- Свечи для индикаторов
        candles = list(self.shared_ws.candles_data.get(symbol, []))
        df = pd.DataFrame(candles) if candles else pd.DataFrame(columns=[
            "startTime","openPrice","highPrice","lowPrice","closePrice","volume"
        ])
        n = len(df)
        volume_1m = sfloat(df["volume"].iloc[-1]) if n else 0.0

        # Серии
        close = df["closePrice"] if n else pd.Series(dtype="float64")
        high  = df["highPrice"]  if n else pd.Series(dtype="float64")
        low   = df["lowPrice"]   if n else pd.Series(dtype="float64")
        vol   = df["volume"]     if n else pd.Series(dtype="float64")

        # --- RSI(14)
        rsi14 = 50.0
        try:
            if n >= 15:
                rsi_ser = ta.rsi(close, length=14)
                val = rsi_ser.iloc[-1]
                rsi14 = float(val) if pd.notna(val) else 50.0
        except Exception:
            rsi14 = 50.0
        ...
        try:
            if n >= 15:
                ...
                val = adx_df.get("ADX_14").iloc[-1]
                adx14 = float(val) if pd.notna(val) else 0.0
        except Exception:
            adx14 = 0.0

        # --- Volume Anomaly (vol / SMA20(vol))
        vol_anom20 = 1.0
        try:
            if n >= 20:
                base = ta.sma(vol, length=20).iloc[-1]
                base = float(base) if pd.notna(base) else 0.0
                vol_anom20 = (volume_1m / base) if base > 0 else 1.0
        except Exception:
            vol_anom20 = 1.0
    ...
    async def manage_open_position(self, symbol: str):
        ...
        try:
            ...
            # Запускаем автономный воркер
            await self._start_stop_worker(symbol, pos)

            # Основной цикл теперь следит за здоровьем воркера и вызывает AI
            ...
            while symbol in self.open_positions:
                ...
                # Периодически консультируемся с AI
                now = time.time()
                ...
                await asyncio.sleep(5) # Проверяем состояние воркера раз в 5 секунд
    ...
    async def _start_stop_worker(self, symbol: str, pos: dict):
        ...
        if symbol in self._stop_workers:
            logger.warning(f"[Guardian] Попытка запустить дубликат воркера для {symbol}. Пропускаем.")
            return
        ...
        init_params = {
            ...
            "avg_price": float(pos.get("avg_price") or pos.get("entryPrice") or 0.0),
            "leverage": float(pos.get("leverage") or getattr(self, "leverage", 10.0)),
            "tick_size": tick_size,
            ...
            "current_stop": self.last_stop_price.get(symbol, 0.0) # Передаем текущий стоп, если он есть
        }
        ...
        proc = await asyncio.create_subprocess_exec(
            sys.executable, "-u", worker_script_path, init_params_json,
        ...
        logger.info(f"🛡️ [Guardian] Автономный stop_worker для {symbol} запущен.")
    ...
    async def _ai_stop_management_check(self, symbol: str):
        ...
        try:
            ...
            # ... (весь ваш код для сбора last_price, entry_price, roi и т.д. остается здесь) ...
            ...
            if not all([last_price > 0, entry_price > 0, side]): return
            current_roi = (((last_price - entry_price) / entry_price) * 100 * leverage) if side.lower() == "buy" else (((entry_price - last_price) / entry_price) * 100 * leverage)
            pos['pnl'] = current_roi
            ...
            # <<< НАЧАЛО ИЗМЕНЕНИЯ 1: Присваиваем ai_response значение по умолчанию >>>
            ai_response = None
            # <<< КОНЕЦ ИЗМЕНЕНИЯ 1 >>>
            ...
            # <<< НАЧАЛО ИЗМЕНЕНИЯ 2: Добавляем проверку на None >>>
            # Если ai_response все еще None (например, из-за таймаута, который мы не поймали здесь)
            # или это не словарь, создаем пустой словарь.
            ...
            logger.error(f"[AI_STOP_ADVISOR] Таймаут при консультации для {symbol}. Отключаем советника на {cooldown_seconds} секунд.")
        except Exception:
    ...
    async def _read_stop_events(self, symbol: str, proc):
        """
        [V3 - FINAL]
        Читает события от воркера и исполняет их.
        Добавлена финальная проверка существования позиции ПЕРЕД отправкой ордера на биржу.
        """
        try:
            # Используем асинхронный итератор для чтения строк, чтобы не блокироваться
            while proc.returncode is None:
                try:
                    line = await asyncio.wait_for(proc.stdout.readline(), timeout=1.0)
                    if not line:
                        break # Поток закрыт, воркер завершился
                except asyncio.TimeoutError:
                    continue # Просто продолжаем цикл, если за секунду ничего не пришло
                ...
                if et in ("activated", "trail_update", "set_stop", "stop_update"):
                    price = safe_to_float(evt.get("stop"))
                    if price > 0:
                        
                        # <<< НАЧАЛО ФИНАЛЬНОГО ИСПРАВЛЕНИЯ >>>
                        # Перед тем, как отправить запрос на биржу,
                        # проверяем, а числится ли у нас еще эта позиция?
                        if symbol not in self.open_positions:
                            logger.warning(f"[Guardian] {symbol}: Получена команда на обновление стопа от воркера, но позиция уже закрыта в боте. Исполнение отменено.")
                            continue # Пропускаем итерацию, не отправляем запрос
                        # <<< КОНЕЦ ФИНАЛЬНОГО ИСПРАВЛЕНИЯ >>>
                            
                        try:
                            # Эта функция внутри себя имеет обработку ошибки 10001,
                            # что является нашей второй линией защиты.
                            await self.set_or_amend_stop_loss(symbol, price)
                            logger.info("✅ [Guardian] %s стоп обновлен на %.10f", symbol, price)
                        except Exception as e:
                            logger.error("[Guardian] Ошибка при исполнении команды от воркера для %s: %s", symbol, e)

                elif et in ("heartbeat", "hb"):
                    logger.debug("[stop_worker_hb] %s %s", symbol, j(evt))

                elif et == "closed":
                    break # Воркер сообщил, что завершает работу
                    
        except asyncio.CancelledError:
            logger.debug(f"[_read_stop_events] Чтение для {symbol} отменено.")
        except Exception as e:
            logger.error(f"[_read_stop_events] Критическая ошибка для {symbol}: {e}", exc_info=True)
        finally:
    ...
    async def _open_position(self, symbol: str, side: str):
        ...
        try:
            if self.mode == "real":
                ...
                logger.info("[ORDER][%s] DEMO → используем REST place_unified_order", cid)
                resp = await self.place_unified_order(
                    symbol=symbol, side=side, qty=qty,
                    order_type="Market", comment="entry_by_core", cid=cid
                )
            order_id = None
            try:
    ...
    # --- ЗАМЕНИТЕ start в классе TradingBot ---

    async def start(self):
        logger.info(f"[User {self.user_id}] Бот запущен")
        
        # --- ЗАПУСК ДОЧЕРНЕГО ПРОЦЕССА-АНАЛИТИКА ---
        import multiprocessing as mp
        from signal_worker import start_worker_process
        ...
        self.signal_worker_process = mp.Process(
            target=start_worker_process,
            args=(self.signal_task_queue, self.order_command_queue, self.user_id), # Передаем user_id
            daemon=True
        )
        self.signal_worker_process.start()
        logger.info("[SignalWorker] Процесс-аналитик запущен с PID: %d", self.signal_worker_process.pid)
        
        # --- ЗАПУСК ВСЕХ ФОНОВЫХ ASYNCIO ЗАДАЧ ---
        ...
        # --- Первоначальная настройка и синхронизация ---
        ...
        logger.info("[warmup] user %s finished", self.user_id)
    ...
    async def route_private_message(self, msg):
        try:
            ...
            # Явное сравнение и поддержка возможных вариантов от провайдера
            if topic in ("position", "position.linear", "position.batch"):
                await self.handle_position_update(msg)
            elif topic in ("execution", "execution.fast"):
                await self.handle_execution(msg)
            # (опционально) elif topic == "order": await self.handle_order_update(msg)
            ...
            for p in data:
                try:
                    ...
                    if prev_pos and new_size == 0:
                        ...
                        # 1) Игнорим любые нулевые апдейты в первые 5 сек после открытия
                        if (now - open_ts) < 5.0:
                            logger.warning(f"[PositionStream] {symbol}: событие size=0 вскоре после открытия ({now-open_ts:.2f}s). Игнорируем.")
                            continue

                        # 2) Если у нас недавно были executions по этому символу (открытие/доливка/частичный),
                        #    не закрываемся по position-пушу — ждём закрывающий execution.
                        last_exec_ts = getattr(self, "last_exec_ts", {}).get(symbol, 0)
                        if (now - last_exec_ts) < 3.0:
                            logger.debug(f"[PositionStream] {symbol}: size=0, но были свежие executions ({now-last_exec_ts:.2f}s). Игнор.")
                            continue

                        # 3) Фолбэк на закрытие ОТКЛЮЧЁН по умолчанию — закрываем только в handle_execution
                        if not getattr(self, "ENABLE_POSITION_FALLBACK_CLOSE", False):
                            logger.debug(f"[PositionStream] {symbol}: fallback close по size=0 отключён (ждём handle_execution).")
                            continue
        ...
        try:
            ...
            for symbol in list(self.open_positions.keys()):
                if symbol not in live_positions:
                    # --- ИЗМЕНЕНИЕ НАЧАЛО ---
                    logger.info(f"[SYNC] Позиция {symbol} больше не активна. Очистка локального состояния.")
                    # Просто очищаем состояние, НЕ вызывая log_trade
    ...
    #             logger.info(f"[market_loop] Итерация #{iteration}. Запускаем сканирование {len(symbols_to_scan)} символов...")

    #             # === КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: ЗАПУСК В ФОНЕ ===
    #             # Мы создаем задачи, но не ждем их завершения здесь
    #             concurrency = getattr(self, "scan_concurrency", 24)
    #             sem = asyncio.Semaphore(concurrency)

    #             async def worker(sym: str):
    #                 async with sem:
    #                     await self.execute_golden_setup(sym)

    #             tasks = [asyncio.create_task(worker(s)) for s in symbols_to_scan]
    #             await asyncio.gather(*tasks, return_exceptions=True)

    #             scan_duration = time.time() - last_heartbeat
    #             sleep_s = max(0.2, getattr(self, "scan_interval_sec", 1.0) - scan_duration)
    #             logger.info(
    #                 f"[market_loop] Итерация #{iteration} завершена за {scan_duration:.2f} сек.; sleep {sleep_s:.2f}s"
    #             )
    #             await asyncio.sleep(sleep_s)
    ...
    async def market_loop(self):
        ...
        while True:
            ...
            try:
                symbols_to_scan = list(self.shared_ws.active_symbols)
                random.shuffle(symbols_to_scan)
                if symbols_to_scan:
                    self.signal_task_queue.put(symbols_to_scan)
                duration = time.time() - start_time
                logger.info(f"[MarketLoop] Итерация #{iteration}. {len(symbols_to_scan)} символов отправлено на анализ. Следующий цикл через {scan_interval:.1f} сек.")
                await asyncio.sleep(scan_interval)
            except asyncio.CancelledError:
                logger.info("[MarketLoop] Остановлен.")
                break
            except Exception:
                logger.exception("[MarketLoop] Критическая ошибка, перезапуск через 10 секунд...")
                await asyncio.sleep(10)
    ...
    async def _ai_dispatch(self, provider: str, candidate: dict, features: dict) -> dict:
        ...
        # <<< НАЧАЛО ИЗМЕНЕНИЯ >>>
            ...
            logger.error(f"[AI_TIMEOUT] Primary model ({self.ai_primary_model}) не ответил за {self.ai_timeout_sec}с.")
            # Пробрасываем ошибку выше, чтобы сработал circuit breaker
            raise
        # <<< КОНЕЦ ИЗМЕНЕНИЯ >>>
    ...
    async def evaluate_entry_candidate(self, candidate: dict, features: dict):
        ...
        ttl = getattr(self, "entry_cooldown_sec", 30)  # не дольше кулдауна
        cached = self._evaluated_signals_cache.get(signal_key)
        if cached and (now - float(cached.get("time", 0) or 0.0) < ttl):
            logger.debug(f"[AI_SKIP_CACHE] {symbol}/{side} ({source}) — cached {cached.get('status')} {int(now - cached.get('time', now))}s<{ttl}s")
            return
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        if lock.locked():
            logger.debug(f"[EXECUTE_SKIP_LOCKED][{cid}] {symbol}")
            return

        async with lock:
            if symbol in self.open_positions or symbol in self.pending_orders:
                logger.info(f"[EXECUTE_DENY][{cid}] {symbol}: already open/pending")
                return

            logger.info(f"[EXECUTE_CALL][{cid}] {symbol}/{side} src={source} vol_usdt={candidate.get('volume_usdt', getattr(self,'POSITION_VOLUME',1000))} price_hint={features.get('price')}")

            # Ваша надежная логика получения цены - сохранена
            last_price = self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice")
            try: last_price = float(last_price)
            except: last_price = None
            if not (last_price and last_price > 0):
                try: last_price = float(features.get("price", 0))
                except: last_price = 0.0
            if not (last_price and last_price > 0):
                logger.warning(f"[EXECUTE_ABORT_PRICE][{cid}] {symbol}/{side}: no valid price")
                return

            # Ваша надежная логика расчета количества - сохранена
            usd = float(candidate.get("volume_usdt") or getattr(self, "POSITION_VOLUME", 1000.0))
            qty = await self._calc_qty_from_usd(symbol, usd, last_price)
            if not (qty and qty > 0):
                step = self.qty_step_map.get(symbol); mn = self.min_qty_map.get(symbol)
                logger.warning(f"[EXECUTE_ABORT_QTY_ZERO][{cid}] {symbol}/{side}: usd={usd} price={last_price} step={step} min={mn}")
                return

            # Ваша надежная логика риск-чека - сохранена
            ok = await self._risk_check(symbol, side, qty, last_price, cid=cid)
            if not ok:
                logger.info(f"[EXECUTE_ABORT_RISK][{cid}] {symbol}/{side}")
                return

            logger.info(f"✅ [EXECUTE_GO][{cid}] {symbol}/{side} qty={qty} @ {last_price}")

            # <<< ИЗМЕНЕНИЕ: БЛОК ТАКТИЧЕСКОГО ВХОДА УДАЛЕН >>>
            # Теперь эта функция отвечает только за прямой вход по рынку,
            # так как решение о моменте входа принимает "охотник".
            
            comment_to_pass = candidate.get('justification', f"entry_by_{source}")

            await self.place_unified_order(
                symbol=symbol, side=side, qty=qty, order_type="Market",
                comment=comment_to_pass, cid=cid
            )



    async def _process_signal(self, candidate: dict, features: dict, signal_key: tuple):
        """Готовит пакет данных и отправляет его в очередь для воркера."""
        try:
            prompt = self._build_primary_prompt(candidate, features)
            data_packet = {"candidate": candidate, "prompt": prompt}
            self.signal_task_queue.put(data_packet)
            logger.debug(f"Пакет для сигнала {signal_key} отправлен в очередь Аналитику.")
        except Exception as e:
            logger.error(f"[_process_signal] Ошибка подготовки данных для {signal_key}: {e}", exc_info=True)
        finally:
            self.active_signals.discard(signal_key)
            symbol = candidate.get("symbol")
            if symbol:
                self.strategy_cooldown_until[symbol] = time.time() + 60
            logger.debug(f"Блокировка для {signal_key} снята, символ на кулдауне.")
    ...
    #         ))
    #         #ai_primary, ai_confirm = await asyncio.gather(primary_task, confirm_task, return_exceptions=False)
    #         ai_primary, ai_confirm = await asyncio.gather(primary_task, confirm_task, return_exceptions=True)

    #         # --- Проверяем, не вернулись ли ошибки ---
    #         if isinstance(ai_primary, Exception):
    #             logger.error(f"[AI_ERROR] Primary model failed: {ai_primary}")
    #             # Если основная модель упала, считаем это полным отказом
    #             return {**default_response, "justification": f"Primary AI failed: {ai_primary}"}
            
    #         if isinstance(ai_confirm, Exception):
    #             logger.warning(f"[AI_WARN] Confirm model failed: {ai_confirm}")
    #             # Если упала подтверждающая модель, это не критично, просто заменяем ее ответ на пустой
    #             ai_confirm = {"action": "HOLD", "justification": f"Confirm AI failed: {ai_confirm}"}
    ...
    # async def evaluate_candidate_with_ollama(self, candidate: dict, features: dict) -> dict:
    #     """
    #     [V2 - ULTRA ROBUST ENSEMBLE]
    #     - Сохраняет сложную логику ансамбля и вето.
    #     - Добавляет отказоустойчивость: если основная модель падает, использует подтверждающую как основную.
    #     """
    #     default_response = {"confidence_score": 0.7, "justification": "Ошибка локального AI.", "action": "REJECT"}
    #     prompt = self._build_primary_prompt(candidate, features)
    #     messages = [{"role": "user", "content": prompt}]
    ...
    #         # --- НАЧАЛО ИСПРАВЛЕННОЙ ЛОГИКИ ОБРАБОТКИ РЕЗУЛЬТАТОВ ---
    #         primary_failed = isinstance(ai_primary, Exception)
    #         confirm_failed = isinstance(ai_confirm, Exception)

    #         if primary_failed and confirm_failed:
    #             logger.error(f"[AI_CATASTROPHE] Обе AI модели не ответили для {candidate['symbol']}. Primary: {ai_primary}, Confirm: {ai_confirm}")
    #             return {**default_response, "justification": "Обе AI модели недоступны."}
            
    #         if primary_failed:
    #             logger.warning(f"[AI_FALLBACK] Primary model '{self.ai_primary_model}' failed: {ai_primary}. Используем ответ от confirm model.")
    #             # Основная модель упала, используем подтверждающую как единственное решение
    #             ai_primary = ai_confirm # Теперь ai_primary содержит ответ от plutus
    #             ai_confirm = {"action": "HOLD", "justification": "N/A"} # Подтверждающая уже использована
            
    #         if confirm_failed:
    #             logger.warning(f"[AI_WARN] Confirm model '{self.ai_advisor_model}' failed: {ai_confirm}")
    #             ai_confirm = {"action": "HOLD", "justification": "Confirm AI failed"} # Заглушка, если confirm упал
    #         # --- КОНЕЦ ИСПРАВЛЕННОЙ ЛОГИКИ ОБРАБОТКИ РЕЗУЛЬТАТОВ ---

    #         # --- Ваша существующая логика ансамбля теперь будет работать с уже обработанными результатами ---
    #         prim_act  = (ai_primary.get("action") or "REJECT").upper()
    #         prim_conf = _clamp01(ai_primary.get("confidence_score", 0.7))
    #         prim_just = ai_primary.get("justification", "")
            
    #         # ... (весь остальной ваш код для _has_negative, правил объединения, WATCH, VETO и т.д. остается здесь БЕЗ ИЗМЕНЕНИЙ) ...
    #         conf_act  = (ai_confirm.get("action") or "REJECT").upper()
    #         conf_conf = _clamp01(ai_confirm.get("confidence_score", 0.7))
    #         conf_just = ai_confirm.get("justification", "")
    #         def _has_negative(just: str) -> bool:
    ...
    #         negative_confirm = _has_negative(conf_just)
    #         if prim_conf < AI_WATCH_LO:
    #             final_action = "REJECT"; final_conf = prim_conf; final_just = f"[PRIMARY] {prim_just}"
    #         elif prim_conf < AI_EXEC_TH:
    #             final_action = "WATCH"; final_conf = prim_conf; final_just = f"[PRIMARY->WATCH] {prim_just}"
    #         else:
    #             if negative_confirm or (conf_conf < AI_WATCH_LO and conf_act == "REJECT"):
    #                 final_action = "WATCH"; final_conf = min(prim_conf, 0.58); final_just = f"[PRIMARY EXECUTE, CONFIRM VETO→WATCH] {prim_just} | Plutus: {conf_just}"
    #                 logger.info(f"[AI_VETO] {candidate['symbol']}/{candidate['side']}: Plutus понизил сигнал")
    #             else:
    #                 final_action = "EXECUTE" if prim_act == "EXECUTE" else "WATCH"; final_conf = prim_conf; final_just = f"[EXECUTE] {prim_just} | Plutus: {conf_just}"
    #         if negative_primary and final_action == "EXECUTE":
    #             final_action = "WATCH"; final_just = f"[NEGATIVE_CUES→WATCH] {prim_just}"
            
    #         out = {"action": final_action, "confidence_score": float(f"{final_conf:.3f}"), "justification": final_just, "full_prompt_for_ai": prompt, "ai_primary": ai_primary, "ai_confirm": ai_confirm}
    #         logger.info(f"[AI_ENS] {candidate['symbol']}/{candidate['side']} → {final_action} ({final_conf:.2f})")
    #         return out
...
# --- маленькие утилиты ---
    ...
    def _build_squeeze_entry_prompt(self, symbol: str, side: str, extreme_price: float, last_price: float, features: dict) -> str:
        """
        [V2 - ADVANCED]
        Формирует продвинутый промпт для AI-советника, фокусируясь на признаках истощения.
        """
        pullback_pct = abs(last_price - extreme_price) / extreme_price * 100
        
        # Собираем дополнительные метрики из полного набора фичей
        ...
        prompt = f"""
        SYSTEM: Ты элитный аналитик рыночной микроструктуры. Твоя задача - определить оптимальный момент для КОНТРТРЕНДОВОГО входа после сильного ценового импульса (сквиза). Твоя цель - поймать разворот, а не летящий нож. Ответ - только валидный JSON.
        ...
        **Торговый сигнал (Squeeze):**
        ...
        - Величина отката от пика/дна: {pullback_pct:.2f}%

        **Контекст в момент отката:**
        - RSI(14): {features.get('rsi14', 0):.1f}
        - CVD(1m) Trend: {'Растет' if cvd_1m > 0 else 'Падает'} (Абсолютное значение: {cvd_1m:,.0f})
        - Аномалия объема (1m): {vol_anomaly:.1f}x от среднего

        **ЗАДАЧА:** Основываясь на этих данных, ищи признаки ИСТОЩЕНИЯ импульса.
        - Для входа в LONG (после падения): ищи затухание объемов продаж, разворот CVD вверх.
        - Для входа в SHORT (после роста): ищи затухание объемов покупок, разворот CVD вниз.
        
        Является ли СЕЙЧАС оптимальный момент для входа? Верни JSON:
        - "action": "EXECUTE" (только если видишь явные признаки разворота/истощения) или "WAIT" (если импульс все еще силен или ситуация не ясна).
        - "reason": Краткое обоснование (например, "Объем на откате затухает, CVD подтверждает разворот").
        """
        return prompt.strip()
    ...
    async def _hunt_squeeze_entry_point(self, candidate: dict, features: dict, signal_key: tuple):
        """
        Функция-"охотник". Сначала получает стратегическое одобрение от основного AI,
        затем в цикле опрашивает AI-советника в поиске лучшей точки входа.
        """
        symbol = candidate["symbol"]
        side = candidate["side"]
        
        try:
            # --- Фаза 1: Страгическое одобрение от основного AI ---
            ...
            if ai_response.get("action") != "EXECUTE":
                logger.info(f"[AI_REJECT] {symbol}/{side} (squeeze) — {ai_response.get('justification', 'Причина не указана.')}")
                return # Если основной AI отклонил, охота прекращается

            # <<< ДОБАВЬТЕ ЭТУ СТРОКУ >>>
            candidate['justification'] = ai_response.get('justification', 'Одобрено основным AI')
            # <<< -------------------- >>>

            logger.info(f"[AI_CONFIRM] {symbol}/{side} (squeeze) ОДОБРЕНА. Начинаем тактическую охоту за точкой входа...")

            # --- Фаза 2: Тактическая охота за точкой входа ---
            timeout = float(getattr(self, "tactical_entry_window_sec", 90.0))
            start_time = time.time()
            extreme_price = features.get("impulse_high") if side == "Sell" else features.get("impulse_low")

            while time.time() - start_time < timeout:
                # Запрашиваем у AI-советника, пора ли входить
                is_time_to_enter = await self._ai_confirm_squeeze_entry(symbol, side, extreme_price)
                
                if is_time_to_enter:
                    logger.info(f"✅ [AI_EXECUTE] {symbol}/{side}: AI-советник дал команду на вход! Исполняем.")
                    # Вызываем простой вход по рынку
                    await self.execute_trade_entry(candidate, features)
                    return # Успешный вход, завершаем охоту

                # Если AI сказал "WAIT", ждем перед следующей попыткой
                await asyncio.sleep(self.squeeze_ai_confirm_interval_sec)

            logger.warning(f"[HUNT_TIMEOUT] {symbol}/{side}: Окно входа ({timeout}s) истекло, AI не дал команду. Вход отменен.")

        except asyncio.TimeoutError:
            logger.error(f"[HUNT_FAIL] {symbol}/{side}: Таймаут при общении с AI. Охота отменена.")
        except Exception as e:
            logger.error(f"[HUNT_FAIL] {symbol}/{side}: Критическая ошибка в процессе охоты: {e}", exc_info=True)
        finally:
            # Вне зависимости от исхода, снимаем блокировку сигнала
            self.active_signals.discard(signal_key)
            logger.debug(f"Охота за {signal_key} завершена, блокировка снята.")
...
# --- SQUEEZE FILTER (REAL) ---------------------------------------------------
    ...
    async def _squeeze_logic(self, symbol: str) -> bool:
        """
        [V3 - Refactored]
        Обнаруживает сырой сигнал сквиза и запускает "охотника" для дальнейшей обработки.
        """
        try:
            features, impulse_dir = self._build_squeeze_features_5m(symbol)
            if not features: return False

            # ... (вся ваша логика определения порога и наличия сквиза остается здесь) ...
            ret_5m = float(features.get("ret_5m", 0.0)); atr_5m = float(features.get("atr_5m", 0.0)); price = float(features.get("price", 0.0))
            if price <= 0: return False
            thr_base = float(getattr(self, "squeeze_min_abs_move", 0.03)); k = float(getattr(self, "squeeze_k_factor", 1.5))
            thr_dyn = (k * atr_5m / price) if atr_5m > 0 else 0.0
            threshold = max(thr_base, thr_dyn)
            if abs(ret_5m) < threshold: return False
            ...
            logger.info(f"🔥 [{symbol}] ОБНАРУЖЕН СКВИЗ! Движение: {ret_5m*100:.2f}%. Передано на обработку Охотнику.")
            ...
            candidate = {"symbol": symbol, "side": side, "source": "squeeze", "base_metrics": {'pct_5m': ret_5m * 100}}
            
            # Запускаем "охотника" в фоновом режиме
            asyncio.create_task(self._hunt_squeeze_entry_point(candidate, features, signal_key))
            return True
            
        except Exception as e:
            logger.error(f"[_squeeze_logic] Критическая ошибка для {symbol}: {e}", exc_info=True)
            # Убедимся, что блокировка снимается при ошибке
            side_on_error = "Sell" if (features or {}).get("ret_5m", 0) > 0 else "Buy"
            self.active_signals.discard((symbol, side_on_error, 'squeeze'))
            return False
    ...
    async def _liquidation_logic(self, symbol: str) -> bool:
        try:
            ...
            # <<< ИЗМЕНЕНИЕ: БЛОК ПРОВЕРКИ И УСТАНОВКИ БЛОКИРОВКИ >>>
            signal_key = (symbol, trade_side, 'liquidation')
            if signal_key in self.active_signals:
                return True # Сигнал уже в обработке, но считаем, что событие все еще актуально.
            self.active_signals.add(signal_key)
            # <<< КОНЕЦ ИЗМЕНЕНИЯ >>>
    ...
    async def _golden_logic(self, symbol: str):
            ...
            try:
                ...
                if action:
                    ...
                    if signal_key in self.active_signals:
                        return # Сигнал уже в обработке, выходим
                    self.active_signals.add(signal_key)
                    # <<< КОНЕЦ ИЗМЕНЕНИЯ >>>
                    ...
                    if not features:
                        ...
                        self.active_signals.discard(signal_key) # Освобождаем блокировку при ошибке
                        return
                    ...
                    # Передаем signal_key в обработчик, чтобы он мог снять блокировку
                    asyncio.create_task(self._process_signal(candidate, features, signal_key))
                ...
                # В случае ошибки здесь, блокировка может не установиться, но это безопасно
            finally:
    ...
    async def place_unified_order(self, symbol: str, side: str, qty: float,
                                ...
                                comment: str = "", cid: str | None = None):
        cid = cid or ("cid_" + str(int(time.time()*1000)))
        pos_idx = 1 if side == "Buy" else 2
        if comment:
        ...
        logger.info(f"➡️ [ORDER_SENDING][{cid}] {symbol} {side} {order_type} qty={params['qty']} {('price='+params['price']) if 'price' in params else ''}")
        ...
        try:
            ...
            if resp.get("retCode") == 0:
                ...
                # передай воркеру постановку стопа (если есть шина/вызов)
                try:
                    if hasattr(self, "stop_bus_publish"):
                        await self.stop_bus_publish({
                            "symbol": symbol, "side": side, "qty": qty,
                            "entryPrice": float(self.shared_ws.ticker_data.get(symbol,{}).get("lastPrice", 0)) or price,
                            "orderLinkId": cid, "strategy": "entry"
                        })
                        logger.info(f"[STOP_TASK_PUBLISH][{cid}] {symbol}")
                except Exception:
                    logger.exception(f"[STOP_TASK_PUBLISH_FAIL][{cid}] {symbol}")
                return
            else:
                logger.error(f"❌ [ORDER_REJECTED][{cid}] {symbol} retCode={resp.get('retCode')} retMsg={resp.get('retMsg')}")
        except Exception as e:
    ...
    # было:
    # async def adaptive_squeeze_entry(self, symbol: str, side: str, qty: float, max_entry_timeout: int = 45) -> bool:
    ...
    async def adaptive_squeeze_entry(
        ...
        seed_price: float | None = None,   # ← примем стартовую цену (ранее хавали last_price)
        cid: str | None = None,            # ← часто прокидываешь корреляционный id — пусть не падает
        **_                                 # ← проглатываем любые лишние kwargs, чтобы не валиться
    ) -> bool:        
        """
        [V3 - ИСПРАВЛЕННАЯ ЛОГИКА] Адаптивный вход с разделением на фазы
        поиска экстремума и ожидания отката. Устойчив к потере WS-соединения.
        """
        logger.info(
        ...
        # Параметры
        pull = float(getattr(self, "squeeze_pullback_ratio", 0.004))  # 0.4% откат для входа
        peak_finding_window_sec = 10  # Сколько секунд активно ищем пик/дно
        ...
        try:
            while time.time() - start_time < max_entry_timeout:
                ...
                if time.time() - last_update_time > 10:
                    logger.error(f"[{symbol}] Нет обновлений тикера более 10 секунд. Вероятно, WS разорван. Отмена входа.")
                    break # Выходим из цикла, если данные устарели
                ...
                last_update_time = time.time() # Обновляем таймер, как только получили валидную цену
                elapsed_time = time.time() - start_time

                # === ФАЗА 1: Поиск экстремума (первые N секунд) ===
                if elapsed_time <= peak_finding_window_sec:
                    if side == "Sell":  # Ищем самый высокий пик
                        if extreme_price == 0 or last_price > extreme_price:
                            extreme_price = last_price
                    else:  # "Buy", ищем самое низкое дно
                        if extreme_price == 0 or last_price < extreme_price:
                            extreme_price = last_price
                ...
                # === ФАЗА 2: Ожидание отката от зафиксированного экстремума ===
                if extreme_price == 0:
                    logger.warning(f"[{symbol}] Не удалось определить экстремум за {peak_finding_window_sec}с. Отмена.")
                    break

                entry_condition_met = False
                if side == "Sell":
                    target_price = extreme_price * (1 - pull)
                    if last_price <= target_price:
                        entry_condition_met = True
                        logger.info(f"[TACTICAL_ENTRY] {symbol} Short. Цена ({last_price:.6f}) достигла цели отката ({target_price:.6f}) от пика {extreme_price:.6f}.")
                else:  # "Buy"
                    target_price = extreme_price * (1 + pull)
                    if last_price >= target_price:
                        entry_condition_met = True
                        logger.info(f"[TACTICAL_ENTRY] {symbol} Long. Цена ({last_price:.6f}) достигла цели отскока ({target_price:.6f}) от дна {extreme_price:.6f}.")
                
                if entry_condition_met:
                    await self.place_unified_order(
                        symbol, side, qty, "Market", comment="squeeze_tactical_entry"
                    )
                    entry_made = True
                    break
            ...
            logger.error(f"[TACTICAL_SQUEEZE_AI] Критическая ошибка для {symbol}: {e}", exc_info=True)
            entry_made = False
        finally:
            if not entry_made:
                logger.warning(f"[TACTICAL_SQUEEZE_AI] {symbol}: Окно входа истекло или произошла ошибка. Вход НЕ выполнен.")
        return entry_made

    # Эта функция становится почти полной копией, что не очень хорошо,
    # но пока оставим так для ясности. В будущем можно будет их объединить.
    async def adaptive_squeeze_entry_ws(self, symbol: str, side: str, qty: float, position_idx: int, max_entry_timeout: int = 45) -> bool:
        # Просто вызываем основную реализацию.
        # В place_unified_order уже заложена логика выбора между WS и REST.
        return await self.adaptive_squeeze_entry(symbol, side, qty, max_entry_timeout)
    ...
    async def log_trade(
        ...
        *,
        ...
        logger.info(
            f"[log_trade] user=%s %s %s: side=%s, vol=%s, price=%s, result=%s%s%s",
            self.user_id, action.upper(), symbol, side, safe_volume, safe_avg_price, result, pnl_info, comment_info
        )
        ...
        try:
            base_row = {
                ...
                "event": action,
                "result": result,
                "volume_trade": safe_volume,        # объём сделки
                "price_trade": safe_avg_price,      # цена сделки
                "pnl_usdt": pnl_usdt,
                "pnl_pct": pnl_pct,
            }
            # Добавляем рыночные метрики для обучения
            base_row.update(self._dataset_metrics(symbol))

            _append_trades_unified(base_row)
        except Exception as e:
            logger.warning(f"[CSV Log] Ошибка записи в trades_unified.csv: {e}", exc_info=True)
        ...
        final_comment = ""
        if comment and "jarvis" not in comment.lower():
            final_comment = f"<i>JARVIS' OPINION: {comment}</i>"
        elif comment:
            final_comment = f"<i>{comment}</i>"


        if action == "open":
            icon = "🟩" if side == "Buy" else "🟥"
            title = f"Открыта {side.upper()} {symbol} {link}"
            ...
            if last_price > 0:
                ...
                msg = (f"🛡️ <b>Трейлинг-стоп обновлен: {symbol} {link}</b>\n\n"
        ...
        if msg:
            msg += f"\n<b>Время:</b> {time_str}\n#{symbol}"
            try:
                # Оборачиваем вызов в отдельную защищенную функцию
                await self.notify_user(msg)
            except Exception as e:
                # Логируем ошибку, даже если она произошла внутри notify_user
                logger.error(f"[Telegram] Критическая ошибка в задаче log_trade при отправке уведомления: {e}", exc_info=True)
...
if __name__ == "__main__":
    # Эта проверка гарантирует, что код внутри этого блока
    # будет выполнен только тогда, когда вы запускаете этот файл напрямую
    # (python3 MultiuserBot_v3RC.py), а не когда его импортирует другой скрипт (например, signal_worker).
