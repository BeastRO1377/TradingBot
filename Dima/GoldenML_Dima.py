...
OPERATION_MODE = "ST_cross2"  # Режимы: drift_only, drift_top10, golden_setup, model_only, super_trend, ST_cross1, ST_cross2, ST_cross_global
HEDGE_MODE = True
        ...
        if telegram_bot and telegram_message_queue:
            ...
            if result == "Opened":
                ...
                message = (f"🔄 *Трейлинг стоп\-лосс*\n"
                        f"*Символ:* {symbol_link}\n"
                        f"*Время:* {escape_markdown(formatted_time)}\n"
                        f"*Расстояние:* {escape_markdown(str(profit_perc))}\n"
                        f"*Тип:* Трейлинг")
            else:
                message = (f"🫡🔄 *Сделка*\n"
                            f"*Символ:* {symbol_link}\n"
...
def escape_markdown(text) -> str:
    text = str(text)  # Преобразуем входное значение в строку, если оно не строка.
    escape_chars = r"_*\[\]()~`>#+\-={}|.,!\\"
...
def open_averaging_position(symbol):
    try:
        with open_positions_lock:
            ...
            # (Дополнительно можно проверить, что позиция действительно в убытке, если нужно)
            # Например, если у вас в open_positions уже сохранен PnL:
