...
# Глобальный семафор для ограничения одновременных API вызовов (чтобы не исчерпать пул соединений)
...
OPERATION_MODE = "ST_cross2"  # drift_only, drift_top10, golden_setup, model_only, super_trend, ST_cross1, ST_cross2, ST_cross_global, ST_cross2_drift
HEDGE_MODE = True
...
# Создаём клиент Bybit без передачи параметра session!
session = HTTP(testnet=False, api_key=BYBIT_API_KEY, api_secret=BYBIT_API_SECRET, timeout=60)
# Затем подменяем внутреннюю сессию на нашу настроенную
session.session = custom_session

# Телеграм и дополнительные объекты
telegram_bot = None
router = Router()
...
# Блокировки и глобальное состояние
...
# Режим мониторинга: "ws" или "http"
MONITOR_MODE = "http"
IS_RUNNING = True
ws = None

# ===================== Асинхронные утилиты =====================
async def async_get(session: aiohttp.ClientSession, url, params=None):
...
def adjust_quantity(symbol: str, raw_qty: float) -> float:
    try:
        ...
        if not info:
            logger.warning(f"[adjust_quantity] Нет информации по {symbol}")
            return 0.0
        ...
        if not last_price or last_price <= 0:
            logger.warning(f"[adjust_quantity] Недопустимая цена для {symbol}")
            return 0.0
        price_dec = Decimal(str(last_price))
        dec_qty = Decimal(str(raw_qty))
        adj_qty = (dec_qty // qty_step) * qty_step
        if adj_qty < min_qty:
            logger.info(f"[adjust_quantity] {symbol}: {adj_qty} < {min_qty}")
            return 0.0
        if adj_qty > max_qty:
            adj_qty = max_qty
        ...
        if order_value < min_order_value:
            ...
            if needed_qty > max_qty or needed_qty < min_qty:
                logger.warning(f"[adjust_quantity] {symbol}: не удовлетворяет требованиям")
                return 0.0
            adj_qty = needed_qty
        final_qty = float(adj_qty)
        logger.info(f"[adjust_quantity] {symbol}: итоговое количество = {final_qty}")
        return final_qty
    except Exception as e:
...
def get_last_close_price(symbol):
    try:
        ...
        if not resp or resp.get("retCode") != 0:
            logger.error(f"[get_last_close_price] {symbol}: ошибка")
            return None
...
def get_selected_symbols():
    ...
    if not selected_symbols or now - last_asset_selection_time >= ASSET_SELECTION_INTERVAL:
        ...
        logger.info(f"Обновлен список активов: {selected_symbols}")
        ...
        for sym in list(open_positions.keys()):
            if sym not in expos:
                ...
                if close_price:
                    ...
                    pnl = (cp - ep) / ep * volume if pos["side"].lower() == "buy" else (ep - cp) / ep * volume
                    if trade_id:
                        update_trade_outcome(trade_id, float(pnl))
                to_remove.append(sym)
                logger.info(f"[update_open_positions_from_exch_positions] Удаляем {sym}")
        for sym in to_remove:
            del open_positions[sym]
        for sym, newpos in expos.items():
            if sym in open_positions:
                open_positions[sym].update({
                    "side": newpos["side"],
                    "size": newpos["size"],
...
def log_trade(symbol, row, open_interest, action, result, closed_manually=False):
    try:
        ...
        if telegram_bot and telegram_message_queue:
            ...
            escaped_symbol = re.sub(r'([_*[\]()~`>#+\-={}|.,!\\])', r'\\\1', symbol)
            symbol_url = f"https://www.bybit.com/trade/usdt/{symbol}"
            symbol_link = f"[{escaped_symbol}]({symbol_url})"
            formatted_time = time_str
            if result == "Opened":
                msg = f"🟩 *Открытие ЛОНГ* {symbol_link}\nВремя: {formatted_time}\nЦена: {open_str}\nОбъем: {vol_str}\nРежим: {OPERATION_MODE}" if action.lower() == "buy" else f"🟥 *Открытие ШОРТ* {symbol_link}\nВремя: {formatted_time}\nЦена: {open_str}\nОбъем: {vol_str}\nРежим: {OPERATION_MODE}"
            elif result == "Closed":
                msg = f"❌ *Закрытие позиции* {symbol_link}\nВремя: {formatted_time}\nЦена: {close_str}\nОбъем: {vol_str}\nТип: {'Вручную' if closed_manually else OPERATION_MODE}"
            elif result == "Trailing Stop Set":
                msg = f"🔄 *Трейлинг стоп* {symbol_link}\nВремя: {formatted_time}\nРасстояние: {action}\nДополнительно: {result}"
            else:
                msg = f"🫡🔄 *Сделка* {symbol_link}\nРезультат: {result}\nЦена: {close_str}\nДействие: {action}"
            asyncio.run_coroutine_threadsafe(telegram_message_queue.put(msg), loop)
    except Exception as e:
...
def handle_drift_order_with_trailing(symbol, side):
    try:
        ...
        vol = POSITION_VOLUME
        qty = vol / dec_price
        order_result = place_order(symbol, side, float(qty), order_type="Market", time_in_force="GoodTillCancel", reduce_only=False, positionIdx=1 if side.lower() == "buy" else 2)
        if order_result and order_result.get("retCode") == 0:
            ...
            if TRAILING_STOP_ENABLED:
                set_trailing_stop(symbol, pos=open_positions[symbol]["size"], trailing_gap_percent=TRAILING_GAP_PERCENT, side=side)
        else:
...
def get_historical_data_for_model(symbol, interval="1", limit=200, from_time=None):
    try:
        ...
        df.dropna(subset=["closePrice"], inplace=True)
        logger.debug(f"[get_historical_data_for_model] {symbol}: {len(df)} свечей.")
        return df
    except Exception as e:
...
def open_averaging_position(symbol):
    try:
        ...
        order_result = place_order(symbol, side, float(qty_in_coins), order_type="Market", time_in_force="GoodTillCancel", reduce_only=False, positionIdx=1 if side.lower() == "buy" else 2)
        if order_result and order_result.get("retCode") == 0:
            with open_positions_lock:
...
# ===================== Функция place_order и set_trailing_stop =====================
def place_order(symbol, side, qty, order_type="Market", time_in_force="GoodTillCancel", reduce_only=False, positionIdx=None):
    try:
...
def open_position(symbol: str, side: str, volume_usdt: Decimal, reason: str):
    if IS_SLEEPING_MODE:
        ...
        return
    try:
        logger.info(f"[open_position] Попытка открытия {side} позиции по {symbol}, объем: {volume_usdt} USDT, причина: {reason}")
        with state_lock, open_positions_lock:
            ...
            if current_total + volume_usdt > MAX_TOTAL_VOLUME:
                logger.warning(f"[open_position] Превышен глобальный лимит: {current_total} + {volume_usdt} > {MAX_TOTAL_VOLUME}")
                return
            if symbol in open_positions:
                logger.info(f"[open_position] Позиция по {symbol} уже открыта, пропуск.")
                return
        ...
        order_res = place_order(symbol, side, qty_float, order_type="Market", time_in_force="GoodTillCancel", reduce_only=False, positionIdx=pos_idx)
        if not order_res or order_res.get("retCode") != 0:
            logger.info(f"[open_position] Ошибка при выполнении ордера для {symbol}, пропуск.")
            return
        ...
        logger.info(f"[open_position] {symbol}: {side} позиция успешно открыта, объем {volume_usdt} USDT")
    except Exception as e:
            ...
            try:
                if telegram_bot:
                    async with send_semaphore:
                        await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=msg, parse_mode="MarkdownV2", disable_web_page_preview=True, request_timeout=120)
...
async def monitor_positions():
    while IS_RUNNING:
        try:
            ...
            positions = await asyncio.to_thread(get_exchange_positions)
                ...
                if leveraged_pnl_percent >= threshold_trailing:
                    with open_positions_lock:
                        if symbol in open_positions and not open_positions[symbol].get("trailing_stop_set", False):
                            ...
                            set_trailing_stop(symbol, size=open_positions[symbol]["size"], trailing_gap_percent=TRAILING_GAP_PERCENT, side=side)
...
async def main_coroutine():
    ...
    exch_positions = await asyncio.to_thread(get_exchange_positions)
    ...
    try:
        while IS_RUNNING:
            ...
            final_expos = await asyncio.to_thread(get_exchange_positions)
        ...
        if not IS_RUNNING:
            ...
            if telegram_bot and TELEGRAM_CHAT_ID:
                try:
                    await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text="✅ Бот успешно остановлен")
                except Exception as e:
                    logger.error(f"Ошибка отправки сообщения об остановке: {e}")
        else:
            logger.error("Бот остановился неожиданно. Проверьте логи!")
            if telegram_bot and TELEGRAM_CHAT_ID:
                try:
                    await telegram_bot.send_message(chat_id=TELEGRAM_CHAT_ID, text="⚠️ Бот остановился не по команде. Проверьте логи!")
                except Exception as e:
                    logger.error(f"Ошибка отправки сообщения об остановке: {e}")
        if telegram_bot:
            try:
