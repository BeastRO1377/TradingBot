...
# Глобальное состояние для drift анализа
drift_state = {
    "last_analysis": {}
}
drift_lock = asyncio.Lock()
drift_running = True  # Флаг, по которому можно остановить цикл
...
def adjust_quantity(symbol: str, raw_qty: float) -> float:
    """
    Проверяем лот, шаг, minOrderValue, и т.д.
    Принимает raw_qty как "количество монет" (coins), возвращает округленное.
    """
    try:
        ...
        # Бывает, что Bybit отдаёт minOrderValue отдельно
        min_order_value = Decimal(str(info.get("minOrderValue", 5)))

        # Для контроля цены нужно взять "последнюю" цену
        ...
        # Округлим по шагу qty_step
        dec_qty = Decimal(str(raw_qty))
        # floor до ближайшего шага qty_step:
        adj_qty = (dec_qty // qty_step) * qty_step

        # Проверка min_qty/max_qty
        if adj_qty < min_qty:
        ...
        # Проверка minOrderValue: если цена*adj_qty < 5 USDT → пытаемся повысить
        order_value = adj_qty * price_dec
        if order_value < min_order_value:
            needed_qty = (min_order_value / price_dec).quantize(qty_step, rounding="ROUND_UP")
            # Если needed_qty всё ещё < max_qty и > min_qty, то используем его
            if needed_qty > max_qty:
                logger.warning(
                    f"[adjust_quantity] {symbol}: needed_qty={needed_qty} > max_qty={max_qty}. "
                    "Ордер не пройдёт по minOrderValue."
                )
                return 0.0
            if needed_qty < min_qty:
                logger.warning(
                    f"[adjust_quantity] {symbol}: needed_qty={needed_qty} < min_qty={min_qty}. "
                    "Ордер не пройдёт по minQty."
                )
                return 0.0
            # Иначе берём needed_qty
            adj_qty = needed_qty

        final_qty = float(adj_qty)
        logger.info(
            f"[adjust_quantity] {symbol}: raw_qty={raw_qty}, price={price_dec}, "
            f"adj_qty={final_qty}, min_qty={min_qty}, step={qty_step}, order_value ~ {adj_qty*price_dec} USDT"
        )
        return final_qty
...
def get_exchange_positions():
    ...
    try:
        ...
        # Подробно логируем "сырое" содержание ответа
        ...
        for pos in positions:
            ...
            volume_usdt = size * entry_price  # в USDT
...
def place_order(symbol, side, qty,
                ...
                positionIdx=None):
    """
    qty здесь — количество монет, НЕ USDT.
    """
    try:
        adj_qty = adjust_quantity(symbol, qty)
        if adj_qty <= 0:
            logger.error(f"[place_order] qty={qty} => adj_qty={adj_qty}, недопустимо для {symbol}.")
            return None
        ...
        logger.info(f"[place_order] Отправляем ордер: {params}")
        ...
        if resp.get("retCode") == 0:
            ...
            logger.error(f"[place_order] Ошибка place_order: retCode={resp.get('retCode')} => {resp.get('retMsg')}")
            return None
...
def update_open_positions_from_exch_positions(expos: dict):
    ...
    """
    with open_positions_lock, state_lock:
        logger.info(f"[update_open_positions_from_exch_positions] BEFORE:\n  open_positions = {open_positions}\n  expos = {expos}")

        # Удаляем из open_positions те символы, которых нет в expos (значит, позиция закрылась)
        to_remove = []
        for sym in list(open_positions.keys()):
            if sym not in expos:
                # Позицию считают закрытой. Если она числится у нас — фиксим логику закрытия
                pos = open_positions[sym]
                side   = pos["side"]
                volume = Decimal(str(pos.get("position_volume", 0)))
                trade_id = pos.get("trade_id")

                # Дополнительно, если нужно, можно фиксировать итоговый pnl и логировать "закрытие" тут.
                ...
                logger.info(f"[update_open_positions_from_exch_positions] Позиция {sym} закрыта (нет в expos).")

        for sym in to_remove:
            del open_positions[sym]

        # Обновляем/добавляем те, что прислал Bybit
        ...
        # Пересчитываем общий объём
        total = sum(Decimal(str(p["position_volume"])) for p in open_positions.values())
        state["total_open_volume"] = total

        logger.info(f"[update_open_positions_from_exch_positions] AFTER:\n  open_positions = {open_positions}\n  total_open_volume = {total}")
...
def set_trailing_stop(symbol, size, trailing_gap_percent, side):
    try:
        ...
        # Если запрос прошёл успешно, обрабатываем ответ:
        rc = resp.get("retCode")
        if rc == 0:
            with open_positions_lock:
                if symbol in open_positions:
        ...
        # Если ошибка связана с кодом 34040, обрабатываем её как "not modified"
        if "34040" in str(e):
            logger.info(f"[set_trailing_stop] {symbol}: not modified, retCode=34040.")
        else:
...
def open_averaging_position(symbol):
    """
    Открывает позицию усреднения, если уже есть базовая позиция.
    """
    try:
        with open_positions_lock:
            if symbol not in open_positions:
            ...
            logger.info(f"[Averaging] base_volume_usdt = {base_volume_usdt} для {symbol} (side={side})")
        ...
        last_price = get_last_close_price(symbol)
        if not last_price or last_price <= 0:
            logger.warning(f"[Averaging] Нет актуальной цены для {symbol}, пропуск.")
            return

        # Если у вас есть условие "минимальный объём лота — 100 USDT", проверяем его
        #if base_volume_usdt < Decimal("100"):
        #    logger.warning(f"[Averaging] base_volume_usdt={base_volume_usdt} < 100 USDT, пропуск.")
        #    return

        # Переводим USDT -> количество монет:
        qty_in_coins = base_volume_usdt / Decimal(str(last_price))
        logger.info(f"[Averaging] Будем открывать {symbol} на ~{qty_in_coins} монет (side={side}).")

        # Вызываем place_order, передавая qty_in_coins
        order_result = place_order(
            symbol=symbol,
            side=side,
            qty=float(qty_in_coins),  # важно, что теперь это монеты
            order_type="Market",
            time_in_force="GoodTillCancel",
            reduce_only=False,
            positionIdx=1 if side.lower() == "buy" else 2
        ...
        if order_result and order_result.get("retCode") == 0:
            ...
            logger.info(
                f"[Averaging] Усредняющая позиция для {symbol} ОТКРЫТА на объём {base_volume_usdt} USDT "
                f"(~{qty_in_coins:.2f} монет). Текущее усредняющее суммарно={averaging_total_volume}"
            )
        else:
...
async def menu_info_cb(query: CallbackQuery):
    ...
    await query.message.edit_text(
        "ℹ️ **Информация** – выберите действие:",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
...
async def initialize_telegram_bot():
    ...
    try:
        if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
            ...
            # Вместо await dp.start_polling(...) — создание фоновой задачи:
...
def get_last_row(symbol):
    """
    Возвращает последнюю свечу DataFrame или None, если данных нет.
    """
    df = get_historical_data_for_trading(symbol, interval="1", limit=1)
    if df.empty:
        return None
    return df.iloc[-1]
...
async def async_drift_analyzer(interval: int = 60):
    """
    Асинхронная функция, аналог класса DriftAnalyzer.
    Каждые `interval` секунд проходит по выбранным символам,
    получает новые данные и обновляет глобальный словарь drift_state["last_analysis"].
    """
    ...
    while drift_running:
        try:
            ...
            random.shuffle(symbols)
            for sym in symbols:
                # Если цикл остановлен, выходим из цикла символов
                ...
                if not new_data.empty:
                    is_anomaly, strength, direction = monitor_feature_drift_per_symbol(
                        sym,
                        new_data,
                        pd.DataFrame(),  # Если нет референсных данных – пустой DataFrame
                        feature_cols,
                        threshold=0.5
                    )
...
async def get_latest_drift_analysis():
    """
    Возвращает копию текущего состояния drift анализа.
    """
    async with drift_lock:
...
def stop_drift_analyzer():
    """
    Останавливает асинхронный цикл drift анализа.
    """
    global drift_running
    drift_running = False
...
async def main_coroutine():
    ...
    # Текущий event loop и очередь для отправки Telegram сообщений
    loop = asyncio.get_running_loop()
    telegram_message_queue = asyncio.Queue()
    ...
    # Запускаем задачу-отправитель в Телеграм (чтобы отправлять логи и т.д.)
    telegram_sender_task = asyncio.create_task(telegram_message_sender())

    # Запускаем задачу инициализации Telegram-бота (где создаётся Dispatcher и polling)
    tg_task = asyncio.create_task(initialize_telegram_bot())

    # Дадим боту время запуститься, затем отправим начальное сообщение
    await asyncio.sleep(3)
    await send_initial_telegram_message()

    # Загружаем/обучаем модель (если нужно)
    current_model = load_model()

    # Начальная сборка исторических данных
    symbols_all = get_usdt_pairs()
    collect_historical_data(symbols_all, interval="1", limit=200)

    # Синхронизация локальных позиций с биржевыми
    exch_positions = get_exchange_positions()
    update_open_positions_from_exch_positions(exch_positions)

    # Запускаем асинхронный drift-анализ как фоновую задачу
    drift_task = asyncio.create_task(async_drift_analyzer(interval=60))

    # Запуск мониторинга позиций (http или ws)
    monitor_task = None
    if MONITOR_MODE == "ws":
        logger.info("[Main] Режим мониторинга: WebSocket")
        threading.Thread(target=start_ws_monitor, daemon=True).start()
    ...
    try:
        while IS_RUNNING:
            # 1) Попытка синхронизации позиций
            try:
                exch_positions = get_exchange_positions()
                update_open_positions_from_exch_positions(exch_positions)
            except Exception as e:
            ...
            # 2) Проверяем состояние задачи Telegram-бота
            if tg_task.done():
                exc = tg_task.exception()
                if exc:
                    logger.exception("Telegram-бот упал с исключением:", exc)
            ...
            # 3) Основная логика торгового цикла
            iteration_count += 1
            logger.info(f"[INNER_LOOP] iteration_count={iteration_count} — новый цикл")
            ...
            # Пример: если OPERATION_MODE == "model_only"
            if OPERATION_MODE == "model_only":
                tasks = [process_symbol_model_only(s) for s in symbols]
                if tasks:
                    await asyncio.gather(*tasks)
            ...
            # Если включен check_and_close_active — проверяем trailing stop
            if check_and_close_active:
                check_and_set_trailing_stop()

            # Периодически публикуем таблицы
            if iteration_count % 5 == 0:
                await publish_drift_and_model_tables()

            # Логируем модельные предсказания (запись в CSV)
            tasks_log = []
            for s in symbols:
                tasks_log.append(asyncio.to_thread(log_model_prediction_for_symbol, s))
            if tasks_log:
            ...
            # Периодически пробуем дообучить модель
            if iteration_count % 20 == 0:
                logger.info(f"[INNER_LOOP] iteration_count={iteration_count}, вызываем maybe_retrain_model()")
                await maybe_retrain_model()

            # Финальная синхронизация и генерация отчёта
            final_expos = get_exchange_positions()
            update_open_positions_from_exch_positions(final_expos)
            await asyncio.to_thread(generate_daily_pnl_report, "trade_log.csv", "daily_pnl_report.csv")
        ...
        # Завершение бота: если IS_RUNNING == False => /stop
        stop_drift_analyzer()
        drift_task.cancel()
