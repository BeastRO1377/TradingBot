...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler("GoldenML_Tolik.log", maxBytes=5 * 1024 * 1024, backupCount=2),
...
# Торговые параметры
MAX_TOTAL_VOLUME = Decimal("1500")  # общий лимит (USDT)
...
OPERATION_MODE = "ST_cross2_drift"  # Режимы: drift_only, drift_top10, golden_setup, model_only, super_trend, ST_cross1, ST_cross2, ST_cross_global
HEDGE_MODE = True
...
# def handle_position_update(message):
#     """
#     Теперь это колбэк для kline_stream (замена старого handle_message).
#     Обрабатывает свечные данные и при достижении целевого убытка — усредняет.
#     """
#     logger.info(f"[WS] Получено сообщение: {message}")
#     if "data" in message and isinstance(message["data"], list):
#         for candle in message["data"]:
...
def handle_position_update(message):
    ...
    if "data" in message and isinstance(message["data"], list):
        for candle in message["data"]:
            ...
            # Расчёт с учётом плеча (например, 10x) – аналогично логике в check_and_set_trailing_stop.
    ...
    try:
        ...
        if telegram_bot and telegram_message_queue:
            ...
            if result == "Opened":
                ...
                profit_perc = calculate_profit_perc(pos, current_price)
                message = (f"🔄 *Трейлинг стоп\-лосс*\n"
                        f"*Символ:* {symbol_link}\n"
                        f"*Время:* {escape_markdown(formatted_time)}\n"
                        f"*Расстояние:* {profit_perc}\n"
                        f"*Тип:* Трейлинг")
            else:
                message = (f"🫡🔄 *Сделка*\n"
                           f"*Символ:* {symbol_link}\n"
...
### ПРАВКА (6) train_and_load_model(...) – "На твоё усмотрение"
### Здесь оставим, но если хотите — можно использовать для обучения. 
...
def load_model():
    try:
        ...
        ### ПРАВКА: Заменяем return train_new_model() => return train_and_load_model()
...
def process_symbol_st_cross2_drift(symbol, interval="1", limit=200):
    """
    Обрабатывает символ по логике ST_cross2_drift.
    Если условия по пересечению SuperTrend удовлетворяются,
    открывается позиция основной логики (с проверкой времени обновления свеч).
    Дополнительно, если еще не открыта drift-позиция, производится выбор из drift‑таблицы
    и открывается позиция фиксированного объёма (500 USDT) вне глобального лимита.
    Все позиции (основные и drift) управляются единообразно трейлинг‑стопами и усреднением.
    """
    global drift_position_active
        ...
        if not df_fast.empty and not df_slow.empty:
            try:
                ...
                if last_candle_time >= pd.Timestamp.utcnow() - pd.Timedelta(minutes=5):
                    ...
                    prev_diff_pct = (Decimal(prev_diff) / Decimal(last_close)) * 100
                    curr_diff_pct = (Decimal(curr_diff) / Decimal(last_close)) * 100
                    # Логика ST_cross2: порог изменения 0.3% для смены сигнала
                    long_signal = (prev_diff_pct <= Decimal("-0.3") and curr_diff_pct >= Decimal("0.3"))
                    short_signal = (prev_diff_pct >= Decimal("0.3") and curr_diff_pct <= Decimal("-0.3"))
                ...
                if long_signal:
                    if curr_diff_pct > Decimal("1"):
                        ...
                        # Для основной логики ST_cross2 устанавливаем is_drift=False,
                        # чтобы ордер учитывался в глобальном лимите.
                        open_position(symbol, "Buy", POSITION_VOLUME, reason="ST_cross2_drift", is_drift=False)
                elif short_signal:
                    if curr_diff_pct < Decimal("-1"):
                        ...
                        open_position(symbol, "Sell", POSITION_VOLUME, reason="ST_cross2_drift", is_drift=False)
                else:
                    logger.info(f"[ST_cross2_drift] {symbol}: Условия для сигнала не выполнены.")
            except Exception as e:
                logger.error(f"[ST_cross2_drift] {symbol}: ошибка обработки: {e}")
    # Дополнительная логика по drift‑таблице (открывается только одна drift‑позиция)
    with drift_lock:
        if not drift_position_active:
            drift_signals = get_top_anomalies_from_analysis(drift_history, top_k=1)
            if drift_signals:
                top_symbol, _, top_direction = drift_signals[0]
                # Определяем направление: если сигнал "вниз" – открываем LONG (Buy), если "вверх" – SHORT (Sell)
                drift_side = "Buy" if top_direction == "вниз" else "Sell"
                open_position(top_symbol, drift_side, Decimal("500"), reason="ST_cross2_drift_drift", is_drift=True)
                drift_position_active = True
            else:
    ...
    try:
        ...
        for sym, pos in positions_copy.items():
            ...
            if leveraged_pnl_percent >= threshold_roi and not pos.get("trailing_stop_set", False):
                ...
                set_trailing_stop(sym, pos=open_positions[sym]["size"], trailing_gap_percent=TRAILING_GAP_PERCENT, side=side)
    except Exception as e:
...
def open_averaging_position(symbol):
    """
    Открывает позицию усреднения для заданного символа.
    Если базовая позиция является drift-позицией (is_drift=True),
    то глобальный лимит усреднения не учитывает её объём.
    """
    try:
        with open_positions_lock:
            if symbol not in open_positions:
            ...
            is_drift = base_pos.get("is_drift", False)
            
            global averaging_total_volume
            # Проверяем лимит усреднения только для обычных позиций
            if not is_drift and averaging_total_volume + base_volume_usdt > MAX_AVERAGING_VOLUME:
        ...
        if order_result and order_result.get("retCode") == 0:
            ...
            logger.info(f"[Averaging] Усредняющая позиция для {symbol} открыта на объём {base_volume_usdt}. Текущий усредняющий объём: {averaging_total_volume}")
        else:
...
### ПРАВКА (12) process_symbol(...) – «Оставить»
def process_symbol(symbol):
    """
    Универсальная функция для обработки символа (если хотите этим пользоваться).
    В текущем коде вызывается иной логикой, но можно задействовать,
    если хотите всё проверять в одном месте.
    """
    ...
    try:
        ...
        elif OPERATION_MODE == "ST_cross_global":
            process_symbol_st_cross_global(symbol, interval=INTERVAL)

        elif OPERATION_MODE == "ST_cross1":
            process_symbol_st_cross1(symbol, interval=INTERVAL)

        elif OPERATION_MODE == "ST_cross2":
            process_symbol_st_cross2(symbol, interval=INTERVAL)

        elif OPERATION_MODE == "model_only":
            # Либо тут, либо отдельным методом
            pass
...
### ПРАВКА (13) check_btc_correlation(...) – «Предложить как увязать»
###   Закомментируем, сохраним пример использования. 
"""
def check_btc_correlation(symbol, df):
    \"\"\"Проверяет корреляцию с BTC. Пример, как можно отсекать слишком коррелированные активы\"\"\"
    try:
        if symbol == \"BTCUSDT\":
            return True
...
### ПРАВКА (14) retry_on_error(...) – «Предложить как увязать»
###   Закомментируем, сохранить как пример декоратора.
"""
...
### ПРАВКА (15) sync_position_volumes(...) – Удалить
# Полностью убираем функцию sync_position_volumes(...) из кода.


# ===================== MAIN COROUTINE И ТЕЛЕГРАМ =====================
...
async def monitor_positions():
    ...
    while IS_RUNNING:
        try:
            ...
            # Получаем все актуальные позиции с биржи
            positions = get_exchange_positions()
            if not positions:
                continue
            ...
            for symbol, pos in positions.items():
                ...
                # Вычисляем базовый процент прибыли/убытка
                if side.lower() == "buy":
                    ratio = (Decimal(str(current_price)) - entry_price) / entry_price
                else:
                    ratio = (entry_price - Decimal(str(current_price))) / entry_price
                ...
                # Если убыток ниже порога, открываем усредняющую позицию
                if profit_perc <= -TARGET_LOSS_FOR_AVERAGING:
...
### Здесь у нас проблема: check_position_status(...) нет в коде. 
### Если нужно, придётся реализовать или закомментировать вызов.
...
def open_position(symbol: str, side: str, volume_usdt: Decimal, reason: str, is_drift: bool = False):
    """
    Открывает позицию для символа.
    Если is_drift=True, позиция открывается вне глобального лимита,
    но дальше подчиняется тем же правилам трейлинг‑стопов и усреднения.
    """
    if is_sleeping():
    ...
    try:
        ...
        # Для НЕ drift-позиций проверяем глобальный лимит (только считаем позиции без флага is_drift)
        if not is_drift:
            with state_lock:
                current_total = Decimal("0")
                with open_positions_lock:
                    for pos in open_positions.values():
                        if not pos.get("is_drift", False):  # учитываем только основную логику
        ...
        # Сбор признаков для логирования модели
        features_dict = {}
        ...
        with open_positions_lock:
            open_positions[symbol] = {
                ...
                "trade_id": trade_id,
                "open_time": datetime.datetime.utcnow(),
                "is_drift": is_drift  # Помечаем drift-позицию
            }
        
        # Обновляем глобальный лимит только для НЕ drift-позиций
        if not is_drift:
            with state_lock:
...
async def check_position_status(symbol, pos):
    # Заглушка или любая логика контроля
    pass
...
def calculate_profit_perc(pos, current_price):
    try:
        ...
        if pos.get("side", "").lower() == "buy":
            ...
            ratio = (entry_price - Decimal(str(current_price))) / entry_price
        return (ratio * PROFIT_COEFFICIENT).quantize(Decimal("0.0001"))
    except Exception as e:
        logger.exception(f"Ошибка в calculate_profit_perc: {e}")
        return "N/A"

def get_top_anomalies_from_analysis(analysis_data, top_k=10):
    try:
        anomalies = []
        # analysis_data — словарь вида: { symbol: [(timestamp, strength, direction), ...], ... }
        for symbol, recs in analysis_data.items():
            if recs:
                # Усредняем силу аномалии по всем записям для данного символа
                avg_strength = sum(record[1] for record in recs) / len(recs)
                # Берем последнее направление (из последнего кортежа)
                last_direction = recs[-1][2]
                anomalies.append((symbol, avg_strength, last_direction))
        anomalies.sort(key=lambda x: x[1], reverse=True)
        return anomalies[:top_k]
    except Exception as e:
        logger.exception(f"Ошибка в get_top_anomalies_from_analysis: {e}")
    ...
    try:
        ...
        while IS_RUNNING:
            try:
                ...
                if OPERATION_MODE in ["ST_cross_global", "ST_cross1", "ST_cross2", "ST_cross2_drift"]:
                    tasks = []
                    for s in symbols:
