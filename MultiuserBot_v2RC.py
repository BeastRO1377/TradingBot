...
class TradingBot:
    ...
    __slots__ = (
        ...
        "ai_rev_pending", "ai_rev_results", "last_ai_check", "trailing_activated",
        )
    ...
    async def manage_open_position(self, symbol: str):
        """
        [V15.1 – resilient]
        Гарантированный трейлинг + периодический AI, устойчив к любым исключениям.
        """
        logger.info(f"🛡️ [AI Guardian] Активирован для {symbol}.")

        # флаг включения трейлинга
        if 'trailing_activated' not in self.open_positions.get(symbol, {}):
            pos = self.open_positions.get(symbol)
            if pos:
                pos['trailing_activated'] = False

        last_ai_check = time.time()
        ai_check_interval = 30  # сек
        last_heartbeat = 0
        ...
        while symbol in self.open_positions:
            try:
                ...
                last_price = float(pos.get("markPrice") or pos.get("last_price") or 0)
                lev = float(pos.get("leverage") or self.leverage or 1)

                if avg_price <= 0 or last_price <= 0:
                    continue

                # ROI в %, с учётом стороны и плеча
                if side == "Buy":
                    roi_pct = (last_price / avg_price - 1.0) * 100.0 * lev
                else:
                    roi_pct = (avg_price / last_price - 1.0) * 100.0 * lev

                # Первичное включение трейлинга при достижении порога
                if not pos.get('trailing_activated', False) and roi_pct >= self.trailing_start_pct:
                    new_stop = self._calculate_initial_profitable_stop(avg_price, side, lev)  # твоя функция
                    await self.set_or_amend_stop_loss(symbol, new_stop)
                    pos['trailing_activated'] = True
                    logger.info(f"✅ [{symbol}] Трейлинг АКТИВИРОВАН. ROI={roi_pct:.2f}%. Стоп={new_stop:.6f}")

                # Собственно трейлинг: подтягиваем с отступом gap_pct (%)
                if pos.get('trailing_activated', False):
                    gap = float(getattr(self, "trailing_gap_pct", 2.5))
                    if side == "Buy":
                        candidate = last_price * (1.0 - gap / 1000.0)
                        cur = float(self.last_stop_price.get(symbol, 0) or 0)
                        if cur <= 0 or candidate > cur:
                            await self.set_or_amend_stop_loss(symbol, candidate)
                    else:
                        candidate = last_price * (1.0 + gap / 1000.0)
                        cur = float(self.last_stop_price.get(symbol, 0) or 0)
                        if cur <= 0 or candidate < cur:
                ...
                # Периодически пингуем AI-советника (без права «ронять» цикл)
                now = time.time()
                if now - last_ai_check >= ai_check_interval:
                    last_ai_check = now
                    try:
                        # твой существующий вызов:
                        # advice = await self._position_advice_with_ai(symbol, position_data=pos)
                        pass
                    except asyncio.TimeoutError:
                        logger.warning(f"⏳ [{symbol}] AI timeout — продолжаю по базовой логике.")
                    except Exception as e:
                        logger.error(f"💥 [{symbol}] AI советник: {e}", exc_info=True)

                # Heartbeat раз в 60с
                if now - last_heartbeat >= 60:
                    last_heartbeat = now
                    logger.debug(f"[Guardian♥] {symbol} live; stop={self.last_stop_price.get(symbol)}")

            except Exception as e:
                # Любая ошибка внутри — логируем и продолжаем цикл (таска не умирает)
                logger.error(f"💥 [Guardian] {symbol} loop error: {e}", exc_info=True)
                await asyncio.sleep(0.5)

        logger.info(f"🛡️ [Guardian] Позиция {symbol} закрыта. Хранитель завершён.")
            ...
            async with self.position_lock:
                ...
                if exec_side and pos.get("side") and exec_side != pos.get("side"):
                    ...
                    if self._sf(exec_data.get("leavesQty", 0)) == 0:
                        ...
                        comment = pos.get("entry_candidate", {}).get("justification") or \
                                self.last_entry_comment.pop(symbol, "Причина не указана.")
    ...
    async def handle_position_update(self, msg: dict):
        """
        [V10 - CORRECT COMMENT & TRAILING FLAG] Обрабатывает открытие, обновление и 
        служит надежным fallback-механизмом для закрытия.
        """
        data = msg.get("data", [])
        ...
        async with self.position_lock:
            for p in data:
                ...
                if prev_pos is None and new_size > 0:
                    ...
                    # >>> КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Получаем комментарий из правильного хранилища <<<
                    comment = self.pending_strategy_comments.pop(symbol, None)
                    
                    # Запомним комментарий, чтобы показать его и при закрытии
                    if comment:
                    ...
                    self.open_positions[symbol] = {
                        ...
                        "entry_candidate": {}, # Просто пустой словарь для совместимости
                        "markPrice": avg_price, 
                        "pnl": 0.0,
                        "entry_features": self._build_entry_features(symbol),
                        'trailing_activated': False # Инициализируем флаг трейлинга
                    }
                    logger.info(f"[PositionStream] NEW {side_raw} {symbol} {new_size:.3f} @ {avg_price:.6f}")
                    
                    # Логируем открытие, передавая извлеченный комментарий
                    asyncio.create_task(self.log_trade(
                    ...
                    # Запускаем "Хранителя" для этой позиции
                ...
                if prev_pos and new_size > 0 and abs(new_size - self._sf(prev_pos.get("volume", 0))) > 1e-9:
                    ...
                    self.open_positions[symbol]["avg_price"] = self._sf(p.get("avgPrice") or p.get("entryPrice"))
                    self.write_open_positions_json()
                    continue

                # --- Сценарий 3: Fallback-закрытие ---
    ...
    async def _risk_check(self, symbol: str, side: str, qty: float, last_price: float, cid: str | None = None) -> bool:
        if symbol in EXCLUDED_SYMBOLS:
            logger.info(f"[Risk] Skip {symbol} — EXCLUDED_SYMBOLS")
            return False
        if last_price is None or last_price <= 0:
            logger.info(f"[Risk] Skip {symbol} — last_price<=0 ({last_price})")
            return False

        if symbol in self.pending_orders:
            # Разрешаем пройти текущей попытке (тот же cid)
            try:
                if cid is not None and self.pending_cids.get(symbol) == cid:
                    pass
                else:
        ...
        if est_cost > self.POSITION_VOLUME:
            logger.info(
                f"[Risk] Skip {symbol} — стоимость {est_cost:.0f} > лимита на позицию {self.POSITION_VOLUME:.0f}"
            )
            return False
        ...
        if effective_volume + est_cost > self.MAX_TOTAL_VOLUME:
            logger.warning(
                f"[Risk] Skip {symbol} — превышен ОБЩИЙ лимит. "
    ...
    def _build_primary_prompt(self, candidate: dict, features: dict) -> str:
        ...
        btc_1h, eth_1h = self._market_context_from_features(features)
        ...
        prompt = (
            "SYSTEM: Ты — профессиональный трейдер, риск-менеджер и крипто-квант. "
            "Отвечай кратко и по делу. Итоговый ответ будет приведён к JSON на стороне вызова.\n"
            "USER:\n"
            f"Анализ сигнала ({mode}):\n"
            f"- Инструмент: {sym}, Направление: {side}, Источник: {src}\n"
            f"- Метрики: PriceΔ(5m)={price_5m}%, Volume Anomaly={vol_anom_s}x, OIΔ(1m)={oi_1m}%\n"
            f"- Техника: Trend={trend}, ADX={adx}, RSI={rsi}\n"
            f"- Рынок: BTC Δ(1h)={btc_1h}%, ETH Δ(1h)={eth_1h}%\n\n"
            "ЗАДАЧА: Оцени вероятность прибыльного входа с учётом направления, дисперсии объёмов и контекста рынка. "
            "Сформируй вывод в JSON с ключами:\n"
            '- "confidence_score": число [0..1],\n'
            '- "action": "EXECUTE" или "REJECT",\n'
            '- "justification": короткое объяснение (начни с источника сигнала).\n'
        )
        return prompt
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        logger.info("[EXECUTE_CALL] %s/%s src=%s price=%s vol_usdt=%s",
                    symbol, side, source, features.get('price'),
                    candidate.get('volume_usdt', self.POSITION_VOLUME))

        started = True          # ставим True в момент «все проверки пройдены»
        exit_reason = "unknown"  # переписываем перед каждым ранним return

        try:
            last_price = float(features.get("price", 0) or 0.0)

            # Подстраховка по цене
            tp = safe_to_float(self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0))
            if tp > 0 and (last_price <= 0 or abs(tp - last_price) / max(tp, 1e-12) > 0.20):
                logger.warning("[PRICE_SANITY] %s: features.price=%s -> use ticker.lastPrice=%s",
                            symbol, last_price, tp)
                last_price = tp
            ...
            if symbol in self.pending_orders or symbol in self.active_trade_entries:
                exit_reason = "pending_or_processing"
                logger.info("[EXECUTE_DENY][%s] %s/%s: уже pending/processing", cid, symbol, side)
                return

            if now - self.last_entry_ts.get(symbol, 0) < self.entry_cooldown_sec:
                exit_reason = "cooldown_last_entry"
                logger.info("[EXECUTE_COOLDOWN][%s] %s/%s: pause %ss",
                            cid, symbol, side, self.entry_cooldown_sec)
                return

            if now - self.recently_closed.get(symbol, 0) < self.entry_cooldown_sec:
                exit_reason = "cooldown_recently_closed"
                logger.info("[EXECUTE_COOLDOWN][%s] %s/%s: recently closed", cid, symbol, side)
                return

            # === Funding-gate (с троттлингом логов) ===
            funding_rate = None
            try:
                if hasattr(self.shared_ws, "funding_cache"):
                    funding_rate = self.shared_ws.funding_cache.get(symbol)
                if funding_rate is None:
                    funding_rate = features.get("fundingRate") or features.get("funding") or None
            ...
            if funding_rate is not None:
                thr_pos = getattr(self, "funding_thr_pos", 0.0010)   # +0.10%
                thr_neg = getattr(self, "funding_thr_neg", -0.0010)  # -0.10%
                if not self._funding_allows(symbol, side, funding_rate,
                                            thr_pos=thr_pos, thr_neg=thr_neg,
                                            log_every_sec=600):
                    exit_reason = "funding_gate"
                    return
            # === /Funding-gate ===
            ...
            # Диагностика перед риском
            step = self.qty_step_map.get(symbol, 0.001)
            min_qty = self.min_qty_map.get(symbol, step)
            logger.info("[EXEC_PRECHECK][%s] %s/%s price=%.12f usd=%.2f -> qty=%.12f step=%s minQty=%s",
                        cid, symbol, side, last_price, volume_usdt, qty, step, min_qty)

            if not (qty > 0):
                exit_reason = "qty_zero"
            ...
            # Риск-чек
            if not await self._risk_check(symbol, side, qty, last_price, cid=cid):
                exit_reason = "risk_check_fail"
                logger.info("[EXECUTE_DENY][%s] %s/%s: отклонено _risk_check", cid, symbol, side)
                return

            # --- собрать OPINION для телеги и ордер-логов ---
            ai_comment = (
                (candidate.get("comment") or candidate.get("justification") or "")
                .strip()
            )

            if ai_comment:
                ai_comment = f"JARVIS' OPINION: {ai_comment}"
            else:
                # мягкий запасной вариант, чтобы не было пусто
                src = candidate.get("source") or candidate.get("setup") or "strategy"
                ai_comment = f"JARVIS' OPINION: {src}"

            # сохраним заранее — PositionStream может прилететь раньше ack/place_unified_order
            self.pending_strategy_comments[symbol] = ai_comment

            # Резервируем pending (под локом ТОЛЬКО этого символа)
            sym_lock = self._get_sym_lock(symbol)
            try:
                await asyncio.wait_for(sym_lock.acquire(), timeout=1.5)
            except asyncio.TimeoutError:
                who, since = self._lock_owner.get(symbol, ("unknown", 0.0))
                waited_ms = int((time.time() - since) * 1000) if since else 0
                logger.error("[LOCK_TIMEOUT][%s] %s/%s: sym_lock занят (owner=%s, %d ms). Пропуск — будет повтор на следующем цикле.",
                            cid, symbol, side, who, waited_ms)
                return

            try:
                # помечаем владельца локом для диагностики
                self._lock_owner[symbol] = (f"execute_trade_entry:{cid}", time.time())

                # быстрые проверки/пометки — ТОЛЬКО под локом
                if symbol in self.pending_orders or symbol in getattr(self, "active_trade_entries", {}):
                    logger.info("[EXECUTE_DENY][%s] %s/%s: уже pending/processing", cid, symbol, side)
                    return
                ...
                # освобождаем метку владельца и лок МГНОВЕННО — никакой сети внутри!
                self._lock_owner.pop(symbol, None)
                if sym_lock.locked():
                    sym_lock.release()

            pos_idx = 1 if side == "Buy" else 2

            # 1) бюджет строго из candidate/настроек
            ...
            # 3) безопасный лог-пречек
            step = float(self.qty_step_map.get(symbol, 1.0))
            min_qty = float(self.min_qty_map.get(symbol, step))
            logger.info(
                "[EXEC_PRECHECK][%s] %s/%s price=%.12f usd=%.2f -> qty=%.12f step=%s minQty=%s",
                cid, symbol, side, last_price, usd_amount, qty, step, min_qty
            )

            # 4) обычный риск-чек и размещение
            if not await self._risk_check(symbol, side, qty, last_price, cid=cid):
                self.pending_orders.pop(symbol, None)
                self.pending_cids.pop(symbol, None)
                self.pending_timestamps.pop(symbol, None)
            ...
            if source == 'squeeze':
                if self.mode == 'real':
                    resp = await asyncio.wait_for(
                        self.adaptive_squeeze_entry_ws(symbol, side, qty, pos_idx), timeout=6.0
                    )
                    logger.info("[ORDER][%s] squeeze_ws ack %s", cid, str(resp)[:400])
                else:
                    resp = await asyncio.wait_for(
                        self.adaptive_squeeze_entry(symbol, side, qty), timeout=6.0
                    )
                    logger.info("[ORDER][%s] squeeze_rest ack %s", cid, str(resp)[:400])

            elif source == 'liquidation':
                resp = await asyncio.wait_for(
                    self.place_unified_order(symbol, side, qty, "Market"), timeout=6.0
                )
            ...
            logger.critical("[execute_trade][%s] Uncaught %s for %s/%s: %s",
                            cid, type(e).__name__, symbol, side, e, exc_info=True)
            return

        finally:
            if not started:
                # Ордер так и не стартовал — очищаем pending-признаки
                self.pending_orders.pop(symbol, None)
                self.pending_cids.pop(symbol, None)
                self.pending_timestamps.pop(symbol, None)
                logger.warning(
                    "[EXECUTE_EXIT][%s] %s/%s: завершилось без старта (reason=%s)",
                    cid, symbol, side, exit_reason
                )
            else:
                # Ордер принят биржей — pending снимется по приватному стриму
                logger.debug(
                    "[EXECUTE_EXIT][%s] %s/%s: started=True — cleanup отдаём приватному WS",
                    cid, symbol, side
                )
...
# --- SQUEEZE FILTER (REAL) ---------------------------------------------------
    ...
    # --- Главный оркестратор сквиза: дергает helpers + AI и решает, входить ли ---
    async def _squeeze_logic(self, symbol: str) -> bool:
        """
        [V2 - CORRECTED] True -> сквиз обработан (вход или отказ), golden_setup пропускается.
        False -> сигнала нет, golden_setup продолжает работу.
        """
        ...
        if symbol in getattr(self, "active_trade_entries", {}):
            ent = self.active_trade_entries.get(symbol) or {}
            if ent.get("source") == "squeeze":
                return True
        if symbol in getattr(self, "pending_orders", {}):
            return True

        # --- 1. КОРРЕКТНЫЙ РАСЧЕТ ПОРОГА ДЕТЕКТА ---
        ret_5m = float(features.get("ret_5m", 0.0))
        atr_5m = float(features.get("atr_5m", 0.0))
        price = float(features.get("price", 0.0))

        if price <= 0:
            return False # Не можем рассчитать порог без цены

        # Базовый порог в 3% (более чувствительный)
        thr_base = float(getattr(self, "squeeze_min_abs_move", 0.03)) 
        k = float(getattr(self, "squeeze_k_factor", 1.5))
        
        # Динамический порог: k * ATR в виде процента от текущей цены
        thr_dyn = (k * atr_5m / price) if atr_5m > 0 else 0.0
        
        # Используем больший из двух порогов
        threshold = max(thr_base, thr_dyn)

        if abs(ret_5m) < threshold:
            return False # Движение недостаточно сильное, передаем управление дальше

        # Если мы здесь, значит, сквиз был обнаружен
        logger.info(f"🔥 [{symbol}] ОБНАРУЖЕН СКВИЗ! Движение: {ret_5m*100:.2f}%, Порог: {threshold*100:.2f}%.")

        # 2) Судья (логика остается прежней)
        try:
            judge = await self._ai_squeeze_judge(
                candidate={"symbol": symbol, "side": ("Sell" if impulse_dir == "up" else "Buy"), "setup": "squeeze"},
                features=features,
                impulse_dir=impulse_dir
            )
        except Exception:
            logger.exception("[SQUEEZE_JUDGE] %s: fallback to WAIT", symbol)
            judge = {"action": "WAIT", "style": "limit_pullback", "side": ("Sell" if impulse_dir == "up" else "Buy"), "justification": "judge_error"}

        action = (judge or {}).get("action", "WAIT")

        # 3) Решение (логика остается прежней)
        if action == "ENTER":
            side = judge.get("side") or ("Sell" if impulse_dir == "up" else "Buy")
            candidate = {
                "symbol": symbol,
                "side": side,
                "setup": "squeeze",
            ...
            return True # Сквиз обработан

        if action == "CANCEL":
            logger.info(f"[{symbol}] Сквиз-сигнал отменен AI-судьей. Причина: {judge.get('justification')}")
            return True # Сквиз обработан (отказом)

        # Если действие "WAIT", мы ничего не делаем и позволяем golden_logic продолжить
        logger.info(f"[{symbol}] Сквиз-сигнал в режиме ожидания. Причина: {judge.get('justification')}")
        return False
    ...
    async def _liquidation_logic(self, symbol: str) -> bool:
        try:
            ...
            # headroom: сколько осталось до MAX_TOTAL_VOLUME с учётом pending
            effective_volume = await self.get_effective_total_volume()
            headroom = max(0.0, self.MAX_TOTAL_VOLUME - effective_volume)
            if headroom <= 0:
                return False

            usd_amount = min(
                self.POSITION_VOLUME,
                getattr(self, "max_allowed_volume", self.POSITION_VOLUME),
                headroom,
            )
    ...
    async def place_unified_order(self, symbol: str, side: str, qty: float,
        ...
        try:
            if self.mode == "real":
                # ─────────────────────────────────────────────
                # РЕАЛ: ордер уходит по WS, REST может не вернуть ack.
                # ─────────────────────────────────────────────
                ...
                # Пытаемся вытащить orderId из ответа WS…
                order_id = None
                try:
                ...
                # …или из кэша, который заполняет WS-колбэк (если у вас он есть)
                if not order_id:
                    order_id = getattr(self, "_last_ws_order_id", {}).get(symbol)

                if order_id:
                    # корреляция cid <-> order_id
                    try:
                        self.order_correlation[order_id] = cid
                    except Exception:
                        pass
                ...
                # Возвращаем совместимый ответ, чтобы остальной код не ломался
                resp_ok = {
                    "retCode": 0,
                    "retMsg": "OK",
                ...
                # ─────────────────────────────────────────────
                # ДЕМО/ТЕСТ: идем через REST и строго проверяем retCode
                # ─────────────────────────────────────────────
                response = await asyncio.to_thread(
                ...
                if response.get("retCode") != 0:
                    # Важно: InvalidRequestError(message, time, resp_headers)
                    raise InvalidRequestError(
                ...
                # для маркет-ордеров дожидаемся материализации позиции в демо
                for _ in range(10):
            ...
            logger.error(f"❌ Ошибка размещения ордера для {symbol} (API Error): {e} "
                        f"(Код: {getattr(e, 'status_code', None)})")
            self.pending_orders.pop(symbol, None)
            self.pending_cids.pop(symbol, None)
            self.pending_timestamps.pop(symbol, None)
            self.pending_cids.pop(symbol, None)
