...
# --- SQUEEZE FILTER (REAL) ---------------------------------------------------
    ...
    def _build_squeeze_features_5m(self, symbol: str):
        """
        Возвращает (features: dict, impulse_dir: 'up'|'down') или (None, None), если данных мало.
        Основано на последних 5m-барах, собранных из минуток WS.
        """
        try:

            # 6 последних 5m баров (хватает для ретрейсов/серий)
            bars = self._aggregate_ohlcv_5m(symbol, lookback=6)
            if len(bars) < 2:
                return None, None

            prev, last = bars[-2], bars[-1]
            pc = float(prev.get("close") or 0.0)
            lc = float(last.get("close") or 0.0)
            if pc <= 0 or lc <= 0:
                return None, None
            ...
            hi = max(float(last.get("high", lc) or lc), float(prev.get("high", pc) or pc))
            lo = min(float(last.get("low",  lc) or lc), float(prev.get("low",  pc) or pc))
            atr_1m = max(hi - lo, 0.0)  # грубый прокси

            volume_series = [float(b.get("volume", 0.0) or 0.0) for b in bars[-5:]]
            price_series  = [float(b.get("close",  0.0) or 0.0) for b in bars[-10:]]

            # История минутных объёмов для z-score/percentile
            from collections import deque as _dq
            vh = list(self.shared_ws.volume_history.get(symbol, _dq()))
            if len(vh) >= 30:
                base = np.array(vh[-200:] if len(vh) > 200 else vh, dtype=float)
            ...
            # OI/CVD ряды (если есть)
            oi_series  = list(self.shared_ws.oi_history.get(symbol, _dq()))[-20:]
            cvd_series = list(self.shared_ws.cvd_history.get(symbol, _dq()))[-20:]

            # VWAP (если у тебя где-то считается — подставь здесь)
            vwap = None
            vwap_band_std = None

            features = {
                "price": lc,
                "ret_1m": float(ret_5m),         # используем 5m как прокси «1m скорости»
                "atr_1m": float(atr_1m),
                "impulse_high": hi,
                "impulse_low":  lo,
                ...
                "oi_delta_1m_pct": None,   # подставишь, если начнешь писать OI-дельту
                "liq_sigma":      None,    # можно кормить из кластеров ликвидаций
            }
    ...
    async def _squeeze_logic(self, symbol: str) -> bool:
        """
        True  -> сквиз обработан здесь (инициирована сделка или явный отказ) — golden_setup можно пропустить.
        False -> сигнала нет или WAIT — пусть golden продолжает.
        """
        # 0) Фичи
        ...
        # --- анти-дубликат по вашему контракту ---
        if symbol in getattr(self, "active_trade_entries", {}):
            ent = self.active_trade_entries.get(symbol) or {}
            if ent.get("source") == "squeeze":
                return True  # уже обрабатываем сквиз по этому символу
        if symbol in getattr(self, "pending_orders", {}):
            return True  # ордер по символу уже в процессе

        # 1) Порог детекта: max(ваш базовый, k*ATR5m) — без новых атрибутов/слотов
        ret_1m = float(features.get("ret_1m") or 0.0)
        thr_base = float(getattr(self, "squeeze_min_abs_move", 0.045))  # ваш старый 4.5%
        atr_5m = float(features.get("atr_5m") or 0.0)
        k = float(getattr(self, "squeeze_k_factor", 1.5))               # если нет поля — дефолт 1.5
        thr_dyn = (k * atr_5m) if atr_5m > 0 else 0.0
        thr = max(thr_base, thr_dyn)
        if thr <= 0 or abs(ret_1m) < thr:
            return False
        ...
        try:
            ...
            judge = {"action": "WAIT", "style": "limit_pullback",
                    "side": ("Sell" if impulse_dir == "up" else "Buy"),
                    "justification": "judge_error"}
        ...
        if action == "ENTER":
            ...
            # Ведём через единый пайплайн (как и golden/liquidation)
            await self.evaluate_entry_candidate(candidate, features)
            return True

        if action == "CANCEL":
            return True  # явный отказ — считаем обработанным

        # WAIT — пусть golden дальше рассматривает
        return False
    ...
    async def _submit_with_risk(self, symbol: str, side: str, qty: float,
                                *, last_price: float, order_type: str = "Market",
                                price: float | None = None, comment: str = "", cid: str | None = None) -> bool:
        """
        Унифицированная отправка ордеров с ЖЁСТКИМИ проверками риска и pending-блокировкой:
        - уважает POSITION_VOLUME и MAX_TOTAL_VOLUME;
        - блокирует дубль по symbol на время размещения (pending_orders);
        - на успех/ошибку — корректно чистит pending.
        """
        cid = cid or (f"risk-{int(__import__('time').time()*1000)}")
        last_price = float(last_price or 0.0)
        if qty <= 0 or last_price <= 0:
            return False
    
        # Строгий риск-чек
        if hasattr(self, "_risk_check"):
            ok = await self._risk_check(symbol, side, qty, last_price, cid=cid)
            if not ok:
                return False
    
        # Pending-блокировка ТОЛЬКО на этот символ
        sym_lock = self._get_sym_lock(symbol) if hasattr(self, "_get_sym_lock") else None
        if sym_lock is not None:
            try:
                await __import__('asyncio').wait_for(sym_lock.acquire(), timeout=1.5)
            except __import__('asyncio').TimeoutError:
                return False
    
        try:
            if symbol in getattr(self, "pending_orders", {}) or symbol in getattr(self, "active_trade_entries", {}):
                return False
    
            self.pending_orders[symbol] = float(qty) * float(last_price)
            self.pending_timestamps[symbol] = __import__('time').time()
            try:
                self.pending_cids[symbol] = cid
            except Exception:
                pass
    
            resp = await self.place_unified_order(symbol, side, qty, order_type, price=price, comment=comment, cid=cid)
            return bool(resp)
        finally:
            try:
                self.pending_orders.pop(symbol, None)
                ...
                pass
            try:
                if sym_lock is not None and sym_lock.locked():
                    sym_lock.release()
            except Exception:
                pass
