...
async def _ask_ollama_json(self, model: str, messages: list, *,
                           num_predict: int = 160, temperature: float = 0.2, top_p: float = 0.9,
                           keep_alive: str | int | None = "30m", timeout: float | None = None):
    from openai import AsyncOpenAI
    import httpx, asyncio, openai

    if not hasattr(self, "_ollama_pull_once"): self._ollama_pull_once = set()
    client = AsyncOpenAI(base_url=self.ai_base_url, api_key="ollama")
    ...
    try:
        resp = await asyncio.wait_for(
            client.chat.completions.create(
                ...
                top_p=top_p,
                extra_body={"options": {"num_predict": num_predict, "repeat_penalty": 1.1},
                            "keep_alive": keep_alive},
            ),
            timeout or self.ai_timeout_sec,
        )
        return resp.choices[0].message.content

    except openai.NotFoundError as e:
        # auto-pull один раз и быстрый повтор
        if model not in self._ollama_pull_once:
            self._ollama_pull_once.add(model)
            try:
                async with httpx.AsyncClient(timeout=None) as cli:
                    await cli.post("http://localhost:11434/api/pull", json={"name": model})
                logger.info(f"[AI_PULL] {model} pulled, retrying…")
                return await self._ask_ollama_json(model, messages, num_predict=num_predict,
                                                   temperature=temperature, top_p=top_p,
                                                   keep_alive=keep_alive, timeout=3.5)
            except Exception as pe:
                logger.error(f"[AI_PULL_FAIL] {model}: {pe}")
        raise
...
class TradingBot:
    ...
    __slots__ = (
        ...
        "ai_primary_model", "ai_advisor_model", "ai_base_url",
        )
    ...
    async def ensure_ollama_models(self, models: list[str], keep_alive="30m"):
        import httpx
        base = "http://localhost:11434"
        async with httpx.AsyncClient(timeout=30) as cli:
            tags = await cli.get(f"{base}/api/tags")
            have = {m.get("name") for m in tags.json().get("models", [])}
            for name in models:
                if name not in have:
                    logger.warning(f"[AI_INIT] pulling {name}")
                    await cli.post(f"{base}/api/pull", json={"name": name}, timeout=None)
        # быстрый прогрев
        try: await self._ask_ollama_json(self.ai_primary_model,  [{"role":"user","content":"ping"}], keep_alive=keep_alive, num_predict=1)
        except: pass
        try: await self._ask_ollama_json(self.ai_advisor_model,  [{"role":"user","content":"ping"}], keep_alive=keep_alive, num_predict=1)
        except: pass
    ...
    async def _ai_dispatch(self, provider: str, candidate: dict, features: dict) -> dict:
        ...
        messages = [{"role":"user","content": self._build_primary_prompt(candidate, features)}]

        # PRIMARY
        async with self.ai_primary_sem:
            primary_json = await self._ask_ollama_json(self.ai_primary_model, messages)

        primary = safe_parse_json(primary_json, default={"action":"REJECT","confidence_score":0.0,"justification":"invalid"})
        # Негативная лексика → принудительный REJECT
        just_low = (primary.get("justification") or "").lower()
        if primary.get("action","REJECT").upper()=="EXECUTE" and any(c in just_low for c in NEGATIVE_CUES):
            primary["action"] = "REJECT"

        # ADVISOR (Plutus) — не влияет на решение, идёт в фоне
        asyncio.create_task(self._advisor_plutus(candidate, features, primary))
    ...
    async def evaluate_entry_candidate(self, candidate: dict, features: dict):
        ...
        # Кэш от повторной переоценки одного и того же сигнала
        if self._evaluated_signals_cache.get(signal_key) and (
            now - self._evaluated_signals_cache.get(signal_key, {}).get("time", 0) < CACHE_TTL_SEC
        ):
            return
        self._evaluated_signals_cache[signal_key] = {"status": "pending", "time": now}

        # --- WATCH gate: если символ под наблюдением, игнорируем кандидата и НЕ вызываем AI ---
        try:
            w = self.watchlist.get(symbol) if hasattr(self, "watchlist") else None
            if w and now < float(w.get("t_expire", 0)):
                t_exp = time.strftime("%H:%M:%S", time.localtime(w["t_expire"]))
                logger.info(f"[WATCH_ACTIVE] {symbol}/{side}: под WATCH до {t_exp}, кандидат пропущен")
                return
        except Exception as _e:
            logger.warning(f"[WATCH_GATE_WARN] {symbol}: не удалось проверить статус WATCH: {_e}")
        ...
        try:
            ...
            if getattr(self.ml_inferencer, "model", None) is not None:
                try:
                    ...
                    if not (side_check_ok and roi_threshold_ok):
                        logger.debug(
                            f"[ML_GATE_REJECT] {symbol}/{side} ({source}) | "
                            f"Ожидаемый ROI: {expected_roi:.2f}%, Порог: {self.ml_gate_abs_roi:.2f}%"
                        )
                        return
            ...
            # --- AI circuit breaker (мягкая пауза) ---
            if now < getattr(self, "ai_circuit_open_until", 0):
            ...
            # --- Вызов AI с ограничением конкурентности и таймаутом ---
            ...
            ai_action = (ai_response.get("action") or "REJECT").upper()
            if ai_action != "EXECUTE":
                ...
                # Если режим разрешён для наблюдения и уверенность ≥ порога — берём под WATCH (5 мин)
                if candidate.get("mode") in {"golden_squeeze", "liq_squeeze", "full"} and \
                ai_response.get("confidence_score", 0) >= 0.35:
                    self.put_on_watchlist(candidate, features)
                return

            # Доходим сюда только если НЕ под WATCH и все гейты выше пройдены
    ...
    async def evaluate_candidate_with_ollama(self, candidate: dict, features: dict) -> dict:
        ...
        try:
            ...
            primary_task = asyncio.create_task(_ask_ollama_json(AI_PRIMARY_MODEL, prompt, AI_TIMEOUT_PRIMARY))
            confirm_task = asyncio.create_task(_ask_ollama_json(AI_CONFIRM_MODEL, prompt, AI_TIMEOUT_CONFIRM))
            ...
            prim_conf = _clamp01(ai_primary.get("confidence_score", 0.0))
            prim_just = ai_primary.get("justification", "")

            conf_act  = (ai_confirm.get("action") or "REJECT").upper()
            conf_conf = _clamp01(ai_confirm.get("confidence_score", 0.0))
