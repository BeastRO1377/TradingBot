...
class TradingBot:
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        # после создания других словарей-таймеров
        self.last_liq_trade_time: dict[str, float] = {}
...
# ─── ОБРАБОТКА POSITION_STREAM ──────────────────────────────────────────────────
    ...
    async def execute_golden_setup(self, symbol: str):
        ...
        1. Squeeze‑entry (counter‑trend limit) — 5‑minute price/volume spike.
        2. Liquidation‑entry — large cluster of forced liquidations.
        3. Classic Golden‑Setup momentum play (price/vol/oi/cvd).

        Early‑exit as soon as any sub‑strategy fires.
        """
        try:
            # ── BASIC FILTERS ────────────────────────────────────────────────
            if (await self.listing_age_minutes(symbol)) < self.listing_age_min:
                return
            if symbol in (*self.closed_positions, *self.open_positions,
                           *self.pending_orders, *self.reserve_orders):
                return
            if symbol in self.failed_orders and time.time() - self.failed_orders[symbol] < 600:
                return

            # пропуск GS, если уже был резкий рост и разрешён сквиз‑режим
            if self._squeeze_allowed(symbol) and self.shared_ws.has_5_percent_growth(symbol, 20):
                return

            # helper для суммирования объёма в N последних минут с возможным сдвигом
            def _sum_vol(candles, length: int, shift: int = 0) -> float:
                """
                Суммирует поле 'volume' за последние `length` минут.
                Если shift > 0, окно берётся `length` баров *раньше*.
                Работает и для deque (без slice-поддержки) — сначала делаем list().
                """
                clist = list(candles)                     # гарантируем slicing
                needed = length + shift
                if len(clist) < needed:
                    return 0.0
                if shift == 0:
                    window = clist[-length:]              # последние length баров
                else:
                    window = clist[-needed:-shift]        # смещённое окно
                return sum(safe_to_float(c.get("volume", 0)) for c in window)

            # -----------------------------------------------------------------
            # 1.  S Q U E E Z E   S T R A T E G Y  (5‑min spike, contrarian)
            # -----------------------------------------------------------------
            if self._squeeze_allowed(symbol):
                c1m = self.shared_ws.candles_data.get(symbol, [])
                if len(c1m) >= 6:                    # need 5 completed bars
                    ticker = self.shared_ws.ticker_data.get(symbol, {})
                    last_price = safe_to_float(ticker.get("lastPrice", 0)) or \
                                 safe_to_float(c1m[-1]["closePrice"])
                    if last_price <= 0:
                        return

                    old_close = safe_to_float(c1m[-6]["closePrice"])
                    new_close = safe_to_float(c1m[-1]["closePrice"])
                    if old_close == 0:
                        return
                    pct_5m = (new_close - old_close) / old_close * 100.0

                    prev_vol_5m = _sum_vol(c1m, 5, shift=5)   # bars −11 … −6
                    curr_vol_5m = _sum_vol(c1m, 5)            # bars −5  …  0
                    if prev_vol_5m == 0:
                        vol_change_pct = 0.0
                    else:
                        vol_change_pct = (curr_vol_5m - prev_vol_5m) / prev_vol_5m * 100.0

                    squeeze_power = abs(pct_5m) * abs(vol_change_pct)
                    if squeeze_power >= self.squeeze_power_min:
                        offset = SQUEEZE_LIMIT_OFFSET_PCT
                        tick   = float(DEC_TICK)
                        if pct_5m >= self.squeeze_threshold_pct:      # counter‑short
                            side, pos_idx = "Sell", 2
                            limit_price = math.floor((last_price * (1 + offset)) / tick) * tick
                        elif pct_5m <= -self.squeeze_threshold_pct:   # counter‑long
                            side, pos_idx = "Buy", 1
                            limit_price = math.ceil((last_price * (1 - offset)) / tick) * tick
                        else:
                            side = None
                        ...
                        if side:
                            async with self.position_lock:
                                ...
                                if (await self.get_total_open_volume()) + usd_size > self.MAX_TOTAL_VOLUME:
                                    return
                                await self.ensure_symbol_meta(symbol)
                                step   = self.qty_step_map.get(symbol, 0.001)
                                minqty = self.min_qty_map.get(symbol, step)
                                qty    = max(math.floor((usd_size / limit_price) / step) * step, minqty)
                            if qty <= 0:
                                return

                            bid = safe_to_float(ticker.get("bid1Price", 0))
                            ask = safe_to_float(ticker.get("ask1Price", 0))
                            if bid and ask and (ask - bid) / bid * 100 > 0.1:
                                return
                            ...
                            try:
                                if self.mode == "real":
                                    await self.adaptive_squeeze_entry_ws(symbol, side, qty, pos_idx, 15)
                                else:
                                    await self.adaptive_squeeze_entry(symbol, side, qty, 15)
                                self.last_squeeze_ts[symbol] = time.time()
                                logger.info("[SQUEEZE] %s %s %.0f @ %.6f (power=%.1f)",
                                            symbol, side, qty, limit_price, squeeze_power)
                                return
                            except Exception as e:
                                logger.warning("[SQUEEZE] order failed for %s: %s", symbol, e)

            # -----------------------------------------------------------------
            # 2.  L I Q U I D A T I O N   S T R A T E G Y
            # -----------------------------------------------------------------
            mode = getattr(self, "strategy_mode", "full")
            liq_enabled = mode in ("liquidation_only", "full")
            liq_side = ""
            liq_val  = 0.0
            threshold = 0.0  # default so later GS checks always have a defined var

            if liq_enabled:
                liq = self.shared_ws.latest_liquidation.get(symbol, {})
                if liq:
                    liq_val   = safe_to_float(liq.get("value", 0))
                    liq_side  = liq.get("side", "")
                    liq_ts    = liq.get("ts", 0.0)
                    threshold = self.shared_ws.get_liq_threshold(symbol, 5_000)

                    if (time.time() - liq_ts) <= 60 and liq_side in ("Buy", "Sell"):
                        if liq_val >= threshold and self.shared_ws.check_liq_cooldown(symbol):
                            delta_oi = self.shared_ws.get_delta_oi(symbol)
                            if delta_oi is not None and delta_oi <= -0.0005:
                                c1m = self.shared_ws.candles_data.get(symbol, [])
                                vol_now = safe_to_float(c1m[-1]["volume"]) if c1m else 0.0
                                if vol_now >= 0.6 * self.shared_ws.get_avg_volume(symbol, 30):
                                    price = safe_to_float(self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0))
                                    if not price and c1m:
                                        price = safe_to_float(c1m[-1]["closePrice"])
                                    if price <= 0:
                                        return
                                    await self.ensure_symbol_meta(symbol)
                                    step   = self.qty_step_map.get(symbol, 0.001)
                                    minqty = self.min_qty_map.get(symbol, step)
                                    qty    = max(math.floor((self.POSITION_VOLUME / price) / step) * step, minqty)
                                    side_op = "Buy" if liq_side == "Sell" else "Sell"
                                    if self.shared_ws.rsi_blocked(symbol, side_op, 85):
                                        return
                                    pos_idx = 1 if side_op == "Buy" else 2
                                    try:
                                        if self.mode == "real":
                                            await self.place_order_ws(symbol, side_op, qty, pos_idx)
                                        else:
                                            resp = await asyncio.to_thread(lambda: self.session.place_order(
                                                category="linear", symbol=symbol, side=side_op,
                                                orderType="Market", qty=self._format_qty(symbol, qty),
                                                timeInForce="IOC", positionIdx=pos_idx))
                                            if resp.get("retCode", 0) != 0:
                                                raise InvalidRequestError(resp.get("retMsg", "order rejected"))
                                        self.last_liq_trade_time[symbol] = time.time()
                                        logger.info("[LiqTrade] %s %s %.0f (liq %.0f USDT)",
                                                    symbol, side_op, qty, liq_val)
                                        return
                                    except Exception as e:
                                        logger.warning("[LiqTrade] order failed for %s: %s", symbol, e)

            # -----------------------------------------------------------------
            # 3.  G O L D E N   S E T U P
            # -----------------------------------------------------------------
