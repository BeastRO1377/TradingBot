...
class TradingBot:
    ...
    def _ensure_min_order_value_qty(self, symbol: str, qty: float, last_price: float) -> float:
        """
        Поднимает qty так, чтобы стоимость ордера была >= минимального порога биржи (обычно 5 USDT).
        Возвращает скорректированное qty (с учетом шага лота и minQty).
        """
        try:
            min_usdt = float(getattr(self, "min_order_usdt", 5.0))
            est_cost = float(qty) * float(last_price)
            if est_cost >= min_usdt:
                return qty
            step = self.qty_step_map.get(symbol, 0.001) or 0.001
            min_qty = self.min_qty_map.get(symbol, step)
            needed_qty = min_usdt / max(last_price, 1e-8)
            # округляем ВВЕРХ к шагу лота и учитываем min_qty
            ticks = math.ceil((needed_qty / step) - 1e-12)
            adj_qty = max(min_qty, ticks * step)
            return float(f"{adj_qty:.10f}")
        except Exception:
            return qty
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        try:
            ...
            # Гарантируем минимум биржи по стоимости ордера (например, 5 USDT)
            adj_qty = self._ensure_min_order_value_qty(symbol, qty, last_price)
            if adj_qty > qty:
                logger.debug(f"[MIN_ORDER_VALUE_BUMP] {symbol}: qty поднят {qty:.8f} → {adj_qty:.8f} для достижения ≥{getattr(self, 'min_order_usdt', 5.0)} USDT")
                qty = adj_qty
            if not (qty > 0):
            ...
            if source == 'squeeze':
                ...
                if not ok:
                    # тактическое окно истекло — чистим pending для символа и выходим
                    self.pending_orders.pop(symbol, None)
                    self.pending_cids.pop(symbol, None)
                    self.pending_timestamps.pop(symbol, None)
                    ...
                    return
                # тактический вход уже выполнил ордер; завершаем
                return

            # для остальных источников — обычный немедленный вход
            await self.place_unified_order(
