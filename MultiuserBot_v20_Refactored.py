...
def train_golden_model(training_data, num_epochs: int = 30, lr: float = 1e-3):
    """Обучает GoldenNet на PyTorch."""
    ...
    model.scaler = scaler # Прикрепляем скейлер к модели
    return model

def save_model(model, filename="golden_model_v19.pt"):
    """Сохраняет PyTorch модель и скейлер."""
    torch.save(
        {"model_state": model.state_dict(), "scaler": getattr(model, "scaler", None)},
        filename
    )
...
class TradingBot:
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        self.ml_inferencer = PyTorchInferencer("golden_model_v19.pt")
        try:
        ...
        self.ai_timeout_sec = float(user_data.get("ai_timeout_sec", 3.0))
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    async def _unfreeze_guard(self, ttl: float = 45.0):
        """Очищает висящие ключи из _evaluated_signals_cache вне зависимости от формата значения (float | dict)."""
        while True:
            await asyncio.sleep(10)
            try:
                now = time.time()
                stale = [k for k, ts in self._evaluated_signals_cache.items() if now - float(ts) > ttl]
                for k in stale:
                    self._evaluated_signals_cache.pop(k, None)
                if stale:
                    logger.debug(f"[unfreeze_guard] cleared {len(stale)} stale keys")
            except Exception as e:
                logger.error(f"[unfreeze_guard] crashed: {e}", exc_info=True)
    ...
    def load_ml_models(self):
        ...
        candidates = ("golden_model_v19.pt",
                    "golden_model_v19.pt",
                    "trading_model.pth")
...
# ----------------------------------------------------------------------------------------
    ...
    def train_model(self,
                    ...
                    batch_size: int = 64,
                    lr: float = 1e-3,
                    model_path: str = "golden_model_v19.pt"):
        """
    ...
    # ВНУТРИ КЛАССА TRADINGBOT
    # async def _manage_scalp_stop(self, symbol: str, last_price: float):
    #     """
    #     [НОВАЯ ФУНКЦИЯ] Управляет быстрым скальперским стоп-лоссом.
    #     - Активируется при ROI >= 0.5%
    #     - Устанавливает стоп с гэпом в 2.5% от текущего ROI
    #     - Двигается только вверх
    #     """
    #     async with self.position_lock:
    #         pos_data = self.open_positions.get(symbol)
    #         if not pos_data or pos_data.get("stop_in_progress"):
    #             return
    #         pos_data["stop_in_progress"] = True

    #     try:
    #         # 1. Расчет текущего ROI
    #         avg_price = safe_to_float(pos_data["avg_price"])
    #         leverage = safe_to_float(pos_data.get("leverage", 10.0))
    #         is_long = (pos_data["side"] == "Buy")
            
    #         pnl_pct = (((last_price - avg_price) / avg_price) * 100 * leverage) if is_long else (((avg_price - last_price) / avg_price) * 100 * leverage)

    #         # 2. Проверка условия активации трейлинга
    #         if pnl_pct < 0.5:
    #             return

    #         # 3. Расчет целевой цены для стоп-лосса
    #         target_stop_roi = pnl_pct - 2.5  # Гэп в 2.5% от ROI
            
    #         # Конвертируем целевой ROI обратно в цену
    #         if is_long:
    #             # P_stop = P_avg * (1 + (ROI_stop / (100 * L)))
    #             potential_stop_price = avg_price * (1 + (target_stop_roi / (100 * leverage)))
    #         else: # Short
    #             # P_stop = P_avg * (1 - (ROI_stop / (100 * L)))
    #             potential_stop_price = avg_price * (1 - (target_stop_roi / (100 * leverage)))
            
    #         # 4. Проверка и установка стопа
    #         last_known_stop = self.last_stop_price.get(symbol, 0 if is_long else float('inf'))

    #         is_improvement = (is_long and potential_stop_price > last_known_stop) or \
    #                          (not is_long and potential_stop_price < last_known_stop)

    #         if is_improvement:
    #             logger.debug(f"[SCALP_STOP] {symbol}: ROI {pnl_pct:.2f}%. Двигаем стоп на {potential_stop_price:.6f}")
    #             if await self._set_trading_stop_api(symbol, potential_stop_price):
    #                 await self.log_trade(symbol=symbol, side=pos_data["side"], avg_price=avg_price, volume=pos_data['volume'], action="scalp_trailing_set", result="success")

    #     except Exception as e:
    #         logger.error(f"[_manage_scalp_stop] Ошибка для {symbol}: {e}", exc_info=True)
    #     finally:
    #         async with self.position_lock:
    #             if symbol in self.open_positions:
    #                 self.open_positions[symbol].pop("stop_in_progress", None)
    ...
    #     """
    #     [V2 - Обогащенная]
    #     Отправляет отчет о кандидате в OpenAI API с максимально подробным контекстом.
    #     """
    #     from openai import AsyncOpenAI

    #     default_response = {"confidence_score": 0.5, "justification": "Ошибка AI.", "action": "REJECT"}
        
    #     if not self.openai_api_key:
    #         logger.warning("[OpenAI] Ключ API не найден, используется ответ по умолчанию.")
    ...
    #         # --- Получаем контекст по главным экосистемам ---
    #         btc_ticker = self.shared_ws.ticker_data.get("BTCUSDT", {})
    #         eth_ticker = self.shared_ws.ticker_data.get("ETHUSDT", {})
    #         sol_ticker = self.shared_ws.ticker_data.get("SOLUSDT", {})

    #         btc_price_change = safe_to_float(btc_ticker.get("price24hPcnt", 0)) * 100
    #         eth_price_change = safe_to_float(eth_ticker.get("price24hPcnt", 0)) * 100
    #         sol_price_change = safe_to_float(sol_ticker.get("price24hPcnt", 0)) * 100
            
    #         # --- [ИСПРАВЛЕНО] Формируем подробный промпт ВНУТРИ ФУНКЦИИ ---
    #         prompt = f"""
    #         SYSTEM: Ты - элитный квантовый аналитик и риск-менеджер для крипто-трейдинг фонда. Твоя задача - провести глубокий анализ сигнала и вернуть решение в строгом JSON формате.

    #         USER:
    #         Проанализируй следующий торговый сигнал для принятия решения об открытии позиции.

    #         **1. Сигнал:**
    #         - Монета: {candidate['symbol']}
    #         - Направление: {candidate['side'].upper()}
    #         - Источник сигнала: {candidate['source']}

    #         **2. Ключевые метрики сигнала (триггеры):**
    #         - Изменение цены (5m): {_format_metric(candidate['base_metrics'].get('pct_5m'), '.2f')}%
    #         - Изменение объема (1m vs 5m): {_format_metric(candidate['base_metrics'].get('vol_change_pct'), '.1f')}%
    #         - Изменение Open Interest (1m): {_format_metric(candidate['base_metrics'].get('oi_change_pct'), '.2f')}%

    #         **3. Полный рыночный контекст на момент сигнала:**
    #         - Волатильность (Sigma 1h): {_format_metric(features.get('sigma5m', 0) * 100, '.2f')}%
    #         - Спред Bid/Ask: {_format_metric(features.get('spread_pct'), '.3f')}%
    #         - Индекс силы тренда (ADX 14): {_format_metric(features.get('adx14'), '.1f')} (Сила тренда > 25)
    #         - Индекс относительной силы (RSI 14): {_format_metric(features.get('rsi14'), '.1f')} (Зоны: <30 перепроданность, >70 перекупленность)
    #         - Кумулятивная дельта объема (CVD 5m): {_format_metric(features.get('CVD5m'), '.2f')} (Положительное = давление покупателей)

    #         **4. Общий фон рынка:**
    #         - BTC (24h): {_format_metric(btc_price_change, '.2f')}%
    #         - ETH (24h): {_format_metric(eth_price_change, '.2f')}%
    #         - SOL (24h): {_format_metric(sol_price_change, '.2f')}%

    #         **ЗАДАЧА:**
    #         Верни JSON с тремя ключами:
    #         1. "confidence_score": твоя уверенность в успехе сделки от 0.0 до 1.0.
    #         2. "justification": краткое (1-2 предложения) и четкое обоснование твоего решения, основанное на предоставленных данных.
    #         3. "action": твой вердикт. Только "EXECUTE" или "REJECT".
    #         """
    ...
    # Добавьте этот метод в класс TradingBot
    # async def _set_trading_stop_api(self, symbol: str, stop_price: float) -> bool:
    #     """Внутренний helper для установки стоп-лосса через API с указанием объема."""
    #     pos = self.open_positions.get(symbol)
    #     if not pos: 
    #         return False
            
    #     pos_idx = pos.get("pos_idx", 1 if pos['side'] == 'Buy' else 2)
        
    #     # 1. Получаем текущий размер позиции, который нужно закрыть по стопу
    #     position_qty = safe_to_float(pos.get('volume', 0))
    #     if position_qty <= 0:
    #         logger.warning(f"[Risk] Попытка установить стоп для {symbol} с нулевым объемом.")
    #         return False
            
    #     # 2. Форматируем и цену, и ОБЪЕМ для отправки в API
    #     tick_size = self.price_tick_map.get(symbol, 1e-6)
    #     stop_price_str = f"{round(stop_price / tick_size) * tick_size:.8f}".rstrip('0').rstrip('.')
    #     stop_loss_qty_str = self._format_qty(symbol, position_qty)

    #     try:
    #         logger.info(f"[Risk] Установка стоп-лосса для {symbol} на {stop_price_str} (объем: {stop_loss_qty_str})")
            
    #         # 3. Добавляем обязательный параметр slSize в вызов
    #         await asyncio.to_thread(
    #             lambda: self.session.set_trading_stop(
    #                 category="linear", 
    #                 symbol=symbol, 
    #                 positionIdx=pos_idx,
    #                 stopLoss=stop_price_str,
    #                 slSize=stop_loss_qty_str,  # <-- ВОТ ЭТОТ ПАРАМЕТР МЫ ДОБАВИЛИ
    #                 tpslMode="Partial"
    #             )
    #         )
    ...
    #         return True
    #     except InvalidRequestError as e:
    #         if "110041" in str(e) or "110025" in str(e): # Not modified / same tp/sl
    #             logger.debug(f"Стоп для {symbol} уже установлен или не изменен.")
    #             self.last_stop_price[symbol] = stop_price
    #             return True
    #         logger.error(f"Не удалось установить стоп для {symbol}: {e}")
    #         return False
    #     except Exception as e:
    #         logger.error(f"Критическая ошибка установки стопа для {symbol}: {e}")
    #         return False
...
# ─── ОБРАБОТКА POSITION_STREAM ──────────────────────────────────────────────────
    ...
    #     """
    #     Unified handler for Bybit V5 *position* private‑stream events.

    #     ▶  Что изменено относительно версий V16 и V19
    #         • Дедупликация: используем и `seq`, и `updateTime` (на случай, если
    #           биржа пришлёт сообщение с тем же seq, но другим статусом).
    #         • Закрытие: считаем позицию закрытой, когда `size==0`
    #           («status» может быть не только 'closed') и подтверждаем через REST
    #           `confirm_position_closing`.
    #         • Логирование: при любом закрытии пишем в `trades_unified.csv`
    #           (раньше могли пропустить вручную закрытую позицию).
    #         • Поддержка ML‑памяти: `self.active_trade_entries` надёжно очищается,
    #           чтобы не накапливался «мусор» при редких глюках WS.
    #         • Обновление stop‑loss: поле `stopLoss` синхронизируется всегда,
    #           даже если объём не менялся.
    #         • Идёмпотентность: функция безопасна к повторному вызову с тем же
    #           сообщением – состояние не дублируется.

    #     Аргументы
    #     ---------
    #     msg : dict
    #         Сырый WS‑пакет с топиком `position` / `position.linear`.
    #     """
    #     # 0) Приводим `data` к списку ------------------------------
    #     data = msg.get("data", [])
    #     if isinstance(data, dict):
    #         data = [data]

    #     # 1) Оставляем только релевантные события -----------------
    #     filt: list[dict] = []
    #     for p in data:
    #         sym = p.get("symbol")
    #         if not sym:
    #             continue
    #         if safe_to_float(p.get("size", 0)) > 0 or sym in self.open_positions:
    #             filt.append(p)
    ...
    #                 last_key     = self.last_seq.get(symbol, (0, 0))

    #                 # дедупликация (seq, updateTime)
    #                 if (seq, update_time) <= last_key:
    #                     continue
    #                 self.last_seq[symbol] = (seq, update_time)
    ...
    #                 stop_loss    = safe_to_float(p.get("stopLoss", 0))
    #                 open_int     = self.shared_ws.latest_open_interest.get(symbol, 0.0)
    #                 last_price   = safe_to_float(self.shared_ws.ticker_data
    #                                             .get(symbol, {}).get("lastPrice", 0))
    #                 prev         = self.open_positions.get(symbol)

    #                 # ───────────────────── 5. ОТКРЫТИЕ ─────────────────────
    #                 if prev is None and new_size > 0 and side_raw:
    #                     # [ИСПРАВЛЕНО] Удаляем запись из ожидающих, так как позиция подтверждена
    #                     self.pending_orders.pop(symbol, None)

    #                     # [ИСПРАВЛЕНО] Умное определение pos_idx на основе стороны
    #                     correct_pos_idx = 1 if side_raw == 'Buy' else 2
    #                     self.open_positions[symbol] = {
    #                         "avg_price":  avg_price,
    #                         "side":       side_raw,
    #                         "pos_idx":    p.get("positionIdx", correct_pos_idx), # <--- ИСПРАВЛЕНО
    #                         "volume":     new_size,
    #                         "leverage":   safe_to_float(p.get("leverage", "1")), # <-- УБЕДИТЕСЬ, ЧТО ЭТА СТРОКА ЕСТЬ
    #                         "amount":     safe_to_float(p.get("positionValue")),
    #                         "stop_loss":  stop_loss,
    #                         "lastClose":  last_price,
    #                         "open_timestamp": time.time(), # Время открытия позиции
    #                         "max_pnl_pct": 0.0, # Максимальный достигнутый PnL
    #                         "management_flags": {} # Флаги для частичной фиксации

    #                     }
    #                     # ML entry‑cache
    #                     self.active_trade_entries[symbol] = {
    #                         "price":      avg_price,
    #                         "side":       side_raw,
    #                         "volume":     new_size,
    #                         "symbol":     symbol,
    ...
    #                     self.open_positions[symbol]["pnl"] = safe_to_float(p.get("unrealisedPnl", 0))
    #                     self.write_open_positions_json()

    #                     self.ws_opened_symbols.add(symbol)
    #                     self.ws_closed_symbols.discard(symbol)

    #                     logger.info("[PositionStream] NEW %s %s %.3f @ %.6f",
    #                                 side_raw, symbol, new_size, avg_price)
    #                     # fire‑and‑forget PnL evaluation
    #                     asyncio.create_task(self.manage_open_position(symbol))

    #                     # async trade log
    #                     asyncio.create_task(self.log_trade(
    #                         symbol=symbol, side=side_raw, avg_price=avg_price,
    #                         volume=new_size, open_interest=open_int,
    #                         action="open", result="opened"
    #                     ))

    #                     comment = self.pending_strategy_comments.pop(symbol, "")
    #                     note = f"🟢 Открыта {side_raw.upper()} {symbol}: {new_size} @ {avg_price}"
    #                     if comment:
    #                         note += f"\n<i>{comment}</i>"
    #                     asyncio.create_task(self.notify_user(note))

    #                     self.pending_orders.discard(symbol)
    #                     continue

    #                 # ───────────────────── 6. ЗАКРЫТИЕ ─────────────────────
    #                 closed_flag = (new_size == 0) and (status in ("closed", "settled", ""))
    #                 if prev is not None and closed_flag:
    #                     # REST‑подтверждение (иногда WS врёт)
    #                     if not await self.confirm_position_closing(symbol):
    #                         continue

    #                     # ML exit record
    #                     entry = self.active_trade_entries.pop(symbol, None)
    #                     if entry:
    #                         await self.log_trade_for_ml(
    #                             symbol, entry,
    #                             {"price": avg_price,
    #                             "timestamp": datetime.utcnow().isoformat()}
    #                         )
    ...
    #                     asyncio.create_task(self.log_trade(
    #                         symbol=symbol, side=prev["side"],
    #                         avg_price=prev["avg_price"], volume=prev["volume"],
    ...
    #                         "symbol":    symbol,
    #                         "side":      prev["side"],
    #                         "entry_price": prev["avg_price"],
    #                         "exit_price":  avg_price,
    #                         "qty":         prev["volume"],
    #                         "pnl":         safe_to_float(p.get("unrealisedPnl", 0)),
    #                     })

    #                     # уведомление
    #                     asyncio.create_task(self.notify_user(
    #                         f"⏹️ Закрыта {prev['side'].upper()} {symbol}: "
    #                         f"{prev['volume']} @ {avg_price}"
    #                     ))

    #                     self.closed_positions[symbol] = {
    #                         **prev,
    #                         "closed_pnl":  p.get("unrealisedPnl"),
    #                         "closed_time": p.get("updatedTime"),
    #                     }
    ...
    #                 # ─────────────────── 7. ОБНОВЛЕНИЕ ОБЪЁМА ───────────────
    #                 if prev and new_size > 0:
    #                     changed = False
    #                     if new_size != prev.get("volume"):
    #                         logger.info("[PositionStream] %s volume %.3f → %.3f",
    #                                     symbol, prev["volume"], new_size)
    #                         prev["volume"] = new_size
    #                         changed = True
    #                     # стоп‑лосс мог измениться биржей
    #                     if stop_loss and stop_loss != prev.get("stop_loss", 0):
    #                         prev["stop_loss"] = stop_loss
    #                         changed = True
    #                     if changed:
    #                         self.write_open_positions_json()
    #                         # пересчитываем PnL асинхронно
    #                     if changed:
    #                         self.write_open_positions_json()
    #                         # После изменения позиции также запускаем "Хранителя"
    #                         asyncio.create_task(self.manage_open_position(symbol))

    #         except Exception as exc:
    #             logger.error("[handle_position_update] fatal: %s", exc, exc_info=True)
    ...
    #         # ── обновляем in-memory и JSON —
    #         if symbol in self.open_positions:
    #             self.open_positions[symbol]["pnl"] = pnl
    #             # пишем одним атомарным вызовом, чтобы не повредить файл
    #             _atomic_json_write(
    #                 OPEN_POS_JSON,
    #                 {self.user_id: self.open_positions}
    #             )
    ...
    #         # ── rescue monitor (авто-спасение) ─────────────────────────────
    #         try:
    #             await self._rescue_router(symbol, pnl_pct, side)
    #         except Exception as _e:
    #             logger.debug("[rescue_router] %s error: %s", symbol, _e)
    ...
    #             async def _trailing_worker() -> None:
    #                 """
    #                 Запускается фоном через create_task:
    #                 • вызывает set_trailing_stop;
    #                 • при успешной установке пишет лог + обновляет last_trailing_stop_set.
    #                 """
    #                 try:
    ...
    #                 except Exception as exc:
    #                     logger.error("[trailing_worker] %s failed: %s", symbol, exc, exc_info=True)

    #             # запускаем без ожидания, чтобы не блокировать evaluate_position
    #             task = asyncio.create_task(_trailing_worker(), name=f"trailing-{symbol}")
    #             # логируем любые необработанные исключения
    #             task.add_done_callback(
    #                 lambda t: logger.error("[trailing_worker] %s raised: %s", symbol, t.exception(), exc_info=True)
    ...
    # [ИСПРАВЛЕНО] ВНУТРИ КЛАССА TRADINGBOT
    async def evaluate_entry_with_ml(self, candidate: dict, features: dict):
        """
        [V27 - Упрощенный ML-фильтр]
        Использует более мягкий и логичный ML-фильтр перед отправкой сигнала AI.
        """
        ...
        try:
            # --- [НОВЫЙ УПРОЩЕННЫЙ ML-ФИЛЬТР] ---
            try:
                # 1. Получаем прогноз от PyTorch модели
                ...
                # 2. Рассчитываем ожидаемый ROI с плечом
                leverage = safe_to_float(candidate.get("leverage", self.leverage))
                expected_roi = raw_prediction * 100 * leverage

                # 3. Проверяем условия
                # Знак прогноза должен совпадать со стороной сделки
                side_check_ok = (side == "Buy" and expected_roi > 0) or (side == "Sell" and expected_roi < 0)
                # Абсолютное значение ROI должно быть выше порога
                ...
                # В случае ошибки пропускаем сигнал, чтобы не рисковать
                return
            # --- КОНЕЦ ML-ФИЛЬТРА ---
            ...
            logger.info(f"[AI_CONFIRM] Сделка {symbol}/{side} ({source}) ОДОБРЕНА. Переходим к финальным проверкам...")
            ...
            if not await self._risk_check(symbol, side, qty, last_price): return

            logger.info(f"[EXECUTE] {symbol}/{side}: Все проверки пройдены. Отправка ордера...")

            comment_for_log = f"JARVIS' OPINION: {justification}"
            ...
            if source == 'squeeze':
                if self.mode == 'real': await self.adaptive_squeeze_entry_ws(symbol, side, qty, pos_idx)
                else: await self.adaptive_squeeze_entry(symbol, side, qty)
            elif source == 'liquidation':
                await self.adaptive_liquidation_entry(symbol, side, qty, pos_idx)
            else:
                await self.place_unified_order(symbol, side, qty, "Market", comment=comment_for_log)

        except Exception as e:
            logger.error(f"[evaluate_entry] Критическая ошибка для {symbol}: {e}", exc_info=True)
            self.pending_orders.pop(symbol, None)
            self.pending_timestamps.pop(symbol, None)
        finally:
    ...
    async def _squeeze_logic(self, symbol: str) -> bool:
        ...
        try:
            ...
            await self.evaluate_entry_with_ml(candidate, features)
    ...
    #     """Преобразует dict features -> np.array([[...]]), порядок = FEATURE_KEYS."""
    #     return np.array([[safe_to_float(features.get(k, 0.0)) for k in FEATURE_KEYS]],
    #                     dtype=np.float32)

    # def _ml_gate(self, candidate: dict, features: dict) -> tuple[bool, dict]:
    #     """
    #     Жёсткий ML-фильтр перед ИИ:
    #     - знак прогноза должен совпадать со стороной сделки
    #     - |ожидаемый ROI% с плечом| >= ml_gate_abs_roi
    #     - (опц.) динамически поднимаем порог при высокой σ(5m)
    #     Возвращает (True/False, info_dict).
    #     """
    #     symbol = candidate["symbol"]
    #     side   = candidate["side"]  # "Buy" / "Sell"
    #     lev    = safe_to_float(candidate.get("leverage", 10.0) or 10.0)

    #     # 1) Прогноз PyTorch-модели
    #     try:
    #         vec = self._features_to_vector(features)
    #         raw = float(self.ml_inferencer.infer(vec)[0][0])  # см. GoldenNet: target = pnl_pct/100
    #     except Exception as e:
    #         return False, {"reason": f"ml_infer_error: {e}"}

    #     # 2) Сырой скор фильтруем минимально, чтобы не лез мусор (предикт около 0)
    #     if abs(raw) < self.ml_gate_min_score:
    #         return False, {"reason": f"raw<{self.ml_gate_min_score}", "raw": raw}

    #     # 3) Ожидаемый ROI% с учётом плеча (прогноз был в долях процента)
    #     #    targ в тренинге: targ = pnl_pct / 100, значит raw * 100 → %,
    #     #    умножаем на плечо, чтобы приблизить к фактическому ROI.
    #     roi_pct = raw * 100.0 * lev

    #     # 4) Сила порога: базовый + надбавка за волатильность (если включено)
    #     abs_thr = self.ml_gate_abs_roi
    #     if self.ml_gate_sigma_coef > 0.0 and hasattr(self.shared_ws, "_sigma_5m"):
    #         sigma = float(self.shared_ws._sigma_5m(symbol) or 0.0)
    #         abs_thr = abs_thr * (1.0 + self.ml_gate_sigma_coef * sigma * 100.0)

    #     # 5) Сторона/знак
    #     if side == "Buy":
    #         pass_side = roi_pct >= abs_thr
    #     else:  # Sell
    #         pass_side = (-roi_pct) >= abs_thr

    #     if not pass_side:
    #         return False, {"reason": "side_or_threshold", "raw": raw, "roi_pct": roi_pct, "thr": abs_thr}

    #     return True, {"raw": raw, "roi_pct": roi_pct, "thr": abs_thr}

    # # [ИСПРАВЛЕНО] ВНУТРИ КЛАССА TRADINGBOT
    # async def evaluate_entry_with_ml(self, candidate: dict, features: dict, cache_ttl: int = CACHE_TTL_SEC):
    #     symbol = candidate['symbol']
    #     side   = candidate['side']
    #     source = candidate.get('source', 'unknown')

    #     signal_key = self._make_signal_key(symbol, side, source)
    #     now = time.time()
    #     if now < self.ai_circuit_open_until:
    #         # без постановки в кэш, без лог-спама
    #         if now >= self._ai_silent_until:
    #             left = int(self.ai_circuit_open_until - now)
    #             logger.debug(f"[AI_SKIP] {symbol}/{side} ({source}) — circuit open for {left}s")
    #             self._ai_silent_until = now + 5
    #         return False

    #     ts_prev = self._evaluated_signals_cache.get(signal_key, 0.0)
    #     if now - ts_prev < cache_ttl:
    #         return False

    #     # ставим простой float-таймстамп
    #     self._evaluated_signals_cache[signal_key] = now
    #     logger.debug(f"[evaluate_entry] start {signal_key}")

    #     try:
    #         provider = str(self.ai_provider or "ollama").lower().strip().replace('о','o')
    #         ai_response = await self._ai_call_with_timeout(provider, candidate, features)

    #         ai_action     = ai_response.get("action", "REJECT")
    #         justification = ai_response.get("justification", "Причина не указана.")
    #         full_prompt   = ai_response.get("full_prompt_for_ai", "")

    #         if ai_action != "EXECUTE":
    #             logger.info(f"[AI_REJECT] {symbol}/{side} ({source}) — {justification}")
    #             return False

    #         logger.info(f"[AI_CONFIRM] {symbol}/{side} ({source}) ОДОБРЕНА. Переходим к финальным проверкам...")

    #         volume_usdt = candidate.get('volume_usdt', self.POSITION_VOLUME)
    #         last_price  = features.get("price", 0.0)
    #         if last_price <= 0:
    #             logger.warning(f"[CANCEL] {symbol}/{side}: Не валидная last_price={last_price}")
    #             return False

    #         qty = await self._calc_qty_from_usd(symbol, volume_usdt, last_price)
    #         if qty <= 0:
    #             logger.warning(f"[CANCEL] {symbol}/{side}: Нулевой/невалидный объем (qty={qty}).")
    #             return False
    ...
    #         comment_for_log = f"JARVIS' OPINION: {justification}"
    #         candidate['full_prompt_for_ai'] = full_prompt
    #         candidate['comment']            = comment_for_log
    ...
    #         if source == 'squeeze':
    #             if self.mode == 'real':
    #                 await self.adaptive_squeeze_entry_ws(symbol, side, qty, pos_idx)
    #             else:
    #                 await self.adaptive_squeeze_entry(symbol, side, qty)
    #         elif source == 'liquidation':
    #             await self.adaptive_liquidation_entry(symbol, side, qty, pos_idx)
    #         else:
    #             await self.place_unified_order(symbol, side, qty, "Market", comment=comment_for_log)
    ...
    #     finally:
    #         self._evaluated_signals_cache.pop(signal_key, None)
    #         logger.debug(f"[evaluate_entry][finally] cache cleared: {signal_key}")
    ...
    # ) -> bool:
    #     """
    #     Adaptive trailing‑stop manager (v20).

    #     Key points
    #     ----------
    #     • Debounce 1 s to avoid REST flooding.
    #     • Dynamic gap – starts with self.trailing_gap_pct/1000
    #       and narrows as profit grows:
    #           pnl ≥ 8 %  → gap×0.7
    #           pnl ≥ 15 % → gap×0.5
    #     • Gap additionally shrinks 0.5 pct‑points when open‑interest > 1 000.
    #     • Moves the stop only towards profit (never worse).
    #     • Handles Bybit codes 0 / 34040 (OK / not‑modified) and 10001 (“zero position”).
    #     • At most 3 concurrent REST calls via a per‑bot semaphore.
    #     """

    #     # 0 — position still pending?  → skip
    #     if symbol in self.pending_orders:
    #         return False

    #     # 1 — debounce
    #     now = time.time()
    #     last_ts = getattr(self, "_last_trailing_ts", {}).get(symbol, 0.0)
    #     if now - last_ts < 1.0:
    #         return False

    #     # 2 — local position check
    #     pos = self.open_positions.get(symbol)
    #     if not pos or safe_to_float(pos.get("volume", 0)) <= 0:
    #         return False
    #     pos_idx = pos["pos_idx"]          # 1 = long, 2 = short

    #     # 3 — adaptive gap (% of price)
    #     gap_pct = self.trailing_gap_pct         # e.g. 10 → 1 %
    #     if pnl_pct >= 15:
    #         gap_pct *= 0.5
    #     elif pnl_pct >= 8:
    #         gap_pct *= 0.7
    ...
    #     # 5 — new stop price (kept in tick grid)
    #     if side.lower() == "buy":
    #         stop_price = max(avg_price * 1.001,
    #                          last_price * (1 - gap_pct / 1000))
    #     else:
    #         stop_price = min(avg_price * 0.999,
    #                          last_price * (1 + gap_pct / 1000))

    #     tick = float(DEC_TICK) if not isinstance(DEC_TICK, (int, float)) else DEC_TICK
    #     stop_price = round(math.floor(stop_price / tick) * tick, 6)

    #     # 5a — identical or worse? → skip
    #     prev = self.last_stop_price.get(symbol)
    #     if prev is not None:
    #         if abs(prev - stop_price) < tick:
    #             return False
    #         worse = (side.lower() == "buy" and stop_price <= prev) or \
    #                 (side.lower() == "sell" and stop_price >= prev)
    #         if worse:
    #             return False

    #     # 6 — ensure per‑bot semaphore exists
    #     # if not hasattr(self, "_trailing_stop_sem"):
    #     #     self._trailing_stop_sem = asyncio.Semaphore(3)

    #     # 7 — REST call (rate‑limited)
    #     try:
    #         #async with self._trailing_stop_sem:
    #         resp = await asyncio.to_thread(
    #             lambda: self.session.set_trading_stop(
    #                 category="linear",
    #                 symbol=symbol,
    ...
    #         return False
    #     except Exception as exc:
    #         logger.warning("[trailing_stop] %s REST error: %s", symbol, exc)
    #         return False

    #     # 8 — interpret Bybit response
    ...
    #     if ret == 10001 or "zero position" in str(resp.get("retMsg", "")).lower():
    #         self.open_positions.pop(symbol, None)
    #         self.last_stop_price.pop(symbol, None)
    #         return False

    #     logger.warning("[trailing_stop] %s failed: %s", symbol, resp)
    #     return False
    ...
    # # ЗАМЕНИТЕ СТАРЫЙ pnl_loop НА ЭТОТ
    # async def pnl_loop(self):
    #     """Периодически запускает "Хранителя Позиции" для всех открытых сделок."""
    #     while True:
    #         # Создаем копию ключей, чтобы безопасно итерировать, даже если словарь изменится
    #         open_symbols = list(self.open_positions.keys())
            
    #         for symbol in open_symbols:
    #             try:
    #                 # Вызываем новую, умную логику управления позицией
    #                 await self.manage_open_position(symbol)
    #             except Exception as e:
    #                 logger.error(f"[pnl_loop] Ошибка управления позицией {symbol}: {e}", exc_info=True)
            
    #         await asyncio.sleep(2) # Проверяем состояние каждые 2 секунды


    # УБЕДИТЕСЬ, ЧТО ВАШ pnl_loop ВЫГЛЯДИТ ТАК
    # async def pnl_loop(self):
    #     """Периодически запускает "Хранителя Позиции" для всех открытых сделок."""
    #     while True:
        ...
        while True:
            try:
                ...
                if getattr(self, "ml_inferencer", None):
                    ...
                    try:
                        self.ml_inferencer = PyTorchInferencer(CKPT_PATH)
    ...
    for uid, data in all_users.items():
        if not data.get("banned") and data.get("registered"):
            result.append({
                ...
                "gemini_api_key": data.get("gemini_api_key"), # <--- ДОБАВЛЕНА ЭТА СТРОКА
...
#     MODEL_PATH  = "golden_model_v19.pt"
