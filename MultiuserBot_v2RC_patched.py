...
class TradingBot:
    ...
    __slots__ = (
        ...
        "_funding_log_ts", "_funding_log_last", "funding_thr_pos", "funding_thr_neg", "last_entry_comment",
        )
    ...
    # ---------------- AI REVERSAL BROKER ----------------
    def _ensure_ai_rev_workers(self):
        """
        ...
        if not hasattr(self, "_ai_rev_workers"):
            self._ai_rev_workers = []

        # уже запущено?
        current = len(self._ai_rev_workers)
        need = want - current
        if need <= 0:
            return
        ...
        # создаём недостающих
        for _ in range(need):
            task = asyncio.create_task(self._ai_reversal_worker_loop())
            self._ai_rev_workers.append(task)
    ...
    async def manage_open_position(self, symbol: str):
        ...
        while symbol in self.open_positions:
            ...
            if side == "Buy":
                new_stop_price = last_price * (1 - self.trailing_gap_pct / 1000.0)
            else:
                new_stop_price = last_price * (1 + self.trailing_gap_pct / 1000.0)
    ...
    async def _open_position(self, symbol: str, side: str):
        ...
        try:
            ...
            async with self.position_lock:
                ...
                # mark inflight to block duplicates
                self.active_trade_entries[symbol] = {'cid': cid, 'source': source}
        ...
        async with self.position_lock:
            for p in data:
                ...
                if prev_pos is None and new_size > 0:
                    ...
                    # cleanup pending/active markers on materialized open
                    self.pending_orders.pop(symbol, None)
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        started = False          # ставим True в момент «все проверки пройдены»
        ...
        try:
            ...
            if not started:
                ...
                self.pending_timestamps.pop(symbol, None)
                self.active_trade_entries.pop(symbol, None)
                logger.warning("[EXECUTE_EXIT][%s] %s/%s: завершилось без старта (reason=%s)",
                               cid, symbol, side, exit_reason)
            else:
                logger.debug("[EXECUTE_EXIT][%s] %s/%s: started=True — cleanup via WS", cid, symbol, side)
