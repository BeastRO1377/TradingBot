...
# ----------------------------------------------------------------------------------------
    ...
    def _core_decision(self, features: dict, user_cfg: dict) -> tuple[bool, str, str]:
        ...
        if squeeze_state in ("jump", "dive") and squeeze_atten and squeeze_power >= sq_th_pct:
            ...
            return True, side, "core:squeeze_reversal"
        ...
        if liq_dist is not None and liq_side in ("Buy", "Sell"):
            ...
            if liq_dist <= liq_th:
                ...
                return True, side, "core:liq_bounce"
        return False, "", "core:fail"
    ...
    async def evaluate_entry_candidate(self, candidate: dict, features: dict):
        ...
        passed, side_by_core, reason_core = self._core_decision(features, user_cfg)
        if not passed:
            try:
                logger.debug(f"[EVAL_DROP] {symbol} by CORE ({reason_core}) "
                             f"trend={features.get('trend_5m'):.4f} "
                             f"vol_surge={features.get('volume_surge'):.2f} "
                             f"oi_delta={features.get('oi_delta_5m')}")
            except Exception:
                logger.debug(f"[EVAL_DROP] {symbol} by CORE ({reason_core})")
            return
        side = side_by_core or (candidate.get("side") if isinstance(candidate, dict) else None) or features.get("side") or "Buy"

        # вспомогательные признаки — только для контекста ИИ
        aux = {
        ...
        try:
            ...
            logger.exception(f"[AI_ERROR] {symbol}/{side} confirm failed")
            ai_ok = False
        if not ai_ok:
            logger.info(f"[AI_DECLINE] {symbol}/{side} conf={ai_conf:.2f} msg={ai_msg}")
            return
        ...
        if not ok:
            logger.info(f"[RISK_BLOCK] {symbol}/{side} -> {reason_risk}")
            return
