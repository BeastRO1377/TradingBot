...
class TradingBot:
    ...
    __slots__ = (
        ...
        "squeeze_ai_timeout_sec", "ENABLE_POSITION_FALLBACK_CLOSE",
        )
    ...
    async def _read_stop_events(self, symbol: str, proc):
        ...
        Добавлена финальная проверка существования позиции перед отправкой ордера.
        """
        import json
        try:
            while True:
                ...
                if et in ("activated", "trail_update", "set_stop", "stop_update"):
                    ...
                    if price > 0:
                        ...
                        # проверяем, а есть ли у нас еще эта позиция?
                        if symbol not in self.open_positions:
                            logger.warning(f"[Guardian] {symbol}: Получена команда на обновление стопа, но позиция уже закрыта. Исполнение отменено.")
                            continue # Пропускаем итерацию, не отправляем запрос
                        # <<< КОНЕЦ ФИНАЛЬНОГО ИСПРАВЛЕНИЯ >>>
                            
                        try:
                            await self.set_or_amend_stop_loss(symbol, price)
                            # Логируем успех только после успешного вызова API
                            logger.info("✅ [Guardian] %s стоп обновлен на %.10f", symbol, price)
                        except Exception as e:
                            # set_or_amend_stop_loss уже имеет подробное логирование ошибок
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        async with lock:
            ...
            # Тактика: если есть «окно», пусть оно логирует истечение
            if candidate.get("source") == "squeeze" and hasattr(self, "adaptive_squeeze_entry"):
                ok = await self.adaptive_squeeze_entry(
                    symbol=symbol, side=side, qty=qty, last_price=last_price,
                    ai_preconfirmed=True, max_entry_timeout=float(getattr(self, "tactical_entry_window_sec", 45.0)),
                    cid=cid
                )
                if not ok:
                    logger.warning(f"[EXECUTE_CANCEL][{cid}] {symbol}/{side}: SQUEEZE window expired/no fill")
                return

            # Прямой маркет через единый «ворот»
            await self.place_unified_order(
                symbol=symbol, side=side, qty=qty, order_type="Market",
                comment=f"entry_by_{source}", cid=cid
            )
    ...
    async def _process_signal(self, candidate: dict, features: dict, signal_key: tuple):
        ...
        try:
            ...
            # <<< ИЗМЕНЕНИЕ: ГАРАНТИРОВАННОЕ СНЯТИЕ БЛОКИРОВКИ >>>
            self.active_signals.discard(signal_key)
            symbol = candidate.get("symbol")
            if symbol:
                # Накладываем короткий кулдаун, чтобы не входить в ту же ситуацию сразу
                self.strategy_cooldown_until[symbol] = time.time() + 60 # 1 минута
...
# --- SQUEEZE FILTER (REAL) ---------------------------------------------------
    ...
    async def _squeeze_logic(self, symbol: str) -> bool:
            ...
            try:
                ...
                ret_5m = float(features.get("ret_5m", 0.0))
                atr_5m = float(features.get("atr_5m", 0.0))
                price = float(features.get("price", 0.0))
                if price <= 0: return False

                thr_base = float(getattr(self, "squeeze_min_abs_move", 0.03))
                ...
                signal_key_on_error = (symbol, "Sell" if features.get("ret_5m", 0) > 0 else "Buy", 'squeeze')
                self.active_signals.discard(signal_key_on_error)
                return False
    ...
    #         args["price"] = str(price)
    #     if cid:
    #         args["orderLinkId"] = cid  # важно для корреляции и защиты от дублей

    #     req_id = f"oc:{cid or symbol}:{int(time.time()*1000)}"
    #     req = {"op": "order.create", "header": header, "args": [args], "req_id": req_id}

    #     for attempt in range(2):
    #         try:
    #             async with self._recv_lock:
    #                 await self.ws_trade.send(json.dumps(req))
    #                 logger.info("[ORDER][%s] send %s", cid or "-", {"req_id": req_id, "args": args})
    #                 resp = await self._recv_until("order.create", timeout=max(3.0, recv_timeout))

    #             rc = resp.get("retCode", resp.get("ret_code", 0))
    #             msg = resp.get("retMsg") or resp.get("ret_msg") or ""
    #             logger.info("[ORDER][%s] ack rc=%s msg=%s", cid or "-", rc, msg)

    #             if rc != 0:
    #                 raise RuntimeError(f"order.create rejected: rc={rc} msg={msg}")

    #             return resp.get("data", resp)

    #         except asyncio.TimeoutError:
    #             logger.error("[place_order_ws][%s] ack timeout after %.1fs (req_id=%s)", cid or "-", max(3.0, recv_timeout), req_id)
    #             raise

    #         except (websockets.ConnectionClosed, asyncio.IncompleteReadError) as e:
    #             logger.warning("[place_order_ws] WS closed, reconnecting (attempt %d/2): %s", attempt + 1, e)
    #             await self.init_trade_ws()

    #         except Exception as e:
    #             logger.error("[place_order_ws][%s] Unexpected error: %s", cid or "-", e, exc_info=True)
    #             raise
    ...
    async def log_trade(
        ...
        *,
        ...
        # # (1) то, что мы положили при размещении ордера
        # comment = self.pending_strategy_comments.pop(symbol, None)

        # # (2) если не успели — возьмём из активного кандидата
        # if not comment:
        #     ent = (self.active_trade_entries.get(symbol) or {}) if hasattr(self, "active_trade_entries") else {}
        #     comment = ent.get("__opinion_comment") or ent.get("comment")
        #     # последний шанс: собрать из justification
        #     if not comment:
        #         just = (ent.get("justification") or "").strip()
        #         if just:
        #             comment = f"JARVIS' OPINION: {just}"

        # # (3) подстраховка: если пусто — не падаем, просто без мнения
        # if not comment:
        #     logger.warning("[TELEGRAM] %s: OPINION пуст — отправляю без него", symbol)
        # # --- /Opinion recovery ---
