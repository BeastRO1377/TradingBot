...
class PublicWebSocketManager:
    ...
    async def place_order_ws(self, symbol, side, qty,
        ...
        if not self.ws_trade:
            logger.warning("[place_order_ws] ws_trade is None, waiting for init...")
            for _ in range(10):
                await asyncio.sleep(0.3)
                if self.ws_trade:
                    break
            if not self.ws_trade:
                raise RuntimeError("Trade WebSocket not initialized")
...
class MLInferencer:
    def __init__(self, model_path: str = None):
        ...
        self.model = self._build_default()
        self.model.to(self.device)
        self.model.eval()
        self.model_path = model_path or "golden_model_v19.pt"
    ...
    def _build_default(self):
        """Default tiny 3‑layer classifier (works when no checkpoint)."""
    ...
    def _load_model(self, path: str):
        """
        Load checkpoint *path* and build matching architecture on‑the‑fly.

        • If state_dict contains fc1 / bn1 → assume GoldenNet.
        • Else fall back to default tiny Sequential model.
        """
        ckpt = torch.load(path, map_location=self.device)
        state = ckpt["model_state"]

        # --- choose architecture --------------------------------------
        if any(k.startswith("fc1") for k in state.keys()):
            self.model = GoldenNet(input_size=self.input_dim).to(self.device)
            logger.info("[ML] checkpoint recognised as GoldenNet")
        else:
            self.model = self._build_default().to(self.device)
            logger.info("[ML] checkpoint recognised as 3‑layer classifier")
...
class TradingBot:
    ...
    __slots__ = (
        ...
        "_periodic_eval_task", "_best_entries", "_best_entry_seen", "_best_entry_cache",
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        self.ml_inferencer = MLInferencer("golden_model_v19.pt")
        try:
...
# ─── ОБРАБОТКА POSITION_STREAM ──────────────────────────────────────────────────
            ...
            for p in filt:
                ...
                if prev is None and new_size > 0 and side_raw:
                    ...
                    asyncio.create_task(self.evaluate_position({
                        "symbol": symbol, "size": str(new_size), "side": side_raw
                    }))
    ...
    async def _average_down(self, symbol: str, side: str) -> None:
        """
        Ставит лимит-ордера «по тренду» для усреднения.
        • qty = 30 % от исходного объёма
        • цена — на 0.4 % лучше текущего bid/ask
        """
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 4. HEDGE: противоположная позиция
    # ──────────────────────────────────────────────────────────────────────
    async def _open_hedge(self, symbol: str, side: str) -> None:
        """
        Открываем меньшую (60 %) позицию в противоположную сторону.
        """
        pos = self.open_positions.get(symbol, {})
        base_qty = safe_to_float(pos.get("volume", 0))
        if base_qty == 0:
            return
        hedge_qty = round(base_qty)
        opp_side  = "Sell" if side == "Buy" else "Buy"
        idx       = 2 if opp_side == "Sell" else 1
        try:
            await self.place_order_ws(symbol, opp_side, hedge_qty,
                                    position_idx=idx,
                                    order_type="Market")
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 5. Hard-stop (при глубокой просадке)
    # ──────────────────────────────────────────────────────────────────────
    async def _hard_stop(self, symbol: str) -> None:
        ...
        if not pos:
            return
        stop_qty  = round(pos["volume"] * 0,3, 3)
        side = "Sell" if pos["side"] == "Buy" else "Buy"
        idx  = 2 if side == "Sell" else 1
        try:
            await self.place_order_ws(symbol, side, stop_qty, position_idx=idx,
                                    order_type="Market")
    ...
    async def _risk_check(self, symbol: str, side: str, qty: float, last_price: float) -> bool:
        ...
        if existing:
            ...
            if existing_side and existing_side != side.lower():
                logger.info("[risk] Skip %s — open %s, tried %s",
                            symbol, existing_side, side.lower())
                return False

            # limit averaging
            if existing_side == side.lower() and existing_vol > 0:
                if existing_vol * last_price > self.POSITION_VOLUME * MAX_ACTIVE_SAME_SIDE:
                    logger.info("[risk] Skip %s — already have %.0f USDT %s exposure",
                                symbol, existing_vol * last_price, side)
                    return False
        else:
    ...
    async def _squeeze_logic(self, symbol: str) -> bool:
        ...
        if len(recent_deque) < 6:
            return False                         # need ≥5 completed candles

        recent = list(recent_deque)
        ...
        if pct_5m >= 0 and pct_5m < thr_price:
            return False
        if pct_5m < 0 and pct_5m > -thr_price:
            return False
        if squeeze_power < power_min:
            # ΔOI‑sign gate temporarily disabled (will be used only by ML)
            # if d_oi is not None and math.copysign(1, d_oi) != math.copysign(1, pct_5m):
            #     return False
            return False

        # extra short filter
        if pct_5m < 0 and vol_change_pct < power_min:
            return False    # ΔOI magnitude check removed
        ...
        async with self.position_lock:
            ...
            total = await self.get_total_open_volume()
            if total + usd_size > self.MAX_TOTAL_VOLUME:
                return False
            await self.ensure_symbol_meta(symbol)
            qty = self._calc_qty_from_usd(symbol, usd_size, last_price)

        if qty <= 0:
            return False

        # log best squeeze ML/signal candidate
        sq_prob = squeeze_power            # crude proxy, replace if ML prob exists
        self._record_best_entry(symbol, "squeeze", side, sq_prob, {
            "pct_5m": pct_5m,
            "vol_change_pct": vol_change_pct,
            "squeeze_power": squeeze_power
        })
        # reserve + schedule adaptive entry
        ...
        if self.mode == "real":
            asyncio.create_task(self.adaptive_entry_ws(
                ...
                position_idx=position_idx, max_entry_timeout=60))
        else:
            asyncio.create_task(self.adaptive_entry(
                symbol=symbol, side=side, qty=qty, max_entry_timeout=60))
        return True           # order scheduled
    ...
    async def _liquidation_logic(self, symbol: str) -> bool:
        ...
        if (time.time() - liq_ts) > 60:
            return False

        threshold = self.shared_ws.get_liq_threshold(symbol, 5000)
        if liq_val < threshold or liq_side not in ("Buy", "Sell"):
            return False

        candles = self.shared_ws.candles_data.get(symbol, [])
        volume_now = safe_to_float(candles[-1].get("volume", 0)) if candles else 0.0
        avg_vol_30m = self.shared_ws.get_avg_volume(symbol, 30)
        if volume_now < VOLUME_COEF * 0.8 * avg_vol_30m:
            return False

        if not self.shared_ws.funding_cool(symbol):
            return False

        delta_oi = self.shared_ws.get_delta_oi(symbol)
        thr = -0.5 * self._oi_sigma.get(symbol, 0.003)
        # ΔOI threshold temporarily disabled
        # if delta_oi is None or delta_oi > thr:
        #     return False

        vol_hist = list(self.shared_ws.volume_history.get(symbol, []))
        if len(vol_hist) < 2 or (vol_hist[-1] - vol_hist[-2]) <= 0:
            return False

        if not self.shared_ws.check_liq_cooldown(symbol):
            return False
        ...
        total = await self.get_total_open_volume()
        if total + self.POSITION_VOLUME > self.MAX_TOTAL_VOLUME:
            return False

        close_price = safe_to_float(
            self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0)
        ) or safe_to_float(self.shared_ws.candles_data.get(symbol, [])[-1]["closePrice"])
        if close_price <= 0:
            return False
    ...
    {}
            ...
            # ML/liquidation best‑entry log
            liq_prob = liq_val              # use USDT value as proxy score
            self._record_best_entry(symbol, "liquidation", opposite, liq_prob, {
                "liq_value": liq_val,
                "delta_oi": delta_oi or 0.0
            })
            self.pending_orders.add(symbol)
            self.pending_timestamps[symbol] = time.time()
            return True
    ...
    async def _golden_logic(self, symbol: str):
        ...
        try:
            ...
            spread_pct = (ask_px - bid_px) / bid_px * 100 if bid_px > 0 else 0.0
            # ------------------------------------------------------------------
            # Grab latest liquidation info once, so the variables exist later
            # even if we skip the dedicated liquidation helper.
            # ------------------------------------------------------------------
            liq_info = self.shared_ws.latest_liquidation.get(symbol, {})
            liq_val  = safe_to_float(liq_info.get("value", 0))
            liq_side = liq_info.get("side", "")
            ...
            # --- composite signal strength (auto-weighted) ---
            w = GOLDEN_WEIGHTS
            signal_strength = (
                w["price_change"]  * abs(price_change_pct)
                + w["volume_change"] * abs(volume_change_pct)
                + w["oi_change"]     * abs(float(oi_change_pct))
            )
            ...
            if len(recent) > sp:
                ...
                logger.debug("[Golden skip] %s pchg=%.2f vol=%.1f oi=%.2f  (need ≥ %.2f / %.1f / %.2f)",
                            symbol, float(pchg), float(volchg), float(oichg),
                            p_thr, v_thr, oi_thr)

                logger.debug(
                    "[Golden SELL-probe] %s ΔP=%.2f%% ΔV=%.1f%% ΔOI=%.2f%% ΔCVD=%.1f%% "
                    "(need ≤ %.2f / ≥ %.1f / ≥ %.2f / CVD -50…-18.3)",
                    symbol, float(pchg), float(volchg), float(oichg), cvd_change_pct,
                    -float(sell_params["price_change"]),      # порог отрицательный
                    float(sell_params["volume_change"]),
                    float(sell_params["oi_change"]),
                )
                ...
                if action is None:
                    ...
                    if len(recent) > lp:
                        ...
                        pchgb = (
                            (Decimal(str(newb["closePrice"])) - Decimal(str(oldb["closePrice"])))
                            / Decimal(str(oldb["closePrice"])) * Decimal("100")
                            if oldb["closePrice"] else Decimal("0")
                        )
                        volb = (
                            (Decimal(str(vol_hist_5m[-1])) - Decimal(str(vol_hist_5m[-1 - lp])))
                            / Decimal(str(vol_hist_5m[-1 - lp])) * Decimal("100")
                            if vol_hist_5m[-1 - lp] else Decimal("0")
                        )
                        oib = (
                            (Decimal(str(oi_hist_5m[-1])) - Decimal(str(oi_hist_5m[-1 - lp])))
                            / Decimal(str(oi_hist_5m[-1 - lp])) * Decimal("100")
                            if oi_hist_5m[-1 - lp] else Decimal("0")
                        )
                ...
                try:
                    ...
                    if feats:
                        ...
                        X = _np.array([[feats.get(k, 0.0) for k in FEATURE_KEYS]],
                                      dtype=_np.float32)
                        probs  = self.ml_inferencer.infer(X)[0]      # [Sell, Hold, Buy]
                        ...
                        if conf < ML_CONFIDENCE_FLOOR:
                            logger.info("[Golden ML veto] %s: low conf %.2f — skip", symbol, conf)
                            return

                        # explicit HOLD veto
                        if ml_sig is None:
                            logger.info("[Golden ML veto] %s: model = HOLD", symbol)
                            return

                        # direction override
                        if ml_sig != action:
                            logger.info("[Golden ML override] %s: %s → %s  (p=%.2f)",
                                        symbol, action, ml_sig, conf)
                            action = ml_sig
                except Exception as _e:
            ...
            # --- choose deltas actually used for the triggered side ---------
            if action == "Sell":
                snap_price_change  = float(pchg)
                snap_volume_change = float(volchg)
                snap_oi_change     = float(oichg)
            elif action == "Buy":
                snap_price_change  = float(pchgb)
                snap_volume_change = float(volb)
                snap_oi_change     = float(oib)
            else:
                snap_price_change  = price_change_pct
                snap_volume_change = volume_change_pct
                snap_oi_change     = float(oi_change_pct)
            # 6. Логирование снимка
            snapshot_row = ({
                "close_price":    safe_to_float(close_price),
                "price_change":   snap_price_change,
                "volume_change":  snap_volume_change,
                "oi_change":      snap_oi_change,
    ...
    async def adaptive_squeeze_entry_ws(self,
                                        ...
                                        qty: float,
                                        position_idx: int,
                                        max_entry_timeout: int = 120) -> bool:
        """
        Пост-only лимитник, который «прыгает» за ценой во время сквиза.

        • при SHORT (Sell) ставим выше ask;  
        • при LONG  (Buy)  ставим ниже bid.
        """
        if symbol in self.open_positions:
            logger.info("[adaptive_ws] %s already open – skip", symbol)
            return False

        # ── meta (tick/minQty) ────────────────────────────────────────
        await self.ensure_symbol_meta(symbol)
        tick      = safe_to_float(self.price_tick_map.get(symbol, DEC_TICK))
        qty_step  = self.qty_step_map.get(symbol, tick)
        min_qty   = self.min_qty_map.get(symbol, qty_step)
        offset_pct   = SQUEEZE_LIMIT_OFFSET_PCT / 1000.0   # 0.5 %  →  0.0005
        reprice_int  = SQUEEZE_REPRICE_INTERVAL
        t0           = asyncio.get_running_loop().time()
        order_id     = None
        success      = False

        def _limit(bid: float, ask: float) -> float:
            if side.lower() == "buy":
                raw = bid * (1 - offset_pct)
                return math.floor(raw / tick) * tick
            raw = ask * (1 + offset_pct)
            return math.ceil(raw / tick) * tick

        try:
            while True:
                tk   = self.shared_ws.ticker_data.get(symbol, {})
                bid  = safe_to_float(tk.get("bid1Price", 0))
                ask  = safe_to_float(tk.get("ask1Price", 0))
                last = safe_to_float(tk.get("lastPrice", 0))
                if bid == 0 or ask == 0:
                    await asyncio.sleep(reprice_int)
                    continue

                if not await self._risk_check(symbol, side, qty, last):
                    break

                # abort if qty falls below minQty
                if qty < min_qty:
                    logger.info("[adaptive_ws] %s qty %.8f < min_qty %.8f – abort",
                                symbol, qty, min_qty)
                    break
                ...
                # ----------  place  ----------
                if order_id is None:
                    # отправляем new-order
                    resp = await self.ws_private.place_order_ws(
                        category="linear", symbol=symbol,
                        side=side.capitalize(), orderType="Limit",
                        qty=self._format_qty(symbol, qty),
                        price=str(px), timeInForce="PostOnly",
                        reduceOnly=False, positionIdx=position_idx
                    )
                    rc  = resp.get("retCode", 0)
                    msg = str(resp.get("retMsg", "")).lower()

                    if rc == 10001 and "qty invalid" in msg:
                        qty = max(qty - qty_step, 0)          # уменьшаем
                        continue
                    if rc != 0:
                        logger.warning("[adaptive_ws] %s order err: %s", symbol, resp)
                        break

                    order_id = resp["orderId"]
                    logger.info("[adaptive_ws] %s new %s @ %.6f", symbol, order_id, px)

                # ----------  amend  ----------
                else:
                    try:
                        await self.ws_private.amend_order_ws(
                            symbol=symbol, order_id=order_id, new_price=px
                        )
                    except RuntimeError as exc:
                        m = str(exc).lower()
                        if "order not exist" in m:
                            order_id = None          # fill / cancel → ставим заново
                            continue
                        if "order not modified" in m:
                            pass                     # ничего страшного
                        else:
                            logger.warning("[adaptive_ws] %s amend err: %s", symbol, exc)
                            break

                # ----------  wait / timeout  ----------
                await asyncio.sleep(reprice_int)
                elapsed = asyncio.get_running_loop().time() - t0
                if elapsed >= max_entry_timeout:
                    logger.info("[adaptive_ws] %s timeout %.0fs", symbol, elapsed)
                    break
                if int(elapsed) % 10 == 0:
                    offset_pct = max(0.001, offset_pct * 0.8)   # к 0.1 %

                # ----------  filled?  ----------
                pos = self.open_positions.get(symbol)
                if pos and safe_to_float(pos.get("volume", 0)) >= qty:
                    success = True
                    logger.info("[adaptive_ws] %s filled", symbol)
                    break

        except Exception as exc:
            logger.exception("[adaptive_ws] %s fatal: %s", symbol, exc)

        finally:
            if not success and order_id:
                try:
                    await self.cancel_order_ws(symbol, order_id)
                    logger.info("[adaptive_ws] %s cancel %s", symbol, order_id)
                except Exception as exc:
                    logger.debug("[adaptive_ws] %s final-cancel err: %s", symbol, exc)
    ...
    async def adaptive_squeeze_entry(
        ...
        side: str,
        qty: float,
        max_entry_timeout: int = 120,
    ) -> bool:
        """
        Adaptive PostOnly-лимит для сквиза   (REST / demo).

        Логика идентична WS-версии, но через REST-эндпоинты.
        """
        price_tick  = self.price_tick_map.get(symbol, DEC_TICK)
        offset_pct  = SQUEEZE_LIMIT_OFFSET_PCT / 100.0        # 0.5 % → 0.005
        start_ts    = time.time()
        order_id    = None
        success     = False
        ...
        def quote() -> float:
            ...
            if bid == 0 or ask == 0:
                return 0.0
            if side.lower() == "buy":
                raw = bid * (1 - offset_pct)                  # LONG → ниже рынка
                return math.floor(raw / price_tick) * price_tick
            raw = ask * (1 + offset_pct)                      # SHORT → выше рынка
            return math.ceil(raw / price_tick) * price_tick

        try:
            cur_px = quote()
            if cur_px == 0:
                logger.info("[squeeze] %s no quote – abort", symbol)
                return False
            ...
            while True:
                ...
                if order_id is None:
                    resp = await asyncio.to_thread(lambda: self.session.place_order(
                        ...
                        orderType="Limit", qty=f"{qty}",
                        price=f"{cur_px:.6f}", timeInForce="PostOnly",
                    ...
                    logger.info("[squeeze] %s limit %s @ %.6f", symbol, order_id, cur_px)
                else:
                    try:
                        await asyncio.to_thread(lambda: self.session.replace_order(
                            category="linear", symbol=symbol,
                            orderId=order_id, price=f"{cur_px:.6f}"
                        ))
                        logger.debug("[squeeze] %s amend → %.6f", symbol, cur_px)
                    except InvalidRequestError as e:
                        if "order not exist" in str(e).lower():
                            order_id = None
                            continue
                ...
                # 2. pause / timeout --------------------------------------------
                await asyncio.sleep(SQUEEZE_REPRICE_INTERVAL)
                elapsed = time.time() - start_ts
                if elapsed >= max_entry_timeout:
                    logger.info("[squeeze] %s timeout (%.0fs)", symbol, elapsed)
                    break
                if int(elapsed) % 10 == 0 and offset_pct > 0.001:
                    offset_pct = max(0.001, offset_pct * 0.8)

                # 3. status ------------------------------------------------------
                if order_id:
                    status = await asyncio.to_thread(lambda: self.session.get_orders(
                        category="linear", symbol=symbol, orderId=order_id
                    ))
                    state = status["result"]["list"][0].get("orderStatus", "").lower()
                    if state in ("filled", "partiallyfilled", "partially_filled"):
                        success = True
                        logger.info("[squeeze] %s %s", symbol, state)
                        break
            ...
            logger.warning("[squeeze] %s error: %s", symbol, e)

        finally:
            if not success and order_id:
                try:
                    await asyncio.to_thread(lambda: self.session.cancel_order(
                        category="linear", symbol=symbol, orderId=order_id
                    ))
                    logger.info("[squeeze] %s cancel %s", symbol, order_id)
                except Exception as e:
                    logger.debug("[squeeze] final cancel %s: %s", symbol, e)
            self.pending_orders.discard(symbol)
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  POLAR-LOG: запись сделок + Telegram-уведомления
    # ----------------------------------------------------------------------
    async def log_trade(
        self,
        symbol: str,
        *,
        ...
        action: str,                # "open" | "close" | "trailing_set" …
        result: str,                # "opened" | "closed" | …
        closed_manually: bool = False,
        row: dict | list | pd.DataFrame | None = None,
        csv_filename: str = "trade_log_MUWS.log",
    ) -> None:
        """
        Централизованный логгер всех сделок:

        • пишет компактный CSV-журнал (`trade_log_MUWS.log`);
        • пополняет `trades_for_training.csv` (+ снапшоты для ML);
        • обновляет `trades_history.json` (последние 5k записей);
        • шлёт Telegram-уведомление пользователю.

        Вызов синхронный снаружи, но «тяжёлые» операции выполняются в
        `asyncio.to_thread`, чтобы не блокировать event-loop.
        """

        # ---- 0. базовая валидация -----------------------------------------
        if not symbol:
            logger.warning("[log_trade] called without symbol (row=%s)", row)
            symbol = "UNKNOWN"
        ...
        # ---- 1. нормализуем `row` (может прилететь чем угодно) ------------
        if row is None:
            row = {}
        elif isinstance(row, list) and row:               # [dict, …]
            row = row[-1]
        elif hasattr(row, "iloc"):                        # pandas-DataFrame
            row = row.iloc[-1].to_dict()

        time_str = (
            row.get("startTime", dt.datetime.utcnow())
            if isinstance(row.get("startTime"), dt.datetime)
            else dt.datetime.utcnow()
        ).strftime("%Y-%m-%d %H:%M:%S")

        # ---- 2. INFO-лог в консоль/файл ------------------------------------
        logger.info(
            "[log_trade] user=%s %s position %s: side=%s avg_price=%s volume=%s result=%s",
            self.user_id, action.upper(), symbol, side, avg_price, volume, result
        )

        # ---- 3. компактный CSV-журнал --------------------------------------
        def _csv_writer() -> None:
            fn = Path(csv_filename)
            first = not fn.exists()
            with fn.open("a", newline="", encoding="utf-8") as fp:
                w = csv.writer(fp)
                if first:
                    w.writerow([
                        "user_id", "symbol", "timestamp", "side",
                        "entry_price", "volume", "open_interest",
                        "action", "result", "closed_manually"
                    ])
                w.writerow([
                    self.user_id, symbol, time_str, side,
                    str(avg_price), str(volume), str(open_interest or ""),
                    action, result, "manual" if closed_manually else "signal"
                ])

        await asyncio.to_thread(_csv_writer)

        # ---- 4. trades_for_training.csv (+ snapshot-данные) ----------------
        try:
            tf_path = Path("trades_for_training.csv")
            first   = not tf_path.exists()
            snap    = self.golden_param_store.get("last_snapshot", {}).get(symbol, {})
            close_px = safe_to_float(
                self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0)
            )
            # PnL% ×10 (1 = 0.1 %)   — оставляем как было
            try:
                direction = 1 if side.lower() == "buy" else -1
                pnl_pct   = (
                    (close_px - float(avg_price)) / float(avg_price) * 1000 * direction
                    if avg_price else ""
                )
            except ZeroDivisionError:
                pnl_pct = ""

            with tf_path.open("a", newline="", encoding="utf-8") as fp:
                w = csv.writer(fp)
                if first:
                    w.writerow([
                        "symbol", "datetime", "avg_price", "volume",
                        "open_interest", "side", "event", "pnl_pct",
                        "close_price",
                        # snapshot-fields ↓↓↓
                        "price_change", "volume_change", "oi_change",
                        "period_iters", "user_id", "signal"
                ...
                w.writerow([
                    symbol, datetime.utcnow().isoformat(), avg_price, volume,
                    open_interest or "", side, action, pnl_pct, close_px,
                    snap.get("price_change", ""), snap.get("volume_change", ""),
                    snap.get("oi_change", ""),    snap.get("period_iters", ""),
                    snap.get("user_id", ""),      snap.get("signal", ""),
                ])
        except Exception as exc:
            logger.warning("[log_trade] write trades_for_training.csv error: %s", exc)

        # ---- 5. trades_history.json (последние 5 000 строк) ----------------
        try:
            hist_path = Path(TRADES_JSON)
            hist = []
            if hist_path.exists():
                with hist_path.open("r", encoding="utf-8") as fp:
                    hist = json.load(fp) or []
            hist.append({
                "timestamp": datetime.utcnow().isoformat(),
                "symbol":    symbol,
            ...
            _atomic_json_write(hist_path, hist[-5000:])
        except Exception:
            pass

        # ---- 6. trades_unified.csv (короткий глобальный) -------------------
        _append_trades_unified({
            "timestamp": datetime.utcnow().isoformat(),
            "symbol":    symbol,
            "side":      side,
        ...
        if self.load_user_state().get("quiet_mode", False):
            return

        link = f"https://www.bybit.com/trade/usdt/{symbol}"
        closed_txt = "вручную" if closed_manually else "по сигналу"

        # шаблоны сообщений
        if action == "open":
            icon = "🟩" if side == "Buy" else "🟥"
            title = "Открытие ЛОНГ-позиции" if side == "Buy" else "Открытие SHORT-позиции"
            msg = (
                f"{icon} <b>{title}</b>\n"
                f"<b>Символ:</b> <a href=\"{link}\">{symbol}</a>\n"
                f"<b>Цена входа:</b> {avg_price}\n"
                f"<b>Объём:</b> {volume}\n"
                f"<b>Время:</b> {time_str}\n"
                f"# {symbol}"
            )
            ...
            msg = (
                ...
                f"<b>Цена закрытия:</b> {avg_price}\n"
                f"<b>Объём:</b> {volume}\n"
                f"<b>Тип закрытия:</b> {closed_txt}\n"
                f"<b>Время:</b> {time_str}\n"
                f"# {symbol}"
            )
        elif action.startswith("trailing"):
            # ── расчёт «живого» PnL для уведомления ──────────────────
            entry_price  = safe_to_float(avg_price)
            last_price   = safe_to_float(
                self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice", 0)
            )
            ...
            pnl_val      = (last_price - entry_price) * vol * direction
            try:
                pnl_pct_val = ((last_price - entry_price) / entry_price * 1000 * direction)
            except ZeroDivisionError:
                pnl_pct_val = 0.0
        ...
        try:
            ...
            logger.debug("[log_trade] telegram send error: %s", exc)
        ...
        while True:
            try:
                ...
                if getattr(self, "ml_inferencer", None):
                    ...
                    try:
                        self.ml_inferencer = MLInferencer(CKPT_PATH)
