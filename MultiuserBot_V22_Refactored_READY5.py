...
class TradingBot:
    ...
    def __init__(self, user_data, shared_ws, golden_param_store):
        ...
        self.ai_model = (self.user_data.get("ai_model") or "llama3.1:8b").strip()
        # используем OpenAI-совместимый эндпоинт, как в ваших логах
        self.ai_endpoint = (self.user_data.get("ai_endpoint") or "http://localhost:11434/v1/chat/completions").strip()
        self.ai_temperature = float(self.user_data.get("ai_temperature", 0.15))
...
# ----------------------------------------------------------------------------------------
    ...
    async def _risk_checks_before_entry(self, symbol: str, side: str, features: dict | None = None):
        """Тонкая обёртка для evaluate_entry_candidate: сама считает qty/цены и вызывает _risk_check()."""
        try:
            last_price = float(self.shared_ws.get_last_price(symbol) or 0.0) if hasattr(self, 'shared_ws') else 0.0
            usd = float(getattr(self, 'POSITION_VOLUME', 100.0) or 100.0)
            qty = usd / last_price if last_price > 0 else usd
            ok, reason = await self._risk_check(symbol, side, qty, last_price=last_price)
            return ok, reason
        except Exception as e:
