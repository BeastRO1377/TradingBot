    ...
    async def manage_symbol_selection(self, min_turnover=2_000_000,
        ...
        while True:
            ...
            try:
                ...
                # 5. Динамически обновляем подписки
                if symbols_to_add:
                    self.ws.subscribe(topic=f"kline.{self.interval}.{{symbol}}", symbol=list(symbols_to_add), callback=self._callback)
                    self.ws.subscribe(topic="tickers.{symbol}", symbol=list(symbols_to_add), callback=self._callback)
                    self.ws.subscribe(topic="liquidation.{symbol}", symbol=list(symbols_to_add), callback=self._callback)
                self.active_symbols.update(symbols_to_add)  # <-- важно!
                # УДАЛИТЬ
                if symbols_to_remove and self.ENABLE_UNSUBSCRIBE:
                    self.ws.unsubscribe(topic=f"kline.{self.interval}.{{symbol}}", symbol=list(symbols_to_remove))
                    self.ws.unsubscribe(topic="tickers.{symbol}",     symbol=list(symbols_to_remove))
                    self.ws.unsubscribe(topic="liquidation.{symbol}", symbol=list(symbols_to_remove))
...
class TradingBot:
    ...
    __slots__ = (
        ...
        "stop_update_count", "intraday_trailing_enabled",


    , "last_entry_ts", "entry_cooldown_sec")
    ...
    async def set_trailing_stop(self, symbol: str, avg_price: float, pnl_pct: float, side: str) -> bool:
        ...
        try:
            ...
            if side.lower() == "buy":
                ...
                raw_price = last_price * (1 - gap_pct / 100.0)
            else: # sell
