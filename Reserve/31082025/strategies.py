...
async def squeeze_strategy(bot, symbol: str) -> bool:
    """
    [УНИФИЦИРОВАННАЯ ВЕРСИЯ] Ищет сквиз и передает его стандартному
    обработчику _process_signal, как и все остальные стратегии.
    """
    signal_key = None
    try:
        ...
        logger.info(f"🔥 [{symbol}] ОБНАРУЖЕН СКВИЗ! Движение: {pct_change_5m:.2f}%. Передано AI-аналитику.")
        ...
        # --- ГЛАВНОЕ ИЗМЕНЕНИЕ: ВЫЗЫВАЕМ СТАНДАРТНЫЙ ОБРАБОТЧИК ---
