...
async def trading_mode_select(message: Message):
    kb = ReplyKeyboardMarkup(resize_keyboard=True, keyboard=[
        ...
        [types.KeyboardButton(text="dom_squeeze_only")], # <-- НОВАЯ КНОПКА
...
# --- НАЧАЛО ИЗМЕНЕНИЙ ---
# Добавляем 'dom_squeeze_only' в список разрешенных режимов
@router.message(F.text.in_({"golden_only", "liquidation_only", "full", "squeeze_only", "golden_squeeze", "dom_squeeze_only"}))
