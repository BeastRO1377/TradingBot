...
async def run_all() -> None:
    # --- БЛОК MONKEY PATCH ОСТАЕТСЯ БЕЗ ИЗМЕНЕНИЙ ---
    ...
    # shared_ws создается здесь и будет использоваться для остановки
    ...
    # РЕКОМЕНДАЦИЯ: Выполняем потенциально долгую загрузку моделей неблокирующим способом
    logger.info("Загрузка ML-моделей для всех ботов...")
    ...
    try:
        ...
        # 2. Ожидаем завершения отмены
        await asyncio.gather(*all_tasks, return_exceptions=True)
        
        # 3. Корректно останавливаем экземпляры ботов (это вызовет .exit() для их приватных WS)
        for bot_instance in GLOBAL_BOTS:
            await bot_instance.stop()
            
        # --- НАЧАЛО ИЗМЕНЕНИЙ (КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ) ---
        
        # 4. Корректно останавливаем публичный WS менеджер
        if shared_ws:
            shared_ws.stop()
