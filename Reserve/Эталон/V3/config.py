...
# --- НАЧАЛО ИЗМЕНЕНИЙ ---
# Добавляем параметры для новой стратегии
DOM_SQUEEZE_STRATEGY = {
    "ENABLED": True,
    "SQUEEZE_LOOKBACK_MS": 5000,       # За какой период (в мс) отслеживать изменение цены
    "SQUEEZE_THRESHOLD_PERCENT": 0.2,  # % изменения цены, который считаем сквизом
    "WALL_MULTIPLIER": 10,             # Во сколько раз "стена" должна быть больше средней плотности
    "WALL_SCAN_DEPTH": 20,             # Глубина сканирования стакана для поиска средней плотности
    "WALL_PROXIMITY_TICKS": 5,         # На сколько тиков цена должна подойти к стене для входа
    "COOLDOWN_SECONDS": 300,           # Кулдаун для символа после срабатывания
}
# --- КОНЕЦ ИЗМЕНЕНИЙ ---
