            ...
            async with self.position_lock:
                ...
                if pos and pos.get("is_opening") and exec_data.get("side") == pos.get("side"):
                    ...
                    if exec_price > 0:
                        ...
                        # --- ИЗМЕНЕНИЕ: Записываем базу для стопа в позицию ---
                        sl_base = self.pending_strategy_comments.pop(f"{symbol}_sl_base", None)
                        if sl_base:
...
# bot_core.py
    ...
    async def _set_initial_stop_loss(self, symbol: str, pos: dict):
        """
        [ФИНАЛЬНАЯ ВЕРСИЯ 3.0] Устанавливает стоп-лосс с использованием
        надежной базы, переданной от "Охотника".
        """
        try:
            avg_price = self._resolve_avg_price(symbol, pos)
            if avg_price <= 0: return
            ...
            # --- ЛОГИКА 1: Умный стоп для SQUEEZE на основе данных от "Охотника" ---
            if 'squeeze' in source.lower() and 'sl_base_price' in pos:
                extreme_price = utils.safe_to_float(pos.get('sl_base_price'))
                tick_size = self.price_tick_map.get(symbol, 0.000001)

                if side == "buy":
                    # Стоп ставится чуть НИЖЕ реального дна, найденного "Охотником"
                    stop_price = extreme_price - (tick_size * 5)
                    stop_reason = f"Squeeze Low ({extreme_price:.6f})"
                else: # side == "sell"
                    # Стоп ставится чуть ВЫШЕ реального пика, найденного "Охотником"
                    stop_price = extreme_price + (tick_size * 5)
                    stop_reason = f"Squeeze High ({extreme_price:.6f})"
            
            # --- ЛОГИКА 2: Фоллбэк, если что-то пошло не так ---
            if not stop_price:
                max_stop_pct = utils.safe_to_float(self.user_data.get("max_safety_stop_pct", 2.0)) / 100.0 # Уменьшил до 2%
                stop_price = avg_price * (1 - max_stop_pct) if side == "buy" else avg_price * (1 + max_stop_pct)
                stop_reason = f"Fallback {max_stop_pct:.2%}"

            # --- Финальная проверка и установка стопа ---
            if not stop_price or stop_price <= 0:
                logger.error(f"[{symbol}] Рассчитан невалидный стоп-лосс ({stop_price}). Установка отменена.")
                return
            
            # Дополнительная защита: стоп не может быть дальше 4% от цены входа
            max_allowed_stop_dist_pct = 0.04 
            if abs(stop_price - avg_price) / avg_price > max_allowed_stop_dist_pct:
                 stop_price = avg_price * (1 - max_allowed_stop_dist_pct) if side == "buy" else avg_price * (1 + max_allowed_stop_dist_pct)
                 stop_reason += " (CAPPED at 4%)"
                 logger.warning(f"[{symbol}] Рассчитанный стоп был слишком широким. Ограничен 4% уровнем: {stop_price:.6f}")


            logger.info(f"🛡️ [{symbol}] Установка первоначального стопа. Причина: {stop_reason}. Цена входа: {avg_price:.6f}, Финальный стоп: {stop_price:.6f}")
