...
class TradingBot:
    ...
    def _ensure_trailing_state(self, symbol: str) -> None:
        import asyncio
        if not hasattr(self, "last_trailing_update_ts"):
            self.last_trailing_update_ts = {}
        if not hasattr(self, "trailing_lock"):
            self.trailing_lock = {}
        ...
        if not hasattr(self, "_adx_hist"):
            self._adx_hist = {}
        if not hasattr(self, "_wall_stall_since"):
            self._wall_stall_since = {}
        if not hasattr(self, "_trailing_log_ts"):
            self._trailing_log_ts = {}
        if not hasattr(self, "_trailing_prev_stop"):
            self._trailing_prev_stop = {}
    ...
    # def _ensure_trailing_state(self, symbol: str):
    #     """Гарантирует, что все необходимые переменные для трейлинга существуют."""
    #     if not hasattr(self, "last_stop_price"): self.last_stop_price = {}
    #     if not hasattr(self, "last_sent_stop_price"): self.last_sent_stop_price = {}
    #     if not hasattr(self, "last_stop_attempt_ts"): self.last_stop_attempt_ts = {}
    #     if not hasattr(self, "last_trailing_update_ts"): self.last_trailing_update_ts = {}
    #     if not hasattr(self, "trailing_lock"): self.trailing_lock = defaultdict(asyncio.Lock)

    #     self.last_stop_price.setdefault(symbol, 0.0)
    #     self.last_sent_stop_price.setdefault(symbol, 0.0)
    #     self.last_stop_attempt_ts.setdefault(symbol, 0.0)
    #     self.last_trailing_update_ts.setdefault(symbol, 0.0)
    ...
    async def start(self):        
        ...
        asyncio.create_task(self._golden_setup_screener_loop())
        asyncio.create_task(self._scanner_worker_loop()) # <-- НОВЫЙ ВОРКЕР
    ...
    async def train_and_save_model(self):
        ...
        try:
            ...
            # 3. Обучение модели (с правильным вызовом через ai_ml.)
            # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
            ...
            if model and scaler:
                ...
                # 4. Сохранение модели и скейлера (с правильным вызовом через ai_ml.)
                # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
    ...
    #     # ИСПРАВЛЕНО: берём минимальный откат из настроек
    #     min_retrace = float(cfg.get('MIN_RETRACE_FROM_EXTREME_PCT', 0.4))
    ...
    async def _entry_guard(self, symbol: str, side: str, candidate: dict | None = None, features: dict | None = None) -> tuple[bool, str]:
        ...
        # --- НАЧАЛО КЛЮЧЕВОГО ИЗМЕНЕНИЯ: Убираем блокировку по retrace ---
        # Эта проверка блокировала почти все сигналы, так как требовала слишком большого отката.
        # Мы ее отключаем, доверяя логике "Охотника", который сам найдет точку входа после пика.
        
        # Старый код проверки retrace полностью удален или закомментирован.
        
        # --- КОНЕЦ КЛЮЧЕВОГО ИЗМЕНЕНИЯ ---

        # Проверка на импульс против нашей стороны остается, она полезна
        pump1 = cfg.get("PUMP_BLOCK_1M_PCT", 1.2)
        pump5 = cfg.get("PUMP_BLOCK_5M_PCT", 3.0)
        dump1 = cfg.get("DUMP_BLOCK_1M_PCT", 1.2)
        dump5 = cfg.get("DUMP_BLOCK_5M_PCT", 3.0)
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        if not features:
            features = await self.extract_realtime_features(symbol)
        ok, reason = await self._entry_guard(symbol, side, candidate=candidate, features=features)
        if not ok:
        ...
        if effective_total_vol + volume_to_open > float(self.MAX_TOTAL_VOLUME):
        ...
        # 4) Расчёт и округление qty (ВАЖНО: без умножения на плечо,
        #    чтобы не переливать общий риск и не пробивать MAX_TOTAL_VOLUME)
        ...
        # 5) Бронируем pending-объём (чтобы лимит портфеля учитывал и «висяки»)
        async with self.pending_orders_lock:
            # повторная проверка на гонку между потоками
            if symbol in self.open_positions or symbol in self.pending_orders:
        ...
        # 6) Сохраняем опорную цену pending-входа (пока нет fill'а)
        ...
        try:
            ...
            # снимаем «бронь» только при фейле
            async with self.pending_orders_lock:
                self.pending_orders.pop(symbol, None)
                self.pending_timestamps.pop(symbol, None)
            # опорную цену тоже гасим
            self.pending_open_exec.pop(symbol, None)


    # --- Breakout Pre-Flight: краткая приёмка за стеной перед отправкой заявки ---
    ...
    # --- Breakout Post-Entry Fail-Safe: ранний выход при быстрой инвалидации пробоя ---
    ...
    # --- Breakout Retest Entry: ждём откат к стене и отбой, затем входим ---
    async def _enter_breakout_on_retest(self, candidate: dict, current_features: dict | None = None) -> None:
        ...
        while time.time() < t_deadline:
            ...
            last = self.safe_last_price(symbol)  # у вас эта функция тут уже есть выше
            ...
            # best_ask = self.best_ask_map.get(symbol, 0.0)
            # best_bid = self.best_bid_map.get(symbol, 0.0)
    ...
    async def handle_execution(self, msg: dict):
        for exec_data in msg.get("data", []):
            ...
            async with self.position_lock:
                ...
                # ИСПРАВЛЕНО: корректная обработка усреднения
        ...
        async with self.position_lock:
            for p in msg.get("data", []):
                ...
                if is_new_pos:
                    ...
                    self.open_positions[symbol] = pos  # если вы пересохраняете объект
    ...
    # --- НАЧАЛО ИЗМЕНЕНИЙ: Перенос функции из strategies.py ---
    async def _hunt_reversal_mlx(self, candidate: dict, features: dict, signal_key: tuple):
        """
        Обработка реверса с MLX-обучением на результатах.
        Эта функция теперь является методом класса TradingBot.
        """
        try:
            # Стандартная логика охоты за реверсом
            result = await self._hunt_reversal(candidate, features, signal_key)
            
            # Обучение MLX модели на результате сделки
            if result is not None and 'success' in result and hasattr(self, 'mlx_predictor'):
                success = result['success']
                # Используем сохраненные MLX фичи для обучения
                if 'mlx_features' in candidate:
                    await self.mlx_predictor.incremental_train(
                        candidate['mlx_features'], 
                        1 if success else 0
                    )
                    
                # Периодическое сохранение модели
                if len(self.mlx_predictor.training_buffer) % 50 == 0:
                    self.mlx_predictor.save_model()
                    
                logger.info(f"🧠 [{candidate['symbol']}] MLX модель обновлена. Результат: {'УСПЕХ' if success else 'ПРОВАЛ'}")
            
            return result
            
        except Exception as e:
            logger.error(f"Ошибка в _hunt_reversal_mlx: {e}")
    ...
    async def _hunt_reversal(self, candidate: dict, features: dict, signal_key: tuple):
        ...
        peak_lock_sec = self.user_data.get("DOM_PEAK_LOCK_SEC", 30)
        ...
        required_confirmations = 1 if wall_rating >= 3 else 2
        logger.info(f"🏹 [{symbol}] Охотник V8 активирован. Рейтинг стены: {wall_rating}. Требуется доказательств: {required_confirmations}.")
        ...
        while time.time() - start_time < hunt_window_sec:
            try:
                ...
                confirmations = []
                reasons = []
                pullback_pct = abs(current_price - extreme_price) / extreme_price * 100 if extreme_price > 0 else 0
                ...
                vol_1m, avg_vol_30m = current_features.get('vol1m', 0.0), current_features.get('avgVol30m', 1.0)
                if avg_vol_30m > 0 and vol_1m < avg_vol_30m * 0.8:
                    confirmations.append("Volume")
                    reasons.append("Volume Low")
                if len(confirmations) >= required_confirmations:
                    logger.warning(f"✅ [ОХОТНИК-РЕШАТЕЛЬ] {symbol}/{side}: Найдено {len(confirmations)}/{required_confirmations} подтверждений! Вход разрешен. Причины: {', '.join(reasons)}")
                    candidate['stop_loss_price_base'] = extreme_price
                    await self.execute_trade_entry(candidate, current_features)
                    return
            except Exception as e:
                logger.error(f"💥 [ОХОТНИК-РЕШАТЕЛЬ] {symbol}/{side}: Критическая ошибка: {e}", exc_info=True)
                break
        logger.warning(f"⏳ [ОХОТНИК-РЕШАТЕЛЬ] {symbol}/{side}: Окно входа истекло, цель не подтверждена.")
        self.active_signals.discard(signal_key)

    async def _initiate_hunt(self, candidate: dict, features: dict, signal_key: tuple):
        """
        [ИСПРАВЛЕННАЯ ВЕРСИЯ] Корректно маршрутизирует сигналы 'fade' и 'breakout'
        в соответствующие тактические группы ('Охотники').
        """
        ...
        elif 'fade' in source or 'squeeze' in source or 'liquidation' in source:
            logger.info(f"🎯 [{symbol}] Запуск охотника на ОТБОЙ/РЕВЕРС для '{source}'.")
            asyncio.create_task(self._hunt_reversal(candidate, features, signal_key))

        elif 'golden_setup' in source:
            logger.info(f"🎯 [{symbol}] Запуск охотника 'Прорыватель' для '{source}'.")
            asyncio.create_task(self._hunt_golden_breakout(candidate, features, signal_key))
    ...
    #         ml_score = 0.5
    #         if self.ml_inferencer and self.ml_inferencer.model:
    #             try:
    #                 ml_features_list = [features.get(k, 0.0) for k in config.FEATURE_KEYS]
    #                 ml_features = np.array([ml_features_list], dtype=np.float32)
    #                 prediction = self.ml_inferencer.infer(ml_features)
    #                 ml_score = 1 / (1 + np.exp(-prediction.item())) 
    #                 logger.info(f"🤖 [{symbol}] ML-модель оценила сигнал '{source}'. Вероятность успеха: {ml_score:.2f}")
    #             except Exception as e:
    #                 logger.warning(f"[{symbol}] Ошибка при получении предсказания от ML-модели: {e}")
    #         else:
    #             logger.info(f"[{symbol}] ML-модель не загружена, оценка пропускается.")
    #         features['ml_score'] = ml_score
    #         if 'squeeze' in source or 'liquidation' in source or 'dom_squeeze' in source:
    ...
    #         logger.debug(f"Сигнал {signal_key} передан AI-аналитику с ML-оценкой...")
    ...
    #         if action == "EXECUTE":
    #             logger.info(f"✅ [{symbol}] Сигнал ОДОБРЕН AI. Причина: {ai_response.get('justification')}. Передано тактической группе.")
    #             candidate['justification'] = ai_response.get('justification')
    #             await self._initiate_hunt(candidate, features, signal_key)
    #         else:
    #             logger.info(f"❌ [{symbol}] Сигнал ОТКЛОНЕН AI. Причина: {ai_response.get('justification')}")
    #             self.active_signals.discard(signal_key)
    #     except Exception as e:
    ...
    # async def _process_signal(self, candidate: dict, features: dict, signal_key: tuple):
    #     """
    #     [ГИБРИДНАЯ ВЕРСИЯ] Python проверяет критическое условие (RSI),
    #     а AI выполняет финальную проверку контекста.
    #     """
    #     symbol = candidate.get("symbol")
    #     source = candidate.get("source", "")
        
    #     try:
    #         # --- НАЧАЛО КЛЮЧЕВОГО ИЗМЕНЕНИЯ: Гибридная проверка ---
            
    #         # 1. Python выполняет строгую проверку критического условия
    #         rsi_val = features.get('rsi14', 50.0)
    #         side = candidate.get("side")
    #         is_critical_condition_met = False
            
    #         # Для контртрендовых сигналов проверяем экстремальный RSI
    #         if 'fade' in source or 'squeeze' in source or 'liquidation' in source:
    #             if side == 'Sell' and rsi_val > 75:
    #                 is_critical_condition_met = True
    #             elif side == 'Buy' and rsi_val < 25:
    #                 is_critical_condition_met = True
    #         else:
    #             # Для других стратегий (например, трендовых) условие по умолчанию истинно
    #             is_critical_condition_met = True

    #         # 2. Добавляем результат проверки в `features`, чтобы передать его в промпт
    #         features['critical_condition_met'] = is_critical_condition_met
            
    #         # --- КОНЕЦ КЛЮЧЕВОГО ИЗМЕНЕНИЯ ---

    #         # Нейтральная оценка ML остается как заглушка
    #         features['ml_score'] = 0.5

    #         # Entry Guard (без изменений)
    #         if 'squeeze' in source or 'liquidation' in source or 'dom_squeeze' in source:
    #             pass # Пропускаем guard для этих стратегий
    #         else:
    #             ok, reason_guard = await self._entry_guard(symbol, side, candidate, features)
    #             if not ok:
    #                 logger.info(f"🛡️ [{symbol}] Сигнал заблокирован 'Сторожем': {reason_guard}")
    #                 self.active_signals.discard(signal_key)
    #                 return

    #         # Вызов AI с новым промптом
    #         prompt = ai_ml.build_primary_prompt(candidate, features, self.shared_ws)
    ...
    async def _process_signal(self, candidate: dict, features: dict, signal_key: tuple):
        """
        [АРХИТЕКТУРА V4] Больше не вызывает AI.
        Просто передает сигнал в нужного "Охотника".
        """
        try:
            await self._initiate_hunt(candidate, features, signal_key)
        except Exception as e:
    ...
    # ... (остальной код bot_core.py без изменений) ...
    ...
    async def _hunt_golden_breakout(self, candidate: dict, features: dict, signal_key: tuple):
        ...
        а не жесткие одновременные условия.
        """
        symbol = candidate["symbol"]
        side = candidate["side"]
        ...
        while time.time() - start_time < hunt_window_sec:
            try:
                ...
                if score >= score_threshold:
                    logger.info(f"✅ [СЛЕДОПЫТ] {symbol}/{side}: Достаточно улик! Счет: {score}/{score_threshold}. Причины: {', '.join(reasons)}. Запрос финального одобрения у AI...")
                    
                    prompt = ai_ml.build_final_confirmation_prompt(candidate, current_features)
                    ai_response = await ai_ml.ask_ollama_json(
                        self.ai_advisor_model,
                        [{"role": "user", "content": prompt}],
                        self.ai_timeout_sec,
                    ...
                    if ai_response.get("action") == "EXECUTE":
                        logger.warning("✅ [AI] Финальное ОДОБРЕНИЕ получено. Исполнение сделки.")

                        if candidate.get("source") == "mlx_dom_breakout":
                            ok_pref = await self._pre_flight_revalidate_breakout(candidate)
                            if not ok_pref:
                                logger.warning(f"[{candidate.get('symbol')}] [ПРЕ-ФЛАЙТ] Breakout отменён: нет приёмки за стеной/дрейф назад.")
                                return

                            # Режим входа: ретест или немедленно
                            dcfg = self.user_data.get("dom_squeeze_settings", config.DOM_SQUEEZE_STRATEGY)
                            entry_mode = str(dcfg.get("ENTRY_MODE", "retest")).lower()   # 'retest'|'immediate'

                            if entry_mode == "retest":
                                await self._enter_breakout_on_retest(candidate, current_features)
                                return

                        # По умолчанию — немедленный вход (для не-breakout стратегий тоже)
                        await self.execute_trade_entry(candidate, current_features)

                        if candidate.get("source") == "mlx_dom_breakout":
                            asyncio.create_task(self._post_entry_failsafe_breakout(candidate))
                    else:
                        logger.warning(f"❌ [AI] Финальное ВЕТО на сделку. Причина: {ai_response.get('justification')}")
                    
                    self.active_signals.discard(signal_key)
                    return # Выходим из цикла после решения
    ...
    async def _set_initial_stop_loss(self, symbol: str, pos: dict, force: bool = False):
        """
        [V7 - ЗАЩИЩЕННАЯ ВЕРСИЯ] Добавлена проверка на race condition:
            ...
            if last_price > 0:
                ...
                (side == "sell" and final_stop_price <= last_price):
                    logger.warning(
                        ...
                        f"(SL={final_stop_price:.6f}, last={last_price:.6f}). Передаём управление трейлингу/дальнейшей логике."
                    )
                    # не пытаемся поставить SL, не закрываем ничего экстренно
        ...
        *,
        ...
        try:
            ...
            if resp.get("retCode") == 0:
                ...
                try:
                    sent_sl = float(payload.get("stopLoss"))  # если у тебя переменная называется по-другому — возьми её
                except Exception:
                    sent_sl = float(stop_price)  # fallback, если payload недоступен
            ...
            # Фиксируем успешно принятый биржей стоп как "последний отправленный"
            self.last_sent_stop_price[symbol] = float(stop_price)
            self.last_known_stop_price[symbol] = float(stop_price)  # опционально, если используете
    ...
    #     """
    #     Обрабатывает тики: запускает ВЧ-стратегии или управляет трейлингом открытой позиции.
    #     """
    #     now = time.time()
    #     if now - self.last_hf_check_ts.get(symbol, 0) < 0.2:
    #         return
    ...
    #     # Если позиция есть, запускаем логику трейлинга
    #     if pos.get("initial_stop_set") and not pos.get("is_closing"):
    ...
    async def on_ticker_update(
        ...
        try:
            ...
            # если bid/ask не пришли kwargs — попробуем достать из сырого тикера
            if tick:
                if best_bid is None:
        ...
        if pos and pos.get("initial_stop_set") and not pos.get("is_closing"):
            await self._run_trailing_stop_logic(symbol, last_price, pos)
        # Кэшируем last для фолбэков best bid/ask
        try:
            if symbol and last_price:
                self.last_price_map[symbol] = utils.safe_to_float(last_price)
        except Exception:
            pass

    # --- НАЧАЛО КЛЮЧЕВОГО ИСПРАВЛЕНИЯ: Трейлинг с двумя режимами ---
    async def _run_trailing_stop_logic(self, symbol: str, last_price: float, pos: dict):
        """
        [ФИНАЛЬНАЯ ВЕРСИЯ] Управляет трейлингом в зависимости от выбранного
        ...
        if mode == "simple_gap":
            ...
            # Если режим не указан или неверный, ничего не делаем
            return
    ...
    async def _run_simple_gap_trailing(self, symbol: str, last_price: float, pos: dict):
        ...
        now = time.time()
        if now - self.last_trailing_update_ts.get(symbol, 0) < 1.5: # Простой троттлинг
            return
        ...
        async with lock:
            ...
            # Не ставим стоп «внутрь» текущей цены (иначе он сработает мгновенно)
            if is_better:
                if (side == "buy" and candidate_stop >= last_price) or \
                (side == "sell" and candidate_stop <= last_price):
                    return

                self.last_trailing_update_ts[symbol] = now
                await self.set_or_amend_stop_loss(candidate_stop, symbol=symbol)

                logger.info(f"📈 [{symbol}] Двигаю стоп (простой гэп). Пик: {peak_price:.6f}, Новый SL: {candidate_stop:.6f}")
                self.last_trailing_update_ts[symbol] = now
                await self.set_or_amend_stop_loss(candidate_stop, symbol=symbol)
    ...
    async def _run_dynamic_atr_trailing(self, symbol: str, last_price: float, pos: dict):
        ...
        2) После активации (ROI_лев ≥ TRAIL_ACTIVATE_ROI_LEVERED_PCT): динамический ATR/ADX трейлинг (fade+wall) + ratchet.
        ...
        now = time.time()
        if now - self.last_trailing_update_ts.get(symbol, 0.0) < float(tcfg.get("MIN_UPDATE_INTERVAL_SECS", 1.6)):
            return

        # --- гарантируем наличие всех служебных структур
        if not hasattr(self, "last_sent_stop_price"):
            self.last_sent_stop_price = {}
        if not hasattr(self, "best_bid_map"):
            self.best_bid_map = {}
        if not hasattr(self, "best_ask_map"):
            self.best_ask_map = {}
        if not hasattr(self, "_adx_hist"):
            self._adx_hist = {}  # ИНИЦИАЛИЗИРУЕМ ДО ЛЮБЫХ RETURN
        ...
        async with lock:
            ...
            # --- эффективное плечо (на случай отсутствия в позиции — берём из карты или дефолт)
            ...
            lev = max(1.0, lev)  # не даём уйти < 1

            # --- ROI: считаем и «ценовой», и «левереджный», учитываем направление, берём только профитную часть
            roi_unlev_signed = (last_price - avg_price) / max(1e-12, avg_price) * 100.0  # % хода цены
            if side == "sell":
                roi_unlev_signed = (avg_price - last_price) / max(1e-12, avg_price) * 100.0
            roi_unlev_profit = max(0.0, roi_unlev_signed)
            roi_lev_profit   = roi_unlev_profit * lev  # вот это — «ROI с плечом», то, что ты имеешь в виду

            # === ФАЗА 1: PRE до активации по целевому ROI (С УЧЁТОМ ПЛЕЧА) ===
            roi_target_lev = float(tcfg.get("TRAIL_ACTIVATE_ROI_LEVERED_PCT", 5.0))  # напр. 5.0 → 5% ROI при плече
            if roi_lev_profit < roi_target_lev:
                ...
                # пробуем спец-хелпер (если у тебя он есть), иначе берём "противоположную" стену через общий метод
                get_opp = getattr(self, "get_dom_nearest_opposite_wall_price", None)
                getter  = getattr(self, "get_dom_next_wall_price", None)
                try:
                    if callable(get_opp):
                        maybe = get_opp(symbol, side)  # сам хелпер определит нужную сторону стакана
                        wall_price = await maybe if inspect.iscoroutine(maybe) else maybe
                    elif callable(getter):
                        # «противоположная» сторона стакана: для Buy → нижняя BID; для Sell → верхняя ASK
            ...
            # ROI от пика (для k-тиеров) — по цене (без плеча), так корректнее для волы/ATR
            roi_peak_unlev = abs(peak - avg_price) / max(1e-12, avg_price) * 100.0

            # выбор k по ROI-ступеням + ADX
            ...
            # Армирование безубытка (как у тебя — через время/ROI_по_цене)
            be_buf_pct     = float(tcfg.get("BREAKEVEN_BUFFER_PCT", 0.18))
            be_arm_sec     = float(tcfg.get("BREAKEVEN_ARM_SEC", 20.0))
            be_arm_roi_pct = float(tcfg.get("BREAKEVEN_ARM_ROI_PCT", 0.25))  # это именно по цене (без плеча)
            ...
            # ADX-затухание (история гарантированно инициализирована в начале функции)
            dq = self._adx_hist.get(symbol)
            if dq is None:
                dq = collections.deque(maxlen=int(tcfg.get("ADX_SLOPE_WINDOW", 6)))
                self._adx_hist[symbol] = dq
            dq.append((now, adx))
            adx_slope_pm = 0.0
            if len(dq) >= 2:
                t0, a0 = dq[0]
            ...
            if next_wall_price:
                ...
                if 0 <= dist_ticks <= band_ticks:
                    if not hasattr(self, "_wall_stall_since"):
                        self._wall_stall_since = {}
                    since = self._wall_stall_since.get(symbol) or now
                    self._wall_stall_since[symbol] = since
                    stall_sec = now - since
                    ...
                    if hasattr(self, "_wall_stall_since"):
                        self._wall_stall_since.pop(symbol, None)
    ...
    async def get_effective_total_volume(self) -> float:
        """
        Сумма открытого объёма + забронированные pending.
        Поддерживает как sync-, так и async-реализацию get_total_open_volume().
        """
        import inspect
        import logging
        ...
        # 1) Открытый объём (safe)
        try:
            res = self.get_total_open_volume()
            open_vol = await res if inspect.isawaitable(res) else res
        except Exception as e:
            logger.warning(f"[VOLUME] get_total_open_volume() failed: {e}")
            open_vol = 0.0

        # 2) Pending объёмы (safe)
        try:
            pending_vol = float(sum(float(v) for v in getattr(self, "pending_orders", {}).values()))
        except Exception as e:
            logger.warning(f"[VOLUME] pending sum failed: {e}")
            pending_vol = 0.0

        try:
            return float(open_vol or 0.0) + float(pending_vol or 0.0)
        except Exception:
            # крайняя защита от типов
            return float(open_vol or 0.0)
    ...
    # --- Back-compat shim: старые стратегии ожидают mkt_features ---
    async def extract_realtime_mkt_features(self, symbol: str) -> dict:
        """
        Совместимость для старых вызовов стратегий.
        Делегирует на extract_realtime_features и возвращает dict.
        """
        try:
    ...
    # --- UNIFIED LAST PRICE HELPER ---
    def get_last_price(self, symbol: str) -> float:
        """
        Возвращает last price из доступных источников с учетом разных ключей:
        shared_ws.ticker_data, cached meta, а также fallback на mark/price.
        """
        try:
        ...
        # Bybit WS/HTTP обычно дают lastPrice/markPrice в camelCase
        candidates = [
            tdata.get("lastPrice"),
            tdata.get("markPrice"),
            tdata.get("last_price"),
            tdata.get("price"),
            tdata.get("indexPrice"),
        ]

        # Если в features уже лежит alias (когда кто-то положил заранее)
        # мы тоже попробуем его:
        for k in ("LastPrice", "LAST_PRICE", "lastprice"):
            if k in tdata:
                candidates.append(tdata.get(k))

        # Фолбэк: попробуем из self.symbol_meta
