...
class TradingBot:
    ...
    async def _hunt_reversal(self, candidate: dict, features: dict, signal_key: tuple):
        ...
        logger.info(f"🏹 [{symbol}] Охотник за разворотом V9 (Scoring) активирован. Порог: {score_threshold} очков.")
        ...
        while time.time() - start_time < hunt_window_sec:
            try:
                ...
                # 2. Ожидание стабилизации после пика
                if time.time() - last_peak_update_time < peak_lock_sec:
                    continue

                # 3. Расчет очков для подтверждения разворота
                score = 0
                reasons = []
                ...
                if avg_vol_30m > 0 and vol_1m < avg_vol_30m:
                    ...
                    score += int((1 - volume_ratio) * 30) # Чем ниже объем, тем больше очков
                    reasons.append(f"Volume_Low(x{volume_ratio:.1f})")

                # 4. Принятие решения
                if score >= score_threshold:
                    logger.warning(f"✅ [ОХОТНИК-РЕШАТЕЛЬ] {symbol}/{side}: Набрано {score}/{score_threshold} очков! Вход разрешен. Причины: {', '.join(reasons)}")
                    candidate['stop_loss_price_base'] = extreme_price
                    await self.execute_trade_entry(candidate, current_features)
                    return
                else:
                    logger.debug(f"🏹 [{symbol}] Охотник ждет. Счет: {score}/{score_threshold}. Причины: {', '.join(reasons)}")
    ...
    async def _initiate_hunt(self, candidate: dict, features: dict, signal_key: tuple):
        ...
        elif any(k in source for k in ['breakout', 'golden_setup']):
    ...
    async def _run_simple_gap_trailing(self, symbol: str, last_price: float, pos: dict):
        ...
        now = time.time()
        if now - self.last_trailing_update_ts.get(symbol, 0) < 1.5:
            return
    ...
    async def _run_dynamic_atr_trailing(self, symbol: str, last_price: float, pos: dict):
        """
        Двухфазный динамический трейлинг.
        """
        self._ensure_trailing_state(symbol)
        ...
        if not hasattr(self, "best_bid_map"): self.best_bid_map = {}
        if not hasattr(self, "best_ask_map"): self.best_ask_map = {}
        if not hasattr(self, "_adx_hist"): self._adx_hist = {}
        ...
        async with lock:
            ...
            if avg_price <= 0.0 or last_price <= 0.0: return
            ...
            lev = max(1.0, utils.safe_to_float(pos.get("leverage") or tcfg.get("ASSUME_LEVERAGE_IF_MISSING", 1.0)))

            roi_unlev_signed = ((last_price / avg_price) - 1) * 100.0 if side == "buy" else ((avg_price / last_price) - 1) * 100.0
            roi_unlev_profit = max(0.0, roi_unlev_signed)
            roi_lev_profit   = roi_unlev_profit * lev
            ...
            if roi_lev_profit < roi_target_lev:
                ...
                wall_price = await self._get_opposite_wall_price(symbol, side, last_price)
                ...
                if side == "buy":
                    hard_floor = last_price - min_gap_ticks * tick
                    if candidate >= hard_floor: candidate = hard_floor - tick
                else:
                    hard_ceiling = last_price + min_gap_ticks * tick
                    if candidate <= hard_ceiling: candidate = hard_ceiling + tick

                candidate = self._round_to_tick(symbol, float(candidate), side)
            ...
            def pick_k(adx_val: float, roi_val_unlev: float) -> float:
                ...
                if not elig: return float(tcfg.get("K_DEFAULT", 2.5))
                top_roi = max(e.get("roi", 0.0) for e in elig)
                cand = [e for e in elig if e.get("roi", 0.0) == top_roi]
                def band_match(b: str, a: float) -> bool:
                    return ((b == "adx_gt_30" and a > 30.0) or (b == "adx_ge_20" and 20.0 <= a <= 30.0) or (b == "adx_lt_20" and a < 20.0))
                for e in cand:
            ...
            if be_now and not pos.get("be_armed"):
                pos["be_armed"] = True
            
            dq = self._adx_hist.setdefault(symbol, collections.deque(maxlen=int(tcfg.get("ADX_SLOPE_WINDOW", 6))))
            dq.append((now, adx))
            adx_slope_pm = 0.0
            if len(dq) >= 2:
                t0, a0 = dq[0]; t1, a1 = dq[-1]
                adx_slope_pm = (a1 - a0) / max(1e-6, t1 - t0) * 60.0

            fade = 0.0
            if adx < float(tcfg.get("FADE_ADX_LT", 18.0)): fade += float(tcfg.get("FADE_WEIGHT_LOW_ADX", 0.5))
            if adx_slope_pm < -abs(float(tcfg.get("FADE_ADX_SLOPE_DOWN_PM", 4.0))): fade += float(tcfg.get("FADE_WEIGHT_SLOPE", 0.5))
            fade = min(1.0, max(0.0, fade))

            u_wall = 0.0
            next_wall_price = await self._get_opposite_wall_price(symbol, "sell" if side == "buy" else "buy", last_price)
            if next_wall_price:
                dist_ticks = abs(next_wall_price - last_price) / tick
                if 0 <= dist_ticks <= int(tcfg.get("WALL_BAND_TICKS", 4)):
                    since = self._wall_stall_since.setdefault(symbol, now)
                    stall_sec = now - since
            ...
            u = min(float(tcfg.get("MAX_TIGHTEN_URGENCY", 0.9)), 
                    float(tcfg.get("FADE_URGENCY_WEIGHT", 0.6)) * fade + float(tcfg.get("WALL_URGENCY_WEIGHT", 0.6)) * u_wall)
            ...
            if pos.get("be_armed"):
                if side == "buy": candidate = max(peak_stop, avg_price * (1.0 + be_buf_pct / 100.0))
                else: candidate = min(peak_stop, avg_price * (1.0 - be_buf_pct / 100.0))
            else:
                candidate = peak_stop

            min_gap_ticks = int(tcfg.get("MIN_GAP_TICKS", 2))
            total_gap_ticks = max(1, min_gap_ticks + int(round(u * int(tcfg.get("URGENCY_EXTRA_GAP_TICKS", 1)))))
            if side == "buy":
                price_floor = last_price - total_gap_ticks * tick
                candidate = (1.0 - u) * candidate + u * price_floor
                if candidate >= last_price - min_gap_ticks * tick:
                    candidate = last_price - (min_gap_ticks + 1) * tick
            else:
    ...
    async def get_effective_total_volume(self) -> float:
        ...
        """
        open_vol_task = self.get_total_open_volume()
        open_vol = await open_vol_task if inspect.isawaitable(open_vol_task) else open_vol_task
        pending_vol = sum(float(v) for v in self.pending_orders.values())
        return float(open_vol) + float(pending_vol)
