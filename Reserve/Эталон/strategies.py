...
async def _prereqs_check(bot, symbol: str) -> bool:
    ...
    if symbol in bot.recently_closed:
        ...
        if now - bot.recently_closed[symbol] < cooldown_period:
            logger.debug(f"[{symbol}] находится в периоде 'остывания' после недавней сделки. Пропуск.")
            return False
...
async def golden_strategy(bot, symbol: str):
    ...
    try:
        ...
        # --- НАЧАЛО ИЗМЕНЕНИЙ: ЛОГИКА "ИНСАЙДЕРА" ---

        # Определяем критерии "элитного" сигнала
        vol_spike_ratio = (last_candle_volume / avg_volume_prev_4m) if avg_volume_prev_4m > 0 else 0
        ELITE_SIGNAL_VOLUME_SPIKE = 100.0 # <--- Порог для аномалии объема

        # Получаем сторону сделки
        ...
        # Если сигнал "элитный" - вызываем "Инсайдера"
        if vol_spike_ratio >= ELITE_SIGNAL_VOLUME_SPIKE:
            # Передаем управление "Инсайдеру" и выходим из стратегии
            asyncio.create_task(bot.execute_priority_trade(candidate))
            bot._last_golden_ts[symbol] = time.time() # Не забываем про кулдаун
            return 
            
        # --- КОНЕЦ ИЗМЕНЕНИЙ ---
        
        # Если сигнал обычный, он идет по стандартному пути
        ...
        logger.info(f"🏆 [{symbol}] GOLDEN SETUP V3 (Сжатая пружина)! OIΔ(5m): {oi_change_5m_pct:+.2f}%, ЦенаΔ(5m): {price_change_5m_pct:.2f}%, Vol Spike: x{vol_spike_ratio:.1f}. Направление: {side}.")
        
        # Отправляем в обычный process_signal
        asyncio.create_task(bot._process_signal(candidate, features, signal_key))
