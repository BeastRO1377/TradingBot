...
class MLXInferencer:
    ...
    def __init__(self, model_path: Path = config.ML_MODEL_PATH, scaler_path: Path = config.SCALER_PATH):
        self.model: Optional[GoldenNetMLX] = None
        self.scaler: Optional[StandardScaler] = None

        # --- НАЧАЛО ИЗМЕНЕНИЙ: Улучшенное логирование загрузки модели ---
        if model_path.exists():
            try:
                self.model = GoldenNetMLX(input_size=len(config.FEATURE_KEYS))
                self.model.load_weights(str(model_path))
                self.model.eval()
                logger.info(f"[MLXInferencer] Модель из {model_path} успешно загружена.")
            except Exception as e:
                logger.error(f"[MLXInferencer] Ошибка загрузки модели {model_path}: {e}", exc_info=True)
                self.model = None # Гарантируем, что модель будет None при ошибке
        else:
            logger.warning(f"[MLXInferencer] Файл модели не найден: {model_path}. ML-предсказания будут отключены.")
            
        if scaler_path.exists():
            try:
                self.scaler = joblib.load(scaler_path)
                logger.info(f"[MLXInferencer] Скейлер из {scaler_path} успешно загружен.")
            except Exception as e:
                logger.error(f"[MLXInferencer] Ошибка загрузки скейлера {scaler_path}: {e}", exc_info=True)
                self.scaler = None # Гарантируем, что скейлер будет None при ошибке
        else:
            logger.warning(f"[MLXInferencer] Файл скейлера не найден: {scaler_path}. ML-предсказания будут менее точными.")
        # --- КОНЕЦ ИЗМЕНЕНИЙ ---

    def infer(self, features: np.ndarray) -> np.ndarray:
        if self.model is None: 
            logger.warning("[MLXInferencer] Запрос инференса, но модель не загружена. Возвращаю 0.0.")
            return np.array([[0.0]])
...
def build_primary_prompt(candidate: dict, features: dict, shared_ws) -> str:
    """
    [ВЕРСИЯ С ML-СОВЕТНИКОМ] Единый промпт, который включает оценку
    от ML-модели.
    """
    sym = candidate.get("symbol", "UNKNOWN")
    side = str(candidate.get("side", "Buy")).upper()
    ...
    # --- НАЧАЛО ИЗМЕНЕНИЙ: Добавляем ML Score в промпт ---
    ml_score = features.get('ml_score', 0.5)
    ml_verdict = "Низкая"
    if ml_score > 0.75:
        ml_verdict = "Высокая"
    elif ml_score > 0.6:
    ...
    ml_prompt_line = f"- **Оценка ML-модели (вероятность успеха): {ml_score:.2f} ({ml_verdict})**"
    # --- КОНЕЦ ИЗМЕНЕНИЙ ---
    ...
    prompt_header = "SYSTEM: Ты — AI риск-менеджер Plutus. Твоя задача — дать четкий и однозначный вердикт: EXECUTE или REJECT, основываясь на всех данных, особенно на оценке от ML-модели. Твой ответ — всегда только валидный JSON."
    ...
    **2. КЛЮЧЕВЫЕ ПОКАЗАТЕЛИ:**
    ...
    # --- ИЗМЕНЕНИЕ: Упрощаем и усиливаем правила для AI ---
    prompt_task = """
    **3. ПРАВИЛА АНАЛИЗА:**
    Твоя главная задача — доверять ML-модели, так как она обучена на истории сделок этой системы.
    - **ЕСЛИ `Оценка ML-модели` > 0.70 (Высокая/Средняя):** Это очень сильный аргумент ЗА. Ищи подтверждения в других метриках (например, экстремальный RSI для сквиза). Если нет явных красных флагов, давай **EXECUTE**.
    - **ЕСЛИ `Оценка ML-модели` < 0.50 (Низкая):** Это очень сильный аргумент ПРОТИВ. Сигнал статистически невыгодный. Давай **REJECT**, даже если другие метрики выглядят неплохо. Обоснование: "Низкая вероятность успеха по ML-модели".
    - Во всех остальных случаях (оценка от 0.50 до 0.70) принимай решение на основе комплексного анализа всех показателей.
    """
    
    prompt_footer = """
    **4. ТВОЙ ВЕРДИКТ (JSON):**
    `{{"action": "EXECUTE" | "REJECT", "justification": "[Твое краткое экспертное заключение с упоминанием ML-оценки]"}}`
    """
