...
class TradingBot:
    def __init__(self, user_data: Dict[str, Any], shared_ws, golden_param_store: Dict):
        ...
        #self.take_profit_set: Dict[str, bool] = {} # Флаг, что TP для позиции уже установлен
    ...
    def _load_trade_counters_from_history(self):
        ...
        try:
            ...
            # Фильтруем только события открытия сделок
            open_trades = df[df['event'] == 'open'].copy()
            
            # Определяем ключ стратегии (упрощаем до 'squeeze' и 'golden_setup')
            ...
            # Считаем количество сделок по каждой стратегии
    ...
    def _should_allow_trade(self, source: str) -> tuple[bool, str]:
            """
            [ГИБКАЯ ВЕРСЯ] Проверяет, следует ли разрешить сделку,
            ...
            # Определяем ключ текущего сигнала
            ...
            # --- НОВАЯ ГИБКАЯ ЛОГИКА ---
            # Коэффициент "терпимости". 0.2 означает, что мы разрешаем превышение
            # квоты до тех пор, пока доля другой стратегии не упадет ниже 20% от ее цели.
            LEEWAY_FACTOR = 0.2 

            if current_signal_key == 'squeeze':
                # Проверяем, не слишком ли мало у нас "золотых сетапов"
                golden_ratio = golden_count / total_trades
                # Если доля голденов упала ниже (30% * 0.2 = 6%), то разрешаем сквиз в любом случае
                ...
                # Если с голденами все более-менее, проверяем квоту на сквизы
                ...
                # Проверяем, не слишком ли мало у нас сквизов
                squeeze_ratio = squeeze_count / total_trades
                # Если доля сквизов упала ниже (70% * 0.2 = 14%), разрешаем голден
                ...
                # Если со сквизами все более-менее, проверяем квоту на голдены
                golden_ratio = golden_count / total_trades
    ...
    async def execute_trade_entry(self, candidate: dict, features: dict):
        ...
        source = candidate.get("source", "N/A") # Нет точки, если source = "unknown"
        ...
        try:
            ...
            self.pending_strategy_comments[symbol] = source # Сохраняем не только комментарий, но и сам source
            ...
            for exec_data in msg.get("data", []):
                ...
                async with self.position_lock:
                    ...
                    if pos and pos.get("is_opening") and exec_data.get("side") == pos.get("side"):
                        ...
                        if exec_price > 0:
                            pos["avg_price"] = exec_price # Надежно сохраняем цену входа
                            pos["comment"] = self.pending_strategy_comments.get(symbol, "N/A")
                            pos["source"] = self.pending_strategy_comments.pop(symbol, "unknown") # Переносим source в pos
                    ...
                    # Проверяем, что это ордер на закрытие И что у нас есть позиция для закрытия
                    if pos and exec_data.get("side") != pos.get("side"):
                        ...
                        if utils.safe_to_float(exec_data.get("leavesQty", 0)) == 0:
                            ...
                            # --- НАЧАЛО ИСПРАВЛЕНИЯ PNL ---
                            entry_price = self._resolve_avg_price(symbol, pos)
                            
                            # КРИТИЧЕСКАЯ ПРОВЕРКА: если цены входа нет, мы не можем посчитать PnL.
                            # Это предотвратит ошибку с -1001$
                            if entry_price <= 0:
                                logger.warning(f"[{symbol}] Не удалось определить цену входа для закрытой позиции. PnL не будет рассчитан.")
                                self._purge_symbol_state(symbol) # Просто чистим состояние
                                continue
                            ...
                            # Используем правильную стоимость позиции для расчета процента
                    ...
                    # --- Сценарий 3: "Раннее" исполнение, позиции еще нет в self.open_positions ---
    ...
    async def _hunt_entry_point(self, candidate: dict, features: dict, signal_key: tuple):
            ...
            try:
                ...
                # --- Шаг 1: Получаем самые свежие данные, включая фандинг ---
                ...
                # Определяем пороги для "горячего" фандинга
                HOT_FUNDING_THRESHOLD = 0.04 # 0.04% - это уже значительный перекос

                # --- Шаг 2: Применяем жесткие правила на основе фандинга ---
                allow_entry = False
                rejection_reason = ""

                if side == "Sell":
                    if funding_rate >= HOT_FUNDING_THRESHOLD:
                        # Идеальный сценарий: шортим, когда лонгисты платят много. Идем против толпы.
                        allow_entry = True
                    elif funding_rate < 0:
                        # Плохой сценарий: шортим, когда шортисты уже платят. Идем вместе с толпой.
                    ...
                    if funding_rate <= -HOT_FUNDING_THRESHOLD:
                        # Идеальный сценарий: лонгуем, когда шортисты платят много.
                        allow_entry = True
                    elif funding_rate > 0:
                        # Плохой сценарий: лонгуем, когда лонгисты уже платят.
                ...
                if allow_entry:
                    ...
                    # Финальная быстрая проверка у тактического AI
                    last_price = current_features.get("price", 0)
                    # В качестве reference_price передаем просто последнюю цену, т.к. экстремум нам уже не важен
    ...
    async def _ai_confirm_entry(self, symbol: str, side: str, reference_price: float, source: str) -> bool:
            ...
            try:
                ...
                if 'squeeze' in source:
                    # --- ИЗМЕНЕНИЕ: Получаем и передаем фандинг ---
    ...
    async def manage_open_position(self, symbol: str):
            ...
            try:
                ...
                # --- НОВАЯ ЛОГИКА: ТАЙМЕР НАДЕЖДЫ ДЛЯ СКВИЗОВ ---
                source = pos.get("source", "")
                is_squeeze = 'squeeze' in source.lower()
                
                if is_squeeze:
                    HOPE_TIMEOUT_SEC = 10 * 60 # 10 минут
                    start_time = time.time()
                    
                    while time.time() - start_time < HOPE_TIMEOUT_SEC:
                        # Проверяем, существует ли еще позиция
                        current_pos = self.open_positions.get(symbol)
                        if not current_pos: return # Позиция была закрыта, выходим
                        
                        # Получаем текущую цену и считаем ROI
                        last_price = utils.safe_to_float(self.shared_ws.ticker_data.get(symbol, {}).get("lastPrice"))
                        ...
                        if last_price > 0 and avg_price > 0:
                            ...
                            # Считаем PnL без плеча для чистой проверки
                            if pnl >= 0:
                                logger.info(f"[{symbol}] Позиция вышла в плюс. 'Таймер надежды' отключен.")
                                break # Выходим из цикла таймера, дальше работает обычный трейлинг
                        
                        await asyncio.sleep(30) # Проверяем каждые 30 секунд
                    
                    else: # Этот блок сработает, если цикл завершился без break
                        ...
                        return # Завершаем работу хранителя

                # --- КОНЕЦ НОВОЙ ЛОГИКИ ---

                # Бесконечный цикл для AI-советника (если он нужен для других задач)
                # или просто для поддержания "Хранителя" в живых.
                while symbol in self.open_positions:
                    await asyncio.sleep(300) # Просыпаемся раз в 5 минут
    ...
    async def close_position(self, symbol: str):
            ...
            try:
                side = pos.get("side")
                # Для закрытия отправляем ордер в противоположную сторону
    ...
    async def _ai_advise_on_position(self, symbol: str):
        ...
        try:
            ...
            # Добавляем в 'pos' все необходимые данные для анализа
            ...
            if action == "ADJUST_SL":
                ...
                if new_price > 0:
                    ...
                    # Используем универсальный метод, но без TP
                    await self.set_or_amend_stop_loss(new_price, symbol=symbol)

            # --- ГЛАВНОЕ ИЗМЕНЕНИЕ: БОЛЬШЕ НИКАКИХ ДЕЙСТВИЙ ---
            if action == "SET_DYNAMIC_TP_AND_TRAIL":
                ...
                if tp_price > 0 and new_gap_pct > 0:
                    ...
                    # Просто сохраняем новые параметры. on_ticker_update их подхватит.
    ...
    async def _calculate_fibonacci_stop_price(self, symbol: str, side: str) -> Optional[float]:
            ...
            try:
                ...
                # --- НОВАЯ ЛОГИКА ---
                if side.lower() == "buy":
                    # Вошли в лонг ПОСЛЕ ПАДЕНИЯ. Считаем от ДНА (lowest_low).
                    fib_level = lowest_low + (price_range * FIB_LEVEL)
                    return fib_level
                else: # side.lower() == "sell"
                    # Вошли в шорт ПОСЛЕ РОСТА. Считаем от ВЕРШИНЫ (highest_high).
    ...
    async def _set_initial_stop_loss(self, symbol: str, pos: dict):
        ...
        try:
            ...
            if is_squeeze:
                candles = list(self.shared_ws.candles_data.get(symbol, []))
                # Нам нужна как минимум последняя свеча, которая и была триггером
                if candles:
                    # --- НОВАЯ ЛОГИКА: СТОП НА ОСНОВЕ РАЗМЕРА СКВИЗ-СВЕЧИ ---
                    impulse_candle = candles[-1] # Берем самую последнюю минутную свечу
                    
                    high_price = utils.safe_to_float(impulse_candle.get("highPrice"))
                    low_price = utils.safe_to_float(impulse_candle.get("lowPrice"))
                    
                    # Рассчитываем "моментальную волатильность"
                    candle_range = high_price - low_price
                    
                    # Коэффициент. 0.5 означает, что мы ставим стоп на расстоянии
                    # в половину длины свечи от ее экстремума.
                    STOP_RANGE_MULTIPLIER = 3.0

                    if candle_range > 0:
                        if side == "buy": # Сквиз был вниз
                            # Стоп = цена входа - (размер свечи * коэф.)
                            stop_price = avg_price - (candle_range * STOP_RANGE_MULTIPLIER)
                        else: # side == "sell", сквиз был вверх
                            # Стоп = цена входа + (размер свечи * коэф.)
                            stop_price = avg_price + (candle_range * STOP_RANGE_MULTIPLIER)
            ...
            # --- ФОЛЛБЭК для Golden Setup и случаев, когда расчет не удался ---
            if not stop_price:
                if is_squeeze:
            ...
            # Защита от логических ошибок остается на месте
            ...
            try:
                ...
                if response.get("retCode") == 0:
                    ...
                    if "takeProfit" in params:
                        pos["tp_set"] = True # Ставим флаг, что TP установлен
                        log_msg += f" TP={params['takeProfit']}"
                    logger.info(log_msg)
                # Ошибку "not modified" теперь будет обрабатывать блок except

            except InvalidRequestError as e:
                # --- УЛУЧШЕННАЯ ОБРАБОТКА ОШИБОК ---
                if "not modified" in str(e).lower() or "34040" in str(e):
                    # Это не ошибка, а штатная ситуация. Логируем как INFO, а не ERROR.
                    logger.info(f"[{symbol}] SL/TP уже установлен на этом уровне (API: not modified).")
                    # Обновляем наше состояние, чтобы анти-спам работал
    ...
    def _purge_symbol_state(self, symbol: str):
            ...
            self.recently_closed[symbol] = time.time()
            self.trailing_activated.pop(symbol, None)
            #self.take_profit_set.pop(symbol, None)
    ...
    # async def on_ticker_update(self, symbol: str, last_price: float):
    #     """
    #     [ФИНАЛЬНАЯ ВЕРСИЯ V6] Гарантирует работу трейлинга и правила "грааля",
    #     корректно используя данные от AI-советника.
    #     """
    #     pos = self.open_positions.get(symbol)
    #     if not pos:
    #         await strategies.high_frequency_dispatcher(self, symbol)
    ...
    #     if pos.get("is_updating_sl"):
    #         return

    #     start_roi_pct, default_gap_pct = self._get_trailing_params()
    #     # Используем динамический гэп от AI, если он был установлен, иначе дефолтный
    ...
    #     # --- РАСЧЕТЫ ВЫНЕСЕНЫ В ОБЩУЮ ЧАСТЬ ---
    #     side = pos.get("side", "Buy")
    ...
    #     # --- ЛОГИКА АКТИВАЦИИ И УПРАВЛЕНИЯ ---

    #     # Сценарий 1: Трейлинг еще НЕ активирован
    #     if not self.trailing_activated.get(symbol):
    #         if current_roi >= start_roi_pct:
    #             # Активируем полноценный трейлинг и ставим TP
    #             self.trailing_activated[symbol] = True
    #             logger.info(f"✅ [{symbol}] ТРЕЙЛИНГ АКТИВИРОВАН! ROI: {current_roi:.2f}%")
    ...
    #             if atr15m > 0:
    #                 ATR_MULTIPLIER = 2.5
    #                 price_at_activation = last_price
    #                 if side == "Buy": tp_price_to_set = price_at_activation + (atr15m * ATR_MULTIPLIER)
    #                 else: tp_price_to_set = price_at_activation - (atr15m * ATR_MULTIPLIER)
    #                 logger.info(f"🎯 [{symbol}] Рассчитан первоначальный предиктивный TP на {tp_price_to_set:.6f}")
        
    #     # Сценарий 2: Трейлинг УЖЕ активирован
    #     if self.trailing_activated.get(symbol):
    #         # Если AI дал нам новую цель TP, используем ее
    #         if pos.get('target_tp_price'):
    #             tp_price_to_set = pos.pop('target_tp_price') # Используем и удаляем, чтобы не ставить повторно

    #         # Расчет стопа по "правилу грааля" (+2.5% PnL)
    #         breakeven_plus_stop_roi = (2.5 / leverage)
    ...
    #         # Расчет обычного трейлинг-стопа с учетом гэпа
    #         target_roi = current_roi - gap_roi_pct
    #         denom = 1.0 + (target_roi / (100.0 * leverage))
    #         new_trail_price = (avg_price * denom) if side == "Buy" else (avg_price / denom if denom > 1e-9 else 0.0)
            
    #         # Выбираем ЛУЧШИЙ (самый безопасный) из двух стопов
    #         if side == "Buy":
    #             new_stop_price = max(new_trail_price, breakeven_plus_stop_price)
    #         else:
    #             # Для шорта min() может выбрать неправильный стоп, если new_trail_price < 0.
    #             # Поэтому нужна дополнительная проверка.
    #             if new_trail_price > 0:
    #                 new_stop_price = min(new_trail_price, breakeven_plus_stop_price)
    #             else:
    #                 new_stop_price = breakeven_plus_stop_price
        
    #     # --- ФИНАЛЬНЫЙ БЛОК ОТПРАВКИ ОРДЕРА ---
    #     if new_stop_price > 0 or tp_price_to_set is not None:
    #         prev_stop = self.last_stop_price.get(symbol)
    #         # Проверяем, что стоп улучшился ИЛИ нам нужно установить/изменить тейк-профит
    #         is_better = prev_stop is None or \
    #                     (side == "Buy" and new_stop_price > prev_stop) or \
    #                     (side == "Sell" and new_stop_price < prev_stop)
    ...
    #                     new_stop_price if is_better else 0, # Отправляем 0, если стоп не улучшился, но нужно поставить TP
    #                     symbol=symbol, 
    ...
    async def on_ticker_update(self, symbol: str, last_price: float):
            ...
            # --- НОВАЯ ЛОГИКА С "КАРАНТИНОМ" ---

            # 1. АКТИВАЦИЯ (срабатывает один раз)
            if not self.trailing_activated.get(symbol) and current_roi >= start_roi_pct:
                self.trailing_activated[symbol] = True
                self.trailing_activation_ts[symbol] = time.time() # Запоминаем время активации
                logger.info(f"✅ [{symbol}] ТРЕЙЛИНГ АКТИВИРОВАН! ROI: {current_roi:.2f}%. Включен 'карантин' на 3 минуты.")
                
                # Сразу устанавливаем предиктивный TP
                features = await self.extract_realtime_features(symbol)
                atr15m = features.get("atr15m", 0.0)
                if atr15m > 0:
                    ATR_MULTIPLIER = 2.5
                    ...
                    logger.info(f"🎯 [{symbol}] Рассчитан предиктивный TP на {tp_price_to_set:.6f}")
                    # Сразу отправляем TP на биржу, не трогая стоп
                    await self.set_or_amend_stop_loss(0, symbol=symbol, take_profit_price=tp_price_to_set)

            # 2. УПРАВЛЕНИЕ СТОПОМ (только если трейлинг активен)
            if self.trailing_activated.get(symbol):
                
                # --- ПРОВЕРКА "КАРАНТИНА" ---
                QUARANTINE_DURATION_SEC = 3 * 60 # 3 минуты
                activation_time = self.trailing_activation_ts.get(symbol, 0)
                
                # Если 3 минуты с момента активации еще не прошли, мы НЕ ДВИГАЕМ СТОП.
                if time.time() - activation_time < QUARANTINE_DURATION_SEC:
                    await strategies.high_frequency_dispatcher(self, symbol)
                    return 
                # --- КОНЕЦ ПРОВЕРКИ ---

                # Если карантин прошел, включаем агрессивный трейлинг
                if pos.get('target_tp_price'):
                    tp_price_to_set = pos.pop('target_tp_price')

                # Расчет стопа по "правилу грааля"
                grail_roi_level = 2.5 
                ...
                # Отправляем ордер, если стоп улучшился или нужно обновить TP
                if new_stop_price > 0 or tp_price_to_set is not None:
                    prev_stop = self.last_stop_price.get(symbol)
                    is_better = prev_stop is None or \
                                (side == "Buy" and new_stop_price > prev_stop) or \
    ...
    async def extract_realtime_features(self, symbol: str) -> Optional[Dict[str, float]]:
        ...
        def _safe_last(series, default):
                ...
                # 1. Проверяем, что series вообще существует (не None)
                # 2. Проверяем, что это объект pandas Series (на всякий случай)
                # 3. Проверяем, что он не пустой
                ...
                try:
                    ...
                    v = series.iloc[-1]
                    # Проверяем, что это не "NaN" (не число)
                    return v if pd.notna(v) else default
                except IndexError:
                    # Эта ошибка может возникнуть, если серия пуста,
                    # но мы уже проверили это выше. Оставляем для надежности.
                    return default
        ...
        # --- ДОБАВЬТЕ ЭТОТ БЛОК ---
        # Агрегируем свечи до 15-минутного таймфрейма для расчета ATR
        candles_15m = self._aggregate_candles_15m(candles) # Нам понадобится эта новая функция
        ...
        # Расчет тренда на H1
        candles_1h = self._aggregate_candles_60m(candles) # Нужна новая функция-агрегатор
        trend_h1 = 0
        if len(candles_1h) > 2:
            # Простое определение тренда по последним двум часовым свечам
    ...
    def _aggregate_candles_60m(self, candles: list) -> list:
            ...
            for i in range(0, len(candle_list), 60):
                ...
                if len(chunk) < 60: continue # Пропускаем неполные блоки
    ...
    async def log_trade(self, **kwargs):
        ...
        source = str(kwargs.get("source", "unknown")) # <-- Получаем source
        ...
        try:
            ...
            # --- ИНКРЕМЕНТИРУЕМ СЧЕТЧИК ПОСЛЕ УСПЕШНОЙ ЗАПИСИ ---
            if action == "open":
                strategy_key = None
