...
class TradingBot:
    ...
    async def _process_signal(self, candidate: dict, features: dict, signal_key: tuple):
            ...
            try:
                ...
                if action == "EXECUTE":
                    ...
                    # Эта логика теперь будет работать и для 'squeeze', и для 'golden_setup'
    ...
    async def on_ticker_update(self, symbol: str, last_price: float):
        """
        [УМНЫЙ КАРАНТИН V9] Внедрен порог ROI для досрочного выхода из периода
        стабилизации, что позволяет быстро реагировать на сильные движения.
        """
        # --- Шаг 1: Главная проверка. Если позиции нет - работаем только как искатель сигналов.
        pos = self.open_positions.get(symbol)
        if not pos:
            await strategies.high_frequency_dispatcher(self, symbol)
            return
        ...
        # Защита от спама через "замок" на уровне позиции
        if pos.get("is_updating_sl"):
            return

        # --- Шаг 2: Если позиция есть, выполняем логику управления.
        ...
        # --- Шаг 3: Активация трейлинга (срабатывает один раз) ---
        if not pos.get("trailing_activated") and current_roi >= start_roi_pct:
            pos["trailing_activated"] = True
            pos["trailing_activation_ts"] = time.time() # Запоминаем время активации
            logger.info(f"✅ [{symbol}] ТРЕЙЛИНГ АКТИВИРОВАН! ROI: {current_roi:.2f}%. Включен 'умный карантин'.")
        ...
        # --- Шаг 4: Управление стопом (только если трейлинг активен) ---
        if pos.get("trailing_activated"):
            
            # --- ЛОГИКА "УМНОГО КАРАНТИНА" ---
            QUARANTINE_DURATION_SEC = 3 * 60  # 3 минуты
            EARLY_EXIT_ROI_THRESHOLD = 10.0  # Порог ROI для досрочного выхода
            activation_time = pos.get("trailing_activation_ts", 0)
            
            is_in_quarantine = True
            # Проверяем, вышло ли время
            if time.time() - activation_time >= QUARANTINE_DURATION_SEC:
                is_in_quarantine = False
                if not pos.get("quarantine_ended_log"):
                    logger.info(f"⏳ [{symbol}] Карантин завершен по времени. Включается агрессивный трейлинг.")
                    pos["quarantine_ended_log"] = True # Логируем только один раз
            # Проверяем, достигнут ли порог ROI
            elif current_roi >= EARLY_EXIT_ROI_THRESHOLD:
                is_in_quarantine = False
                if not pos.get("quarantine_ended_log"):
                    logger.info(f"🚀 [{symbol}] Карантин завершен досрочно! ROI достиг {current_roi:.2f}% (>{EARLY_EXIT_ROI_THRESHOLD}%)")
                    pos["quarantine_ended_log"] = True # Логируем только один раз
            
            # Если мы все еще в карантине, стоп НЕ ДВИГАЕМ.
            if is_in_quarantine:
                await strategies.high_frequency_dispatcher(self, symbol)
                return
            # --- КОНЕЦ ЛОГИКИ "УМНОГО КАРАНТИНА" ---

            # Если карантин прошел, включаем агрессивный трейлинг
            if pos.get('target_tp_price'):
                tp_price_to_set = pos.pop('target_tp_price', None)

            # Расчет стопа по "правилу грааля"
            grail_roi_level = 2.5 
            breakeven_plus_stop_roi = (grail_roi_level / leverage)
            breakeven_plus_stop_price = 0.0
