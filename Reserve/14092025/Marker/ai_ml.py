...
def build_primary_prompt(candidate: dict, features: dict, shared_ws) -> str:
    ...
    **2. КЛЮЧЕВЫЕ ПОКАЗАТЕЛИ:**
    ...
    if 'squeeze' in source:
        prompt_task = """
        **3. ЗАДАЧА (Squeeze):**
        Сигнал прошел базовую проверку по RSI. Твоя задача — провести финальную проверку.
        - **Оцени ADX:** Слишком низкий (<20) или слишком высокий (>65) ADX — это плохо.
        - **Оцени фандинг:** Вход против толпы (например, Sell при высоком положительном фандинге) — это хорошо.
        Если нет явных красных флагов, давай EXECUTE.
        """
        ...
        **Контекст:** Обнаружен паттерн накопления.
        
        **КРИТИЧЕСКИЙ ШАГ: Проверка на перегретость.**
        - **ЕСЛИ сигнал 'Buy', а `Контекст (30м)` > 3.5%:** Верни "REJECT". Причина: "Риск покупки на локальном максимуме".
        - **ЕСЛИ сигнал 'Sell', а `Контекст (30м)` < -3.5%:** Верни "REJECT". Причина: "Риск продажи на локальном минимуме".

        - **В противном случае (контекст благоприятный):** Сигнал перспективен. **Верни "WATCH"**, чтобы передать его "Охотнику" для поиска точной точки входа.
        """
...
def build_golden_entry_prompt(symbol: str, side: str, reference_price: float, last_price: float, features: dict) -> str:
    """
    [ТАКТИЧЕСКИЙ] Создает промпт для AI, который ищет подтверждение
    пробоя после фазы накопления, используя фандинг и ADX.
    """
    funding_rate = features.get("funding_rate", 0.0)
    adx_val = safe_to_float(features.get('adx14'))

    # --- Python надежно проверяет условия ---
    adx_ok = adx_val > 25
    funding_ok = (side.upper() == 'BUY' and funding_rate >= 0) or \
                    (side.upper() == 'SELL' and funding_rate <= 0)

    prompt = f"""
    SYSTEM: Ты — AI-аналитик, эксперт по подтверждению пробоев. Твоя задача — дать финальное ДА/НЕТ на вход. Ответ — только JSON.
    USER:
    **1. СИГНАЛ (Golden Setup - "Сжатая пружина"):**
    - Инструмент: {symbol}, Ожидаемое направление: {side.upper()}

    **2. РЕЗУЛЬТАТЫ СИСТЕМНОГО АНАЛИЗА:**
    - Сила тренда достаточна (ADX > 25): **{adx_ok}**
    - Фандинг не противоречит направлению: **{funding_ok}**

    **3. АЛГОРИТМ ПРИНЯТИЯ РЕШЕНИЯ:**
    - **ЕСЛИ** `Сила тренда` == `True` **И** `Фандинг не противоречит` == `True`:
        Вернуть "EXECUTE".
    - **ИНАЧЕ:** Вернуть "WAIT".

    **4. ФОРМАТ ОТВЕТА (JSON):**
    - "action": "EXECUTE" или "WAIT"
    - "reason": "ADX и фандинг подтверждают начало импульса." или "ADX/фандинг не соответствуют."
    """
    return prompt.strip()
