...
def _append_trades_unified(row: dict) -> None:
    """
    [ИСПРАВЛЕННАЯ ВЕРСЯ] Добавляет строку в CSV-файл с историей всех сделок,
    ...
    try:
        ...
        file_exists = config.TRADES_UNIFIED_CSV_PATH.is_file()
        
        with open(config.TRADES_UNIFIED_CSV_PATH, "a", newline="", encoding="utf-8") as f:
