...
async def squeeze_strategy(bot, symbol: str) -> bool:
    ...
    try:
        ...
        # --- НОВЫЙ ЖЕЛЕЗОБЕТОННЫЙ ФИЛЬТР ---
        # 1. Проверяем RSI. Если он не в экстремальной зоне, даже не рассматриваем.
        features = await bot.extract_realtime_features(symbol)
        if not features: return False
        
        rsi_val = features.get('rsi14', 50.0)
        side = "Sell" if pct_change_1m_body > 0 else "Buy"
        
        is_rsi_extreme = (side == 'Sell' and rsi_val > 75) or \
                         (side == 'Buy' and rsi_val < 25)

        if not is_rsi_extreme:
            # logger.debug(f"[{symbol}] Сквиз-импульс есть, но RSI ({rsi_val:.1f}) не в зоне. Сигнал пропущен.")
            return False
        # --- КОНЕЦ ФИЛЬТРА ---
