...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler("GoldenML_V5_19.log", maxBytes=5 * 1024 * 1024, backupCount=2),
...
async def monitor_position():
    ...
    symbol = "1000CATSUSDT"
    position_status = await check_position_http(symbol)
    
    if position_status:
        logger.info(f"Position status for {symbol}: {position_status}")
    else:
        logger.error(f"Failed to retrieve position status for {symbol}")
