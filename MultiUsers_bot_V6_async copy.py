...
class TradingBot:
    ...
    async def init_websocket(self):
        """Инициализация WebSocket с проверкой соединения"""
        try:
            logger.info("[WebSocket] Запуск WS...")
            await self.update_open_positions_from_exch_positions()  # <- ДО запуска WS
            # await self.start_ws_position_monitor()
            ...
            # Явная активация соединения
            # await self.ws.connect()
            
            # Подписка на каналы
            # await self.ws.subscribe(["position", "order"])
            # logger.info(f"WebSocket: подключено для {self.user_id}")

            # @self.ws.on("position")
            # async def _on_position(message):
            #     await self.handle_position_update(message)

            # @self.ws.on("order")
            # async def _on_order(message):
            #     await self.handle_order_update(message)

            def position_callback(data):
                if self.loop and not self.loop.is_closed():
                    asyncio.run_coroutine_threadsafe(self.handle_position_update(data), self.loop)
                else:
                    logger.error("Нет активного event loop в position_callback")

            def order_callback(data):
                if self.loop and not self.loop.is_closed():
                    asyncio.run_coroutine_threadsafe(self.handle_order_update(data), self.loop)
                else:
                    logger.error("Нет активного event loop в order_callback")

            self.ws.subscribe("position", callback=position_callback)
            self.ws.subscribe("order", callback=order_callback)
            logger.info(f"WebSocket: подключено для {self.user_id}")
            
            # Сразу после установки WebSocket-соединения обновляем начальный snapshot позиций
            self.get_exchange_positions()
    ...
    async def handle_position_update(self, message):
        ...
        try:
            if not isinstance(message, dict):
                logger.info("Получено некорректное сообщение (не словарь)")
                return

            # Извлекаем данные позиций – они могут лежать в message['data'] или message['result']
            positions_data = message.get("data", [])
            if not positions_data:
                logger.info("Получены пустые данные по позициям")
                return

            # Перебираем список позиций
            for pos_info in positions_data:
                symbol = pos_info.get("symbol")
                logger.info(f"Обработка позиции для {symbol}: {pos_info}")

                size = pos_info.get("size")       # Текущее количество (например, 0.005 BTC)
                side = pos_info.get("side")       # Buy / Sell
                entry_price = pos_info.get("entryPrice")
                mark_price = pos_info.get("markPrice")

                self.ws_positions[symbol] = pos_info

                try:
                    size = Decimal(str(pos_info.get("size", "0")))
                    mark_price = Decimal(str(pos_info.get("markPrice", "0")))
                    if size > 0 and mark_price > 0:
                        pos_info["position_volume"] = (size * mark_price).quantize(Decimal("0.0001"))
                except Exception as e:
                    logger.warning(f"Не удалось вычислить position_volume для {symbol}: {e}")
                    logger.info(f"[VOLUME] Суммарный объём открытых позиций: {await self.get_total_open_volume()} USDT")

                logger.info(f"[handle_position_update] Обновление для {symbol} получено, время = {datetime.datetime.utcnow().isoformat()}, данные: {pos_info}")

                if symbol:
                    # Обновляем актуальный markPrice и entryPrice для данного символа,
                    # если данные присутствуют, иначе выводим предупреждение.
                    if mark_price is not None:
                        self.latest_mark_prices[symbol] = mark_price
                    else:
                        logger.warning(f"handle_position_update: markPrice отсутствует для {symbol}")
                    if entry_price is not None:
                        self.latest_entry_prices[symbol] = entry_price
                    else:
                        logger.warning(f"handle_position_update: entryPrice отсутствует для {symbol}")
                        logger.info(f"[handle_position_update] {symbol}: Обновление markPrice = {mark_price}, entryPrice = {entry_price}")


                # Если позиция закрыта (size отсутствует или равен 0), логируем закрытие и удаляем запись:
                if symbol and (size is None or float(size) == 0):
                #    await self.refresh_positions_from_stream()
                    self.ws_positions.pop(symbol, None)
                        # 🛠️ ВАЖНО: также удаляем из open_positions
                    async with self.open_positions_lock:
                        if symbol in self.open_positions:
                            del self.open_positions[symbol]
                ...
                if symbol and size is not None and float(size) > 0:
                    async with self.open_positions_lock:
                        self.open_positions[symbol] = dict(pos_info)  # <- Явная копия, не ссылка

                # Иначе, если позиция открыта и имеются необходимые данные, обновляем self.open_positions
                # if symbol and side and entry_price is not None:
                #     async with self.open_positions_lock:
            ...
            # После обработки всех позиций вызываем метод управления позицией для каждого символа,
            # чтобы, например, выставить кастомный стоп-лосс на основе актуального markPrice.
            # for pos_info in positions_data:
            #     sym = pos_info.get('symbol')
            #     if sym:
            #         await asyncio.sleep(0.5)
            #         await self.check_and_manage_position(sym)

        except Exception as e:
            logger.exception(f"Ошибка handle_position_update: {e}")
    ...
    async def handle_order_update(self, message):
        try:
            ...
            for ord_info in data:
                ...
                order_status = ord_info.get("orderStatus")  # 'New', 'PartiallyFilled', 'Filled' и т.д.
                side = ord_info.get("side")
                # Прочие поля, например filledQty и т.д.
                
                # Если ордер полностью исполнен, можно обновить позицию,
                # либо просто залогировать:
                if order_status == "Filled":
                    logger.info(f"Ордер по {symbol} ({side}) полностью исполнен!")
                    # Если хотите затем что-то сделать: усреднить, закрыть и т.д.
                    # Вызывайте свои методы:
                    await self.check_and_manage_position(symbol)

        except Exception as e:
            logger.exception(f"Ошибка handle_order_update: {e}")
                         
    async def check_and_manage_position(self, symbol):
        # await self.refresh_positions_from_stream()  # Обновляем snapshot
        # async with self.open_positions_lock:
        pos = self.ws_positions.get(symbol)
        if not pos:
            logger.warning(f"Нет позиции в ws_positions для {symbol}")
            return

        # Получаем актуальный markPrice
        current_price = self.latest_mark_prices.get(symbol)
        if current_price is None:
            logger.warning(f"Нет актуального markPrice для {symbol} – пытаюсь взять из pos")
            current_price = pos.get("markPrice")
            if current_price is None or current_price == "0":
                logger.warning(f"Не удалось получить markPrice даже из pos для {symbol}")
                return
            else:
                self.latest_mark_prices[symbol] = current_price
        try:
            current_price_dec = Decimal(str(current_price))
        except Exception as e:
            logger.exception(f"Ошибка преобразования markPrice для {symbol}: {current_price}")
            return

        entry_price = self.latest_entry_prices.get(symbol)
        if entry_price is None:
            logger.warning(f"Нет entryPrice для {symbol}")
            return
        try:
            entry_price_dec = Decimal(str(entry_price))
        except Exception as e:
            logger.exception(f"Ошибка преобразования entryPrice для {symbol}: {entry_price}")
            return

        if entry_price_dec == 0:
            logger.error(f"entryPrice для {symbol} равен 0 – пропускаем расчет pnl.")
            return

        logger.info(f"[check_and_manage_position] {symbol}: Последнее entryPrice = {entry_price}, Последнее markPrice = {current_price}")

        leverage = Decimal('10')
        if pos.get("side", "").lower() == "buy":
            pnl_ratio = (current_price_dec - entry_price_dec) / entry_price_dec
        else:
            pnl_ratio = (entry_price_dec - current_price_dec) / entry_price_dec

        leveraged_pnl_percent = pnl_ratio * leverage * Decimal('100')
        logger.info(f"[check_and_manage_position] {symbol}: entryPrice={entry_price_dec}, markPrice={current_price_dec}, pnl%={leveraged_pnl_percent}, trailing_stop_set={pos.get('trailing_stop_set')}")

        if leveraged_pnl_percent >= Decimal('5') and not pos.get('trailing_stop_set'):
            logger.info(f"[check_and_manage_position] {symbol}: Условие для трейлинг-стопа выполнено, запускаю apply_custom_trailing_stop")
            try:
                logger.info(f"[check_and_manage_position] {symbol}: Расчет pnl_ratio = {pnl_ratio}, leveraged_pnl_percent = {leveraged_pnl_percent}, trailing_stop_set = {pos.get('trailing_stop_set')}")
                await self.apply_custom_trailing_stop(symbol, pos, leveraged_pnl_percent, pos.get("side"))
                async with self.open_positions_lock:
                    self.open_positions[symbol]['trailing_stop_set'] = True
                self.ws_positions[symbol]['trailing_stop_set'] = True  # ← ДОБАВЬ ЭТО
                logger.info(f"[check_and_manage_position] {symbol}: Кастомный стоп-лосс успешно установлен")
            except Exception as e:
                logger.exception(f"[check_and_manage_position] {symbol}: Ошибка установки трейлинг-стопа: {e}")
        else:
            logger.info(f"[check_and_manage_position] {symbol}: Условие для трейлинг-стопа не выполнено. pnl%={leveraged_pnl_percent}, trailing_stop_set={pos.get('trailing_stop_set')}")
    ...
    async def sync_ws_to_open_positions(self):
        async with self.open_positions_lock:
            for sym, ws_pos in self.ws_positions.items():
                if float(ws_pos.get("size", 0)) > 0:
                    self.open_positions[sym] = dict(ws_pos)  # <- ЯВНАЯ КОПИЯ
                elif sym in self.open_positions:
    ...
    async def safe_reconnect(self):
        """Улучшенное переподключение с защитой от None"""
        max_retries = 5
        for attempt in range(max_retries):
            try:
                # Если соединение существует, завершаем его
                if self.ws:
                    ...
                    # Если метода close нет, просто сбросим объект
                self.ws = None
                await self.init_websocket()
                return
                    
            except Exception as e:
                logger.error(f"Попытка {attempt+1}/{max_retries} ошибка: {str(e)}")
                await asyncio.sleep(2 ** attempt)
        logger.critical("Превышено максимальное число переподключений")
        self.active = False
